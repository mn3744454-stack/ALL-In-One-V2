...
DECLARE
  ...
  v_item_allowed constant text[] := ARRAY[
    ...
    'period_start', 'period_end', 'package_id', 'is_taxable', 'tax_rate'
  ];
BEGIN
...
-- Re-affirm narrow SELECT grants (idempotent). Writes remain RPC-only.
GRANT SELECT ON TABLE public.payment_allocations TO authenticated;
GRANT SELECT ON TABLE public.payment_horse_allocations TO authenticated;
