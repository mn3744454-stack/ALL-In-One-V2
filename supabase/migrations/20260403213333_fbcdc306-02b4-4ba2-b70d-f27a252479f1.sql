...
-- Add phone-based identity verification to finalize_invitation_acceptance
-- If the invitation was sent via phone only (invitee_email IS NULL, invitee_phone IS NOT NULL),
-- verify that the authenticated user's profile phone matches the invited phone.
...
BEGIN
  ...
  -- Email-based identity verification (existing behavior)
  ...
  -- Phone-based identity verification (new behavior)
  -- Only applies when invitation was sent via phone only (no email)
  IF v_inv.invitee_email IS NULL AND v_inv.invitee_phone IS NOT NULL THEN
    ...
    IF regexp_replace(v_inv.invitee_phone, '[^0-9+]', '', 'g') <> regexp_replace(v_user_phone, '[^0-9+]', '', 'g') THEN
  ...
  DO UPDATE SET
    ...
    phone = COALESCE(EXCLUDED.phone, hr_employees.phone),
    updated_at = now();
