-- 2QA-C closure — rollback-safe trigger rejection tests. All changes are
-- rolled back inside savepoints. A final RAISE writes results into a temp
-- table that this migration drops at the end. No data persists.
DO $$
DECLARE
...
BEGIN
  ...
  SELECT c.id INTO b_cat FROM public.tenant_service_categories c WHERE c.tenant_id <> a_tenant LIMIT 1;
  ...
  IF NOT (r_ls AND r_ts AND r_ltt) THEN
    RAISE EXCEPTION '2QA-C rejection tests FAILED: ls=%, ts=%, ltt=%', r_ls, r_ts, r_ltt;
  END IF;
