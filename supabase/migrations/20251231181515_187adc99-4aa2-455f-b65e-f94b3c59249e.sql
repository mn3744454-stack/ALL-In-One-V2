-- Historical mock laboratory results.
--
-- This migration originally assumed that the Lovable demo tenant, actor, template,
-- and two sample rows already existed. A clean database rebuild does not contain
-- those business fixtures, and the lab result validation trigger correctly rejects
-- the hard-coded sample identifiers. Keep the historical demo behavior when every
-- dependency is present; otherwise skip only these optional mock rows so the
-- version-controlled schema remains rebuildable.
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM public.tenants
    WHERE id = '61192a20-6f07-441f-8cfc-0a6584426fcd'
  )
  AND EXISTS (
    SELECT 1
    FROM public.profiles
    WHERE id = '343dc041-bd77-4926-ae2a-16897e7859a2'
  )
  AND EXISTS (
    SELECT 1
    FROM public.lab_templates
    WHERE id = '4324055e-8300-4585-af7c-fa72d3a7a839'
      AND tenant_id = '61192a20-6f07-441f-8cfc-0a6584426fcd'
  )
  AND (
    SELECT count(*)
  ...
  ELSE
    RAISE NOTICE
      'Skipping historical mock lab results: required Lovable demo fixtures are absent';
  END IF;
END
$mock_lab_results$;
