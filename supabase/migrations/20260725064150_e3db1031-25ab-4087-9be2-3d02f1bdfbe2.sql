-- ============================================================================
-- Phase N+1B · J5.2-SLICE-01 · Turn 2R · Migration A
-- Atomic Source Checkout Contract (Lab Sample + Horse Order)
--
-- Authorized objects (exactly two):
--   public._finance_source_checkout_apply_trace(uuid,uuid,text,uuid)  -- private, NEW
--   public.create_source_checkout_invoice(uuid,uuid,jsonb)             -- public, REPLACE
--
-- Composition:
--   outer idempotency
--     -> source lock + validation
--     -> active same-kind link conflict guard
--     -> create_invoice_with_items
--     -> trusted trace helper (NEW)
--     -> approve_invoice
--     -> post_payment (cash/card/transfer) OR approved debt
--     -> _finance_billing_link_upsert (source link; corrects=NULL)
--     -> outer idempotency complete
--
-- Non-goals: no _invoice_items_validate_source change (Migration B),
--   no frontend cutover, no POS objects, no tax/permission/historical row edits.
-- ============================================================================
...
DECLARE
  ...
  v_live_raw_fp    constant text := 'b6c7f67991e12f2ad667967f4bf118d1f15ba8246c72028cf3a4bb0e58ecb803';
  v_live_canon_fp  constant text := 'fe638fed78baf0d63dfb24d2c6319662bb8a7f834dc5db4eef37b4b42078064a';
  v_def_raw        text;
  v_def_canon      text;
  v_raw_fp         text;
  v_canon_fp       text;
...
BEGIN
  ...
  FOR v_conflict IN SELECT unnest(v_forbidden_fns) LOOP
    IF to_regprocedure(v_conflict) IS NOT NULL THEN
      RAISE EXCEPTION
        'J5_2_PREFLIGHT_TRACE_HELPER_ALREADY_EXISTS: %', v_conflict
        USING ERRCODE = '42710';
    END IF;
  END LOOP;
  ...
  SELECT is_nullable, column_default INTO v_pit_col
   ...
   WHERE table_schema='public' AND table_name='invoices' AND column_name='prices_include_tax';
  IF v_pit_col IS NULL THEN
    RAISE EXCEPTION 'J5_1_PREFLIGHT_INVOICES_PRICES_INCLUDE_TAX_MISSING' USING ERRCODE = '42703';
  END IF;
  IF v_pit_col.is_nullable = 'YES' THEN
    RAISE EXCEPTION 'J5_1_PREFLIGHT_INVOICES_PRICES_INCLUDE_TAX_NULLABLE' USING ERRCODE = '23502';
  END IF;
  IF v_pit_col.column_default IS NOT NULL THEN
    RAISE EXCEPTION 'J5_1_PREFLIGHT_INVOICES_PRICES_INCLUDE_TAX_HAS_DEFAULT' USING ERRCODE = '42P16';
  END IF;
  ...
  IF v_frozen_count <> array_length(v_required_frozen_cols, 1) THEN
    RAISE EXCEPTION 'J5_1_PREFLIGHT_FROZEN_COLUMNS_MISSING: expected %, found %',
      array_length(v_required_frozen_cols, 1), v_frozen_count USING ERRCODE = '42703';
  END IF;
  IF v_frozen_bad_null > 0 THEN
    RAISE EXCEPTION 'J5_1_PREFLIGHT_FROZEN_COLUMNS_NULLABLE' USING ERRCODE = '23502';
  END IF;
  IF v_frozen_bad_default > 0 THEN
    RAISE EXCEPTION 'J5_1_PREFLIGHT_FROZEN_COLUMNS_WITH_DEFAULT' USING ERRCODE = '42P16';
  END IF;
  ...
  FOR v_missing IN SELECT unnest(v_required_checks) LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'J5_1_PREFLIGHT_MISSING_OR_INVALID_CHECK: %', v_missing USING ERRCODE = '23514';
    END IF;
  END LOOP;
    ...
    IF (
      SELECT count(*) FROM pg_trigger tg
       ...
       WHERE tg.tgrelid = 'public.invoice_items'::regclass
         ...
         AND n.nspname  = 'public' AND p.proname = v_trg.fn_name
    ) <> 1 THEN
      RAISE EXCEPTION 'J5_1_PREFLIGHT_TRIGGER_BINDING_INVALID: % -> public.%()',
        v_trg.tg_name, v_trg.fn_name USING ERRCODE = '42704';
    END IF;
  END LOOP;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'J5_1_PREFLIGHT_PAYMENT_ACCOUNT_UNIQUE_MISSING' USING ERRCODE = '23505';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
     WHERE conrelid='public.payment_accounts'::regclass
    ...
    RAISE EXCEPTION 'J5_1_PREFLIGHT_PAYMENT_ACCOUNT_OWNER_CHECK_MISSING' USING ERRCODE = '23514';
  END IF;
  ...
  FOR v_missing IN SELECT unnest(v_required_table_cols) LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'J5_1_PREFLIGHT_TABLE_COLUMN_MISSING: %', v_missing USING ERRCODE = '42703';
    END IF;
  END LOOP;
END
$preflight$;
...
-- ============================================================================
-- Private trusted trace helper (NEW)
-- ============================================================================
...
COMMENT ON FUNCTION public._finance_source_checkout_apply_trace(uuid,uuid,text,uuid)
  IS 'J5.2 Migration A: Server-owned trusted trace writer. Private; called only from public.create_source_checkout_invoice to stamp entity_type/entity_id on draft-invoice items. Revoked from anon and authenticated.';

-- ============================================================================
-- Public non-POS source checkout RPC (REPLACE)
-- ============================================================================
...
COMMENT ON FUNCTION public.create_source_checkout_invoice(uuid,uuid,jsonb)
  IS 'J5.2 Migration A: Atomic non-POS source checkout (lab_sample, horse_order). Composes create_invoice_with_items, _finance_source_checkout_apply_trace, approve_invoice, post_payment (single-payment) or approved debt, _finance_billing_link_upsert. Deterministic md5-derived child idempotency keys; four transaction-local failure-injection GUCs (fin.fail_after_trace/approve/payment/source_link) inert unless set.';
