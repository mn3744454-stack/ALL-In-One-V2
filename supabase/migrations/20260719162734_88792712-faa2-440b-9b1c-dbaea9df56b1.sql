-- Label 2 — Non-destructive package lineage on invoice_items
-- Adds package_id + snapshots and extends existing validation trigger.

ALTER TABLE public.invoice_items
  ADD COLUMN IF NOT EXISTS package_id uuid NULL,
  ADD COLUMN IF NOT EXISTS package_source text NULL,
  ADD COLUMN IF NOT EXISTS package_name_snapshot text NULL,
  ADD COLUMN IF NOT EXISTS package_name_ar_snapshot text NULL,
  ADD COLUMN IF NOT EXISTS package_price_snapshot numeric NULL,
  ADD COLUMN IF NOT EXISTS package_currency_snapshot text NULL,
  ADD COLUMN IF NOT EXISTS package_services_snapshot jsonb NULL;

CREATE INDEX IF NOT EXISTS idx_invoice_items_package_id
  ON public.invoice_items(package_id) WHERE package_id IS NOT NULL;

-- Mutual exclusion: a single line cannot be both a service line and a package line.
ALTER TABLE public.invoice_items
  DROP CONSTRAINT IF EXISTS invoice_items_service_package_exclusive_chk;
ALTER TABLE public.invoice_items
  ADD CONSTRAINT invoice_items_service_package_exclusive_chk
  CHECK (NOT (service_id IS NOT NULL AND package_id IS NOT NULL));

-- Extend validation trigger without disturbing existing service/horse guards.
