-- =============================================================================
-- Phase N+1B · J3.1 — Narrow backend contract corrections
-- 1) tax_rate_snapshot preserves effective tenant rate (even on exempt lines)
-- 2) Discount ceiling uses SUM(line_gross_amount), not commercial total
-- 3) update_invoice_with_items preserves persisted prices_include_tax when
--    the payload omits the key (create + approve semantics unchanged)
-- No data mutation. No schema mutation. No J4/J5. No frontend/reader changes.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    v_items := v_items || jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
      ...
      -- J3.1: snapshot the effective tenant rate regardless of taxability.
      'tax_rate_snapshot', v_rate
    )));
  END LOOP;
  ...
  -- J3.1: canonical ceiling is SUM(line_gross_amount).
...
-- =============================================================================
-- update_invoice_with_items — preserve persisted prices_include_tax when
-- the payload omits (or nulls) the key. Explicit boolean payload overrides.
-- Every other behavior is preserved verbatim.
-- =============================================================================
CREATE OR REPLACE FUNCTION public.update_invoice_with_items(p_tenant_id uuid, p_idempotency_key uuid, p_invoice_id uuid, p_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  -- J3.1: preserve the invoice-specific tax mode when caller omits the key.
  -- An explicit boolean in the payload continues to override.
  IF p_payload ? 'prices_include_tax'
     AND jsonb_typeof(p_payload->'prices_include_tax') = 'boolean' THEN
  ...
  ELSE
    v_effective_payload := p_payload
      || jsonb_build_object(
           'prices_include_tax',
           COALESCE(v_inv.prices_include_tax, false)
         );
  END IF;

  v_computed           := public._finance_invoice_compute_totals(p_tenant_id, v_effective_payload);
