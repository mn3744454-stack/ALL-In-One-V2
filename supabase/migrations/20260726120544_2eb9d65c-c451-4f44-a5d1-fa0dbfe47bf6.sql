-- Phase N+2 Slice 3 — Supported Lab Payment Fix
-- Root cause: post_payment_session called non-existent `_finance_idempotency_finalize`
-- at the end, which raised 42883 and rolled back the whole transaction.
-- Narrow fix: call the real `_finance_idempotency_complete(tenant, op, key, actor,
-- request_hash, resolved_snapshot, response)` using the request_hash returned by
-- `_finance_idempotency_begin`. No contract change to callers or response shape.
...
BEGIN
  ...
  -- NARROW FIX: call the real completion helper. Previous call to
  -- `_finance_idempotency_finalize` referenced a non-existent function,
  -- which raised 42883 and rolled the whole transaction back.
  PERFORM public._finance_idempotency_complete(
    p_tenant_id, v_op, p_idempotency_key, v_actor,
    v_hash, '{}'::jsonb, v_response);
