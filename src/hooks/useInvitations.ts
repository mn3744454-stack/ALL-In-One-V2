...
export const useInvitations = () => {
  ...
  const createInvitation = async (data: CreateInvitationData) => {
    ...
    // Support email or phone (at least one required by DB constraint)
    if (validatedData.invitee_email) {
      insertPayload.invitee_email = validatedData.invitee_email;
    }
    if ((data as any).invitee_phone) {
      insertPayload.invitee_phone = (data as any).invitee_phone;
    }
