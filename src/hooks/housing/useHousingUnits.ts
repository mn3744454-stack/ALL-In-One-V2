...
export function useHousingUnits(branchId?: string, areaId?: string) {
  ...
  const { data: units = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from('housing_units')
        .select(`*, area:facility_areas(id, name), branch:branches(id, name)`)
      ...
      if (unitIds.length > 0) {
        ...
        occupantCounts?.forEach(o => { countMap[o.unit_id] = (countMap[o.unit_id] || 0) + 1; });
        return data.map(unit => ({ ...unit, current_occupants: countMap[unit.id] || 0 })) as HousingUnit[];
      }
      return data.map(unit => ({ ...unit, current_occupants: 0 })) as HousingUnit[];
    },
    enabled: !!tenantId,
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: CreateUnitData) => {
      ...
      const { data: newUnit, error } = await supabase
        .from('housing_units')
        .insert({
          tenant_id: tenantId, branch_id: data.branch_id, area_id: data.area_id,
          code: data.code, name: data.name || data.code, name_ar: data.name_ar || null,
          unit_type: data.unit_type, occupancy: data.occupancy, capacity,
          status: 'available', is_active: true, is_archived: false, is_demo: false,
    ...
    onSuccess: () => { toast.success(tGlobal('housing.units.created')); invalidateAll(); },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, ...data }: { id: string } & Partial<CreateUnitData>) => {
      ...
      const { data: updated, error } = await supabase
        .from('housing_units').update(updateData).eq('id', id).eq('tenant_id', tenantId).select().single();
      if (error) throw error;
      return updated;
    },
    onSuccess: () => { toast.success(tGlobal('housing.units.updated')); invalidateAll(); },
    onError: (error) => { toast.error(error.message); },
  });
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }) => {
      if (!tenantId) throw new Error(tGlobal('housing.toasts.noActiveOrganization'));
      const { error } = await supabase.from('housing_units').update({ is_active: isActive }).eq('id', id).eq('tenant_id', tenantId);
      if (error) throw error;
    },
    onSuccess: () => { toast.success(tGlobal('housing.units.updated')); invalidateAll(); },
    onError: (error) => { toast.error(error.message); },
  ...
  const setStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      if (!tenantId) throw new Error(tGlobal('housing.toasts.noActiveOrganization'));
      const { error } = await supabase.from('housing_units').update({ status, updated_at: new Date().toISOString() }).eq('id', id).eq('tenant_id', tenantId);
      if (error) throw error;
    },
    onSuccess: () => { toast.success(tGlobal('housing.units.updated')); invalidateAll(); },
    onError: (error) => { toast.error(error.message); },
  ...
  const archiveMutation = useMutation({
    mutationFn: async (id: string) => {
      if (!tenantId) throw new Error(tGlobal('housing.toasts.noActiveOrganization'));
      const { error } = await supabase.from('housing_units')
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      if (!tenantId) throw new Error(tGlobal('housing.toasts.noActiveOrganization'));
      const { error } = await supabase.from('housing_units').delete().eq('id', id).eq('tenant_id', tenantId);
      if (error) throw error;
    },
  ...
  return {
    units, activeUnits, isLoading, error, canManage,
    ...
    archiveUnit: archiveMutation.mutateAsync,
    restoreUnit: restoreMutation.mutateAsync,
