  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Map<string, EnrichedStatementData>> => {
      ...
      for (const entry of entries) {
        ...
        for (const item of items) {
          if (item.entity_type === "lab_sample" && item.entity_id) {
            ...
            if (resolved && resolved.horseId) {
              ...
              // Add sample if not already added
        ...
        // If there are items without horse linkage, add them to a generic group
        if (noHorseItems.length > 0 && horses.length === 0) {
          // No horses at all - just items
        }
