...
import { Package, CheckCircle2, XCircle, Clock, Building2 } from "lucide-react";
...
function IncomingCard({
  ...
  return (
    <Card className={cn(
      "transition-all",
      incoming.status === 'pending' && "border-amber-200 dark:border-amber-800"
    )}>
        ...
        <div className="flex items-start gap-3">
          <Avatar className="h-10 w-10 shrink-0">
            ...
            <AvatarFallback className="bg-primary/10 text-primary">
              {incoming.horse_name?.[0]?.toUpperCase() || 'H'}
            </AvatarFallback>
          </Avatar>
          ...
          <div className="flex-1 min-w-0">
            ...
            {incoming.horse_name_ar && (
              <p className="text-xs text-muted-foreground" dir="rtl">{incoming.horse_name_ar}</p>
            )}
            ...
            {incoming.scheduled_at && (
              <p className="text-xs text-muted-foreground mt-0.5">
                {t('movement.incoming.scheduledAt')}: {format(new Date(incoming.scheduled_at), 'PPp')}
              </p>
            )}

            {incoming.reason && (
              <p className="text-xs text-muted-foreground mt-0.5 truncate">
                {t('movement.detail.reason')}: {incoming.reason}
              </p>
            )}
          </div>
        </div>
        ...
        {incoming.status === 'pending' && canConfirm && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t">
            <Button size="sm" variant="default" className="gap-1 flex-1" onClick={onConfirm}>
              ...
              {t('movement.incoming.confirmArrival')}
            </Button>
