...
export function CreateBreedingAttemptDialog({
  ...
  // Quick-create dialog states — key counter guarantees unique remount every open
  const [quickCreateOpen, setQuickCreateOpen] = useState(false);
  const [quickCreateDefaults, setQuickCreateDefaults] = useState<QuickCreateHorseDefaults | undefined>();
  const [quickCreateTarget, setQuickCreateTarget] = useState<"mare" | "stallion" | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!mareId || !attemptDate || !attemptType || attemptType === "__neutral__") return;
  ...
  const canSubmit = !!mareId && !!attemptDate && !!attemptType && attemptType !== "__neutral__";
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="w-[95vw] max-w-4xl max-h-[85vh] overflow-hidden flex flex-col p-0">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-6 py-4">
            <form id="breeding-attempt-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Right Column */}
                <div className="space-y-4">
                  {/* Method field — neutral in-list option */}
                  <div className="space-y-2">
                    ...
                    <Select value={attemptType} onValueChange={setAttemptType}>
                      ...
                      <SelectContent className="z-[200]">
                        <SelectItem value="__neutral__" className="text-muted-foreground">{t("breeding.selectMethod")}</SelectItem>
          ...
          <div className="shrink-0 px-6 py-4 border-t flex gap-3">
            ...
            <Button type="submit" form="breeding-attempt-form" className="flex-1" disabled={loading || !canSubmit}>
