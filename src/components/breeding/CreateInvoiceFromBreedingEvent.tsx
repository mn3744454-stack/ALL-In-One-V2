...
export function CreateInvoiceFromBreedingEvent({ open, onOpenChange, event }: Props) {
  ...
  const { isIncluded, planName } = usePlanInclusionCheck(event.horseId, selectedServiceId || undefined);
  ...
  useEffect(() => {
    ...
    if (event.contractId) {
      if (event.contractServiceId) setSelectedServiceId(event.contractServiceId);
      if (event.contractUnitPrice != null) setTotalAmount(event.contractUnitPrice.toString());
      if (event.contractClientId) setSelectedClientId(event.contractClientId);
      if (event.contractClientName) setClientName(event.contractClientName);
  ...
  const selectedService = useMemo(() => breedingServices.find(s => s.id === selectedServiceId), [breedingServices, selectedServiceId]);
  ...
  const buildLineItemDescription = (): string => {
    ...
    else parts.push(sourceTypeLabel);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const selectedService = breedingServices.find(s => s.id === selectedServiceId);
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="text-xs text-muted-foreground bg-muted rounded-md p-2">
            ...
            {event.contractNumber && <span className="ml-2 font-mono text-primary">({t("breeding.contracts.contract")} {event.contractNumber})</span>}
          </div>
          ...
          {relevantServices.length > 0 && (
            <div>
              ...
              <Select value={selectedServiceId} onValueChange={handleServiceChange}>
                <SelectTrigger className="w-full"><SelectValue placeholder={t("breeding.billing.selectService")} /></SelectTrigger>
                <SelectContent>
                  {relevantServices.map(svc => (
                    <SelectItem key={svc.id} value={svc.id}>
                      <span className="flex items-center justify-between gap-2 w-full">
                        <span>{displayServiceName(svc.name, svc.name_ar, lang)}</span>
                        {svc.unit_price != null && <span className="text-xs text-muted-foreground">{svc.unit_price} {event.currency || "SAR"}</span>}
                      </span>
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <div>
            ...
            {isZeroCharge && (
              <p className="text-xs text-muted-foreground mt-1">{isIncluded ? t("vet.billing.zeroChargeIncluded") : t("vet.billing.zeroChargeIncluded")}</p>
            )}
          </div>
          <div>
