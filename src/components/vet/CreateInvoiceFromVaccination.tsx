...
export function CreateInvoiceFromVaccination({ open, onOpenChange, data }: Props) {
  ...
  const { isIncluded, planName } = usePlanInclusionCheck(vaccination.horse_id, selectedServiceId || undefined);
  const { payable: linkedPayable } = useSupplierPayableForSource("vaccination", vaccination.id);
  ...
  const buildLineItemDescription = (): string => {
    ...
    if (vaccination.program) parts.push(displayServiceName(vaccination.program.name, vaccination.program.name_ar, lang));
    const horseName = displayHorseName(data.horseName || vaccination.horse?.name, data.horseNameAr || (vaccination.horse as any)?.name_ar, lang);
    if (horseName && horseName !== "—") parts.push(horseName);
    if (vaccination.administered_date || vaccination.due_date) parts.push(formatStandardDate(vaccination.administered_date || vaccination.due_date));
    return parts.join(" | ");
  };
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="text-xs text-muted-foreground bg-muted rounded-md p-2">
            {vaccination.program && <span className="font-medium">{displayServiceName(vaccination.program.name, vaccination.program.name_ar, lang)}</span>}
            {vaccination.horse?.name && ` — ${displayHorseName(vaccination.horse.name, (vaccination.horse as any)?.name_ar, lang)}`}
            {vaccination.administered_date && <span className="ms-2 text-primary">({formatStandardDate(vaccination.administered_date)})</span>}
          </div>
