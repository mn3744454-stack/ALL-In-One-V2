// B2.5e Phase 2c — Reusable Contract Document editor body.
// Mounted by both the SafeFormDialog (list → modal) and the direct route
// fallback. Owns no chrome/sizing; renders <EditorShell> which fills the
// parent. All existing handlers, mutations, validation, and frozen-viewer
// branches are preserved from the original page.
...
export function ContractDocumentEditorBody({
  ...
  const onSend = () => {
    ...
    saveDraft.mutate(
      { document_json: bodyDoc, variables_json: variables, variable_values: values },
      { onSuccess: () => { setIsDirty(false); send.mutate(undefined); } },
    );
  };

  if (isLoading || !doc) {
    return <div className="p-8 text-sm text-muted-foreground">{t("contracts.editor.loading")}</div>;
  }
  ...
  const headerNode = (
    <div className="min-w-0">
      ...
      <h1 className="font-display text-xl lg:text-2xl font-semibold text-navy truncate">{doc.title}</h1>
      {doc.title_ar && <p className="text-sm text-muted-foreground truncate" dir="rtl">{doc.title_ar}</p>}
    </div>
  );
          ...
          {variables.map((v) => {
            ...
            return (
              <div key={v.key} className="space-y-1">
                ...
                <Input
                  value={values[v.key] ?? ""}
                  onChange={(e) => { setValues((s) => ({ ...s, [v.key]: e.target.value })); markDirty(); }}
