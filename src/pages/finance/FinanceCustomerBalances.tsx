...
import { Search, FileText, CreditCard, AlertTriangle, CheckCircle2, MinusCircle } from "lucide-react";
...
export default function FinanceCustomerBalances() {
  ...
  const clientsWithBalances = useMemo(() => {
    return filteredClients.map((client) => {
      ...
      return { ...client, ledgerBalance, creditLimit, availableCredit };
    });
                      ...
                      <TableRow key={client.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center justify-end gap-2">
                            <Button variant="ghost" size="sm" onClick={() => handleViewStatement(client)}>
                              <FileText className="h-4 w-4 me-1" />
                              <span className="hidden sm:inline">{t("clients.statement.view")}</span>
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => handleRecordPayment(client.id)}>
