...
export default function DashboardDoctorConsultationDetail() {
  ...
  if (loading) {
    return <DashboardShell><div className="flex flex-1 items-center justify-center"><div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div></DashboardShell>;
  }

  if (error || !consultation) {
    return <DashboardShell><div className="flex flex-1 items-center justify-center"><p className="text-muted-foreground">{t('doctor.consultationNotFound')}</p></div></DashboardShell>;
  }
