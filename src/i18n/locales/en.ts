export const en = {
  common: {
    loading: "Loading...",
    error: "Error",
    success: "Success",
    cancel: "Cancel",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    close: "Close",
    confirm: "Confirm",
    understood: "Understood",
    back: "Back",
    next: "Next",
    search: "Search",
    filter: "Filter",
    reset: "Reset",
    submit: "Submit",
    create: "Create",
    update: "Update",
    add: "Add",
    remove: "Remove",
    view: "View",
    download: "Download",
    upload: "Upload",
    share: "Share",
    copy: "Copy",
    refresh: "Refresh",
    settings: "Settings",
    logout: "Logout",
    login: "Login",
    register: "Register",
    yes: "Yes",
    no: "No",
    all: "All",
    none: "None",
    select: "Select",
    selected: "selected",
    optional: "Optional",
    required: "Required",
    or: "Or",
    more: "More",
    less: "Less",
    actions: "Actions",
    status: "Status",
    type: "Type",
    date: {
      label: "Date",
      day: "Day",
      month: "Month",
      year: "Year",
      today: "Today",
      clear: "Clear",
    },
    dateRange: {
      startAfterEnd: "Start date cannot be after end date.",
      endBeforeStart: "The end date cannot be earlier than the start date.",
      dueBeforeIssue: "The due date cannot be earlier than the issue date.",
    },
    name: "Name",
    description: "Description",
    notes: "Notes",
    total: "Total",
    details: "Details",
    noResults: "No results found",
    sortAsc: "Ascending",
    sortDesc: "Descending",
    comingSoon: "Coming Soon",
    clearFilters: "Clear Filters",
    print: "Print",
    export: "Export",
    from: "From",
    to: "To",
    unknown: "Unknown",
    active: "Active",
    inactive: "Inactive",
    saving: "Saving...",
    creating: "Creating...",
    goToDashboard: "Go to Dashboard",
    backToDashboard: "Back to Dashboard",
    accessRestricted: "Access Restricted",
    table: "Table",
    list: "List",
    grid: "Grid",
    columns2: "2 Columns",
    columns3: "3 Columns",
    columns4: "4 Columns",
    of: "of",
    tryAdjustingFilters: "Try adjusting your filters",
    viewAll: "View All",
    dialog: {
      discardChanges: {
        title: "Discard changes?",
        description: "You have unsaved changes. Closing now will discard them.",
        confirm: "Discard & close",
        keepEditing: "Keep editing",
      },
    },
    validation: {
      missing: "Required to continue:",
      attemptedSubmit: "Some required fields are missing.",
      enterHorseStepInfo: "Enter the required horse information for this step",
      enterHorseName: "Enter the horse name",
      selectGender: "Select a gender",
      enterClientName: "Enter the client name",
      enterEmployeeName: "Enter the employee name",
      selectRecipient: "Select a recipient",
      validTransferPercentage: "Enter a valid transfer percentage",
      enterRequiredName: "Enter the required name",
      enterRequiredArabicName: "Enter the Arabic name",
      enterRequiredEnglishName: "Enter the English name",
      selectOwner: "Select an owner",
      validOwnershipPercentage: "Enter a valid ownership percentage",
      selectHorsePatient: "Select a horse",
      selectVisitType: "Select visit type",
      selectScheduledDate: "Select scheduled date",
      selectTreatmentCategory: "Select treatment category",
      selectTreatmentDate: "Select treatment date",
      enterTreatmentDetails: "Enter treatment details",
      enterHorseNameEnOrAr: "Enter an English or Arabic horse name",
      enterRejectionReason: "Enter a rejection reason",
    },
  },
  workspace: {
    personal: "Personal",
    organization: "Organization",
  },
  settings: {
    permissionsRoles: {
      title: "Permissions & Roles",
      desc: "Manage member roles, bundle assignments, and delegation scopes.",
      open: "Open",
      unauthorized: "You are not authorized to access this page",
    },
    notifications: {
      title: "Notification Settings",
      desc: "Manage device alerts and notification preferences.",
    },
    roles: {
      title: "Roles",
    },
    permissions: {
      title: "Permissions",
    },
  },
  sidebar: {
    dashboard: "Dashboard",
    community: "Community",
    myBookings: "My Bookings",
    payments: "Payments",
    myPayments: "My Payments",
    horses: "Horses",
    myHorses: "My Horses",
    orders: "Orders",
    breeding: "Breeding",
    vetHealth: "Vet & Health",
    laboratory: "Laboratory",
    schedule: "Schedule",
    records: "Records",
    team: "Team",
    facilities: "Facilities",
    services: "Services",
    revenue: "Revenue",
    sessions: "Sessions",
    manageBookings: "Manage Bookings",
    publicProfile: "Public Profile",
    settings: "Settings",
    signOut: "Sign Out",
    hr: "Human Resources",
    partnerships: "Partnerships",
    movement: "Movement",
    housing: "Housing",
    boardingContracts: "Contracts",
    contracts: "Contracts",
    notificationSettings: "Notifications",
    favorites: "Favorites",
    myPurchases: "My Purchases",
    doctorOverview: "Overview",
    doctorPatients: "Patients",
    doctorConsultations: "Consultations",
    doctorServices: "Services",
    pos: "Point of Sale",
    collapse: "Collapse sidebar",
    expand: "Expand sidebar",
    myProfile: "My Profile",
  },
  pushNotifications: {
    settingsTitle: "Notification Settings",
    settingsDesc: "Manage push notifications, categories, and quiet hours",
    bannerTitle: "Stay Updated",
    bannerBody: "Enable push notifications to get instant updates on messages, results, and requests.",
    iosGuideTitle: "Install Dayli Horse for Notifications",
    iosGuideBody: "To receive notifications, tap Share → Add to Home Screen, then open the app from your home screen.",
    enable: "Enable Notifications",
    disable: "Disable Notifications",
    enabled: "Push notifications enabled",
    disabled: "Push notifications disabled",
    enableFailed: "Failed to enable notifications. Please try again.",
    notSupported: "Push notifications are not supported on this browser.",
    permissionDenied: "Notification permission was denied. Please enable it in your browser settings.",
    pushStatus: "Push Notifications",
    pushEnabledDesc: "You are receiving push notifications on this device.",
    pushDisabledDesc: "Enable to receive notifications when you're away from the app.",
    categories: "Notification Categories",
    categoriesDesc: "Choose which types of notifications you want to receive.",
    channelLayerNote: "These push categories control how notifications reach your device. Your per-area preferences above decide what kinds of notifications you receive in the first place — push categories sit on top of those choices.",
    catMessages: "Messages",
    catResults: "Lab Results",
    catStatus: "Status Updates",
    catInvitations: "Invitations",
    catPartnerships: "Partnerships & Connections",
    inAppSound: "In-App Sound",
    inAppSoundDesc: "Play a sound when notifications arrive while the app is open.",
    quietHours: "Quiet Hours",
    quietHoursDesc: "Pause push notifications during specific hours.",
    enableQuietHours: "Enable Quiet Hours",
    from: "From",
    to: "To",
    timezone: "Timezone",
    devices: "Registered Devices",
    deviceRemoved: "Device removed",
  },
  hr: {
    title: "Employees",
    name: "Name",
    addEmployee: "Add Employee",
    editEmployee: "Edit Employee",
    employeeDetails: "Employee Details",
    fullName: "Full Name (English)",
    fullNamePlaceholder: "Enter full name",
    fullNameAr: "Full Name (Arabic)",
    fullNameArPlaceholder: "أدخل الاسم الكامل",
    employeeType: "Employee Type",
    department: "Department",
    departmentPlaceholder: "e.g. Stables, Admin",
    phone: "Phone",
    phonePlaceholder: "Enter phone number",
    contact: {
      phones: "Phone Numbers",
      addPhone: "Add Phone",
      noPhonesAdded: "No phone numbers added",
      whatsapp: "WhatsApp",
      primaryPhone: "Primary",
      setAsPrimary: "Set as Primary",
    },
    phoneLabels: {
      mobile: "Mobile",
      work: "Work",
      home: "Home",
      other: "Other",
    },
    email: "Email",
    emailPlaceholder: "Enter email address",
    notes: "Notes",
    notesPlaceholder: "Additional notes...",
    tags: "Tags",
    status: "Status",
    activate: "Activate",
    deactivate: "Deactivate",
    createdAt: "Created",
    searchPlaceholder: "Search by name...",
    noEmployees: "No employees found",
    noCollaborators: "No collaborators found",
    collaborators: "Collaborators",
    addCollaborator: "Add Collaborator",
    accessAndInvitations: "Access & Invitations",
    peopleAndInvitations: "People & Invitations",
    addFirstEmployee: "Add your first team member",
    salaryPayment: "Salary Payment",
    salaryPaymentCreated: "Salary payment recorded successfully",
    payroll: {
      title: "Payroll",
      subtitle: "Manage and track employee salary payments",
      description: "Manage and track employee salary payments",
      addPayment: "Add Payment",
      totalPaid: "Total Paid",
      paymentsCount: "Payments Count",
      averagePerEmployee: "Average per Employee",
      searchEmployee: "Search by name...",
      filterByEmployee: "Filter by employee",
      filterByPeriod: "Filter by period",
      recentPayments: "Recent Payments",
      noPayments: "No salary payments found",
      linkedToExpense: "Linked to expense",
      selectEmployee: "Select employee",
      period: "Period",
      periodPlaceholder: "e.g. January 2026",
      createExpense: "Create linked expense",
      createExpenseDesc: "Link this payment to Finance module",
      currency: "Currency",
      notes: "Notes",
      notesPlaceholder: "Additional notes...",
      employeeLabel: "Employee",
      noEmployeesYet: "No employees yet",
      noEmployeesDesc: "Add your first team member to record salary payments.",
      addNewEmployee: "+ Add New Employee",
    },
    quickCreate: {
      title: "Quick Add Employee",
      desc: "Create an initial employee record so you can continue payroll. You can complete the rest of the details later from Team Directory.",
      completeHint: "This creates an initial record. Department, email, start date, multi-phone, and other details can be completed later from Team Directory.",
      salary: "Salary (optional)",
    },
    employeeTypes: {
      trainer: "Trainer",
      groom: "Groom",
      vet_tech: "Vet Technician",
      receptionist: "Receptionist",
      lab_tech: "Lab Technician",
      admin: "Admin",
      manager: "Manager",
      driver: "Driver",
      farrier: "Farrier",
      other: "Other",
    },
    customType: "Custom Type",
    customTypePlaceholder: "Specify type",
    confirmDeactivate: "Are you sure you want to deactivate this employee?",
    confirmActivate: "Are you sure you want to activate this employee?",
    employeeCreated: "Employee added successfully",
    employeeUpdated: "Employee updated successfully",
    employeeActivated: "Employee activated",
    employeeDeactivated: "Employee deactivated",
    filterByType: "Filter by type",
    filterByStatus: "Filter by status",
    allTypes: "All Types",
    allStatuses: "All Statuses",
    // Phase D — Responsibilities column
    responsibilities: "Responsibilities",
    responsibilitiesShort: "Responsibilities",
    responsibilitiesNone: "No responsibilities recorded yet",
    responsibilitiesPopover: {
      title: "Responsibilities",
    },
    
    // Categories
    category: "Category",
    categories: {
      field: "Field Staff",
      office: "Office Staff",
      mixed: "Mixed",
    },
    jobTitle: "Job Title",
    jobTitlePlaceholder: "e.g. Senior Trainer",
    
    // Settings
    settings: {
      title: "HR Settings",
      modules: "Enabled Modules",
      assignments: "Staff Assignments",
      assignmentsDesc: "Assign staff members to horses",
      attendance: "Attendance Tracking",
      attendanceDesc: "Track daily check-ins and work hours",
      documents: "Documents",
      documentsDesc: "Store employee documents and certifications",
      comingSoon: "Coming Soon",
      updated: "Settings updated",
    },
    
    // Assignments
    assignments: {
      title: "Assigned Staff",
      addAssignment: "Assign Staff",
      assignTo: "Assign to {{name}}",
      selectEmployee: "Select Employee",
      searchEmployees: "Search employees...",
      role: "Role",
      selectRole: "Select role",
      notes: "Notes",
      notesPlaceholder: "Optional notes about this assignment...",
      startDate: "Start Date",
      endDate: "End Date",
      noAssignments: "No staff assigned",
      assignFirst: "Assign a staff member to this horse",
      assignmentCreated: "Staff assigned successfully",
      assignmentRemoved: "Assignment removed",
      confirmRemove: "Remove this assignment?",
      roles: {
        primary_groom: "Primary Groom",
        secondary_groom: "Secondary Groom",
        trainer: "Trainer",
        vet_contact: "Vet Contact",
        farrier: "Farrier",
        exercise_rider: "Exercise Rider",
        other: "Other",
      },
      assignedHorses: "Assigned Horses",
      noHorses: "No horses assigned",
      noEmployeesYet: "No employees yet",
      addNewEmployee: "Add new employee",
      assignHorse: "Assign Horse",
      assignHorseTo: "Assign horse to {{name}}",
      selectHorse: "Select Horse",
      searchHorses: "Search horses...",
      noHorsesAvailable: "No horses available to assign",
      assignFirstHorse: "Assign a horse to this employee",
      addStaff: "Add staff member",
      removeRow: "Remove",
      assignedTeamHelp: "Operational staff responsible for this horse during the stay. Distinct from emergency contact.",
    },
    
    // Demo
    demo: {
      title: "Demo Mode",
      description: "Load sample employees and assignments for testing. Demo data is tagged and can be safely removed.",
      load: "Load Demo Data",
      remove: "Remove Demo Data",
      loaded: "Demo data loaded successfully",
      removed: "Demo data removed",
      alreadyLoaded: "Demo data already exists",
      active: "Demo Active",
      needsHorses: "Please create at least 2 horses first, then try again.",
      confirmRemove: "Remove all demo employees and their assignments? This won't affect real data.",
    },
    
    // Attendance placeholder
    attendance: {
      title: "Attendance",
      comingSoon: "Attendance tracking coming soon",
      comingSoonDesc: "Track daily check-ins, work hours, and attendance reports.",
    },
    
    // Toasts
    toasts: {
      noActiveOrganization: "No active organization",
    },
    
    // New MVP fields
    internal: "Internal",
    external: "External",
    total: "Total",
    employmentKind: "Employment Type",
    internalDesc: "Permanent staff with salary and benefits",
    externalDesc: "Contractors, freelancers, or service providers",
    basicInfo: "Basic Information",
    contactInfo: "Contact Information",
    employmentDetails: "Employment Details",
    additionalInfo: "Additional Information",
    startDate: "Start Date",
    salary: "Salary",
    convertToInternal: "Convert to Internal",
    convertToExternal: "Convert to External",
    convertedToInternal: "Converted to internal employee",
    convertedToExternal: "Converted to external employee",
    
    // Employment Timeline
    timeline: {
      title: "Employment History",
      noEvents: "No employment history",
      created: "Created",
      activated: "Activated",
      deactivated: "Deactivated",
      employmentKindChanged: "Employment type changed",
      salaryUpdated: "Salary updated",
      startDateUpdated: "Start date updated",
    },
    
    // Salary Payments
    salaryPayments: "Salary Payments",
    addPayment: "Add Payment",
    noPayments: "No payments recorded",
    amount: "Amount",
    paidAt: "Payment Date",
    paymentPeriod: "Period",
    paymentPeriodPlaceholder: "e.g. January 2026",
    createExpenseRecord: "Create Expense Record",
    createExpenseDesc: "Also record this as a finance expense",
    linkedToExpense: "Linked to expense",
  },
  language: {
    title: "Language",
    select: "Select language",
  },
  laboratory: {
    // Page & Navigation
    title: "Laboratory Management",
    subtitle: "Manage lab samples, results, and templates",
    
    // Primary nav labels (for Lab tenant sidebar/bottom nav)
    nav: {
      samples: "Samples",
      results: "Results",
      requests: "Requests",
      compare: "Compare",
      timeline: "Timeline",
      templates: "Templates",
      settings: "Settings",
      clients: "Clients",
    },
    
    // Tabs
    tabs: {
      samples: "Samples",
      results: "Results",
      catalog: "Catalog",
      compare: "Compare",
      timeline: "Timeline",
      templates: "Templates",
      settings: "Settings",
      requests: "Requests",
    },
    
    // Lab Modes
    modes: {
      none: "No Laboratory",
      requests: "Lab Requests Only",
      full: "Full Laboratory",
    },
    
    // Lab Requests
    requests: {
      bridge: {
        banner: "Created from approved service request",
        ownerNotes: "Owner notes:",
        approvedCost: "Approved service-request cost: {amount} {currency}",
        contextOnly: "context only, not a lab invoice.",
        ownerVisibility: "The owner will not automatically see lab results until they are shared/published later.",
        ownerLabel: "Owner",
        confirmAllowed: "Confirm this lab request is allowed by the boarding contract, owner approval, included service, or emergency policy.",
        horseNotEligible: "Horse is not eligible for lab request creation in this stable.",
      },
      ownership: {
        hosted: "Hosted",
        connected: "Connected",
      },
      title: "Lab Test Requests",
      subtitle: "Request and track lab tests from external laboratories",
      create: "New Request",
      createTitle: "Create Lab Test Request",
      testDescription: "Test Description",
      testDescriptionPlaceholder: "Describe the tests needed...",
      labSource: "Laboratory",
      platformLab: "Partner Lab",
      externalLab: "Non-Connected Lab",
      noHorsesYet: "No horses registered yet",
      addNewHorse: "Add New Horse",
      noHorsesHint: "Register at least one horse to create a lab request",
      selectedHorses: "horses selected",
      batchCreated: "{count} lab request(s) created",
      batchPartialFail: "{success} of {total} created. {failed} failed.",
      externalLabName: "Lab Name",
      externalLabPlaceholder: "Lab name...",
      selectLab: "Select laboratory",
      noLabPartners: "No connected laboratories. Connect to a lab to get started.",
      connectToLab: "Connect to a Laboratory",
      addLabPartner: "Add Laboratory",
      servicesSelected: "service(s) selected",
      pendingApproval: "Partnership request pending approval",
      connectAnother: "Connect another",
      autoFilledFromServices: "Auto-filled from selected services if left empty",
      testAssignment: "Test Assignment",
      sameTests: "Same tests for all",
      differentTests: "Different per horse",
      pendingPartnership: "Partnership request sent",
      partnershipInbox: "Lab Partnerships",
      connectedLabs: "Connected Labs",
      pendingPartnerships: "Pending",
      priority: "Priority",
      notesPlaceholder: "Additional notes...",
      searchPlaceholder: "Search requests...",
      noRequests: "No lab requests yet",
      noRequestsDesc: "Create your first lab test request",
      viewResult: "View Result",
      legacyReferenceLabel: "Legacy reference link",
      legacyReferenceHint: "Deprecated — new results are published via the structured workflow above.",
      markReceived: "Mark as Received",
      created: "Lab request created",
      updated: "Lab request updated",
      deleted: "Lab request deleted",
      createFailed: "Failed to create lab request",
      updateFailed: "Failed to update lab request",
      deleteFailed: "Failed to delete lab request",
      order: "order",
      orders: "orders",
      status: {
        pending: "Pending",
        sent: "Sent",
        processing: "Processing",
        ready: "Ready",
        received: "Received",
        cancelled: "Cancelled",
      },
      // Messaging & thread
      messages: "Messages",
      selectedServices: "Selected Services",
      openThread: "Open thread",
      noMessages: "No messages yet. Start the conversation.",
      typeMessage: "Type a message...",
      team: "Team",
      partner: "Partner",
      connected: "connected",
      pending: "pending",
      sent: "Sent",
      receivedLabel: "Received",
      // Lab Full Mode actions
      incomingTitle: "Incoming Requests",
      incomingSubtitle: "Manage lab test requests from partner stables",
      updateStatus: "Update Status",
      publishResult: "Publish Result",
      resultUrl: "Result URL",
      resultUrlPlaceholder: "https://...",
      initiatorStable: "Requesting Stable",
      unknownService: "Unknown Service",
      createSampleFromRequest: "Create Sample from Request",
      restartRetest: "Restart / Retest",
      sampleCreated: "Sample created",
      // Priority labels (dedicated lab namespace)
      priorities: {
        low: "Low",
        normal: "Normal",
        high: "High",
        urgent: "Urgent",
      },
    },

    // Submissions (parent container)
    submissions: {
      unknownSender: "Unknown Sender",
      horse: "horse",
      horses: "horses",
      service: "service",
      services: "services",
      legacyRequest: "Legacy request",
      noSubmissions: "No incoming submissions",
      noSubmissionsDesc: "Submissions from partner stables will appear here",
      status: {
        pending: "Pending",
        processing: "In Progress",
        partial: "Partial",
        ready: "Ready",
        received: "Received",
        cancelled: "Cancelled",
        rejected: "Rejected",
        mixed: "Mixed",
      },
    },

    // Phase 6A — Submission sampling progress (read-layer)
    samplingProgress: {
      submissionRef: "Sub",
      sampled: "Sampled",
      awaitingSample: "Awaiting sample",
      notSampled: "Awaiting sampling",
      partiallySampled: "Partially sampled",
      fullySampled: "Fully sampled",
    },

    // Phase 6B — Submission-grouped Samples view
    submissionGrouped: {
      viewBySubmissions: "By submission",
      pendingOnly: "Show only submissions with pending sampling",
      pendingOnlyShort: "Pending only",
      empty: "No submissions match the current filters.",
      noSubmission: "Standalone samples",
      awaitingSampleCreation: "Accepted, awaiting sample creation",
      batchHint: "Batch creation will be available in the next phase.",
    },

    // Phase 6C — Inline + Batch sample creation
    batchCreate: {
      title: "Batch create samples",
      headerAction: "Batch create ({count})",
      inlineCreate: "Create sample",
      eligibleSummary: "{count} eligible (accepted + specimen received)",
      eligibleHint: "{count} ready for batch creation",
      noneEligible: "No horses are ready for sampling yet.",
      collectionDate: "Collection date",
      sharedNotes: "Shared notes (optional)",
      sharedNotesPlaceholder: "Applied to all selected samples",
      includedHorses: "Included horses ({selected}/{total})",
      sampleIdPlaceholder: "Sample ID",
      reuseNote: "Each sample uses the same creation path as the single-horse wizard.",
      submit: "Create {count} samples",
      successCount: "{count} samples created",
      failureCount: "{count} failed",
      errorNoSelection: "Select at least one horse to create samples for.",
    },

    // Phase 5 — Intake decision workflow
    intake: {
      title: "Intake",
      stepDecision: "Review & decide",
      stepSpecimen: "Specimen receipt",
      accept: "Accept",
      reject: "Reject",
      acceptAll: "Accept all",
      rejectAll: "Reject all",
      rejectAllTitle: "Reject all pending horses",
      rejectAllDescription: "This rejection reason will be applied to all pending horses in this submission.",
      pendingChildren: "pending review",
      rejectTitle: "Reject request",
      rejectDescription: "Please provide a reason. This will be visible to the requesting stable.",
      rejectionReason: "Rejection reason",
      rejectionReasonPlaceholder: "e.g. wrong specimen tube, hemolyzed sample...",
      confirmReject: "Confirm rejection",
      markSpecimenReceived: "Mark received",
      specimenReceived: "Specimen received",
      awaitingSpecimen: "Awaiting specimen",
      readyForSampling: "Ready for sample creation.",
      waitingForSpecimen: "Specimen must be marked received before sample creation.",
      decision: {
        pending_review: "Pending review",
        accepted: "Accepted",
        rejected: "Rejected",
        partial: "Partially accepted",
      },
      stableView: {
        accepted: "Lab accepted your request — awaiting specimen",
        inProgress: "Lab is processing your request",
        rejected: "Lab rejected this request",
      },
      toast: {
        accepted: "Request accepted",
        rejected: "Request rejected",
        acceptedAll: "All pending horses accepted",
        rejectedAll: "All pending horses rejected",
        specimenReceived: "Specimen marked received",
        acceptFailed: "Failed to accept request",
        rejectFailed: "Failed to reject request",
        specimenFailed: "Failed to mark specimen received",
        serviceAccepted: "Service accepted",
        serviceRejected: "Service rejected",
        serviceAcceptFailed: "Failed to accept service",
        serviceRejectFailed: "Failed to reject service",
        templateAccepted: "Test accepted",
        templateRejected: "Test rejected",
        templateAcceptFailed: "Failed to accept test",
        templateRejectFailed: "Failed to reject test",
      },
      // Phase 5.2 — service-level decision UI
      decidePerService: "Decide per service",
      serviceSummary: "{accepted} of {total} services accepted",
      rejectService: "Reject service",
      rejectServiceDescription: "Provide a precise reason for rejecting this service. It will be visible to the requesting stable.",
      partiallyAccepted: "Partially accepted",
      serviceBreakdown: "Service breakdown",
      serviceStatusAccepted: "Accepted",
      serviceStatusRejected: "Rejected",
      serviceStatusPending: "Pending",
      serviceStatusPartial: "Partially accepted",
      serviceRejectionReasonLabel: "Reason",
      // Phase 5.2.1 — macro discipline copy
      acceptAllServices: "Accept all",
      rejectAllServices: "Reject all",
      macroShortcutsHint: "Need to decide the whole request at once?",
      macroOverwriteAcceptTitle: "Accept the entire request?",
      macroOverwriteRejectTitle: "Reject the entire request?",
      macroOverwriteDescription: "{count} service decisions are already set. This action will overwrite them and apply the same decision to every service in this request.",
      // Phase 5.2.2 — template-level decision vocabulary
      decidePerTemplate: "Decide per test",
      templateSummary: "{accepted} of {total} tests accepted",
      acceptAllTemplates: "Accept all tests in this service",
      rejectAllTemplates: "Reject all tests in this service",
      rejectTemplate: "Reject test",
      rejectTemplateDescription: "Provide a precise reason for rejecting this specific test. The requesting stable will see which test was refused and why.",
      partialShort: "partial",
      optionalTemplate: "Optional",
    },
    
    // Stable Results Tab
    stableResults: {
      tabLabel: "Results",
      title: "Lab Results",
      subtitle: "View results from your lab test requests",
      noResults: "No results yet",
      noResultsDesc: "Results will appear here when labs publish them",
      labName: "Lab",
      publishedAt: "Published",
      report: "report",
      reports: "reports",
    },

    // L4-a-1 — shared read-only report renderer keys.
    report: {
      print: "Print",
      reportStyle: "Report style",
      reportLanguage: "Report language",
      languageEnglish: "English",
      languageArabic: "Arabic",
      parameter: "Parameter",
      value: "Value",
      unit: "Unit",
      referenceRange: "Reference Range",
      status: "Status",
      interpretation: "Interpretation",
      interpretationSummary: "Summary",
      interpretationRecommendations: "Recommendations",
      interpretationStatus: "Status",
      templateContextUnavailable: "Template context unavailable — showing raw values.",
      extraValues: "Additional Values",
      normal: "Normal",
      low: "Low",
      high: "High",
      notAvailable: "N/A",
      reportDate: "Report Date",
      sampleId: "Sample ID",
      laboratory: "Laboratory",
      horse: "Horse",
      combinedReport: "Combined Sample Report",
      analysesCount: "{{count}} analyses",
      incompleteWarning: "This report is incomplete. Some analyses have not been recorded yet.",
      partialReport: "Partial Report",
      pendingApproval: "Pending Final Approval",
      finalApproved: "Final Approved Report",
      notRecorded: "Not recorded yet",
      resultSharingSection: "Result sharing",
      analysisSectionShareLabel: "Share settings for analysis",
      analysesShort: "{{count}} analyses",
      close: "Close",
    },
    
    // Messages Inbox
    messages: {
      tabLabel: "Messages",
      title: "Message Threads",
      subtitle: "All conversations on your lab requests",
      noThreads: "No conversations yet",
      noThreadsDesc: "Start a conversation on any lab request",
      messageCount: "messages",
    },
    
    // Lab Billing
    billing: {
      generateInvoice: "Generate Invoice",
      generateInvoiceDesc: "Create a draft invoice from this lab service",
      createInvoice: "Create Invoice",
      selectClient: "Select Client",
      selectClientPlaceholder: "Choose a client for billing...",
      lineItems: "Line Items",
      noItemsFound: "No billable items found",
      quantity: "Qty",
      sourceSample: "Sample",
      sourceRequest: "Request",
      invoiceCreated: "Invoice created successfully",
      invoiceError: "Failed to create invoice",
      noTenant: "No active organization",
      noPermission: "You don't have permission to create invoices",
      noItems: "No billable items found",
      notesPlaceholder: "Optional notes for the invoice...",
      checkingExisting: "Checking for existing invoice...",
      invoiceExists: "Invoice Already Exists",
      invoiceExistsDesc: "An invoice has already been generated for this item:",
      goToInvoice: "Go to Invoice",
      multipleInvoicesFound: "Multiple Invoices Found",
      chooseInvoice: "Multiple invoices have been created for this item. Choose one to view:",
      collectIntake: "Collect Payment (Intake)",
      collectFinal: "Collect Final Payment",
      missingPricesTitle: "Missing Prices",
      missingPricesDesc: "Some items are missing prices. Please update template pricing before checkout.",
      linkFailed: "Invoice created but failed to link to source record",
      viewInvoice: "View Invoice",
      fromSample: "From Sample",
      invoiceCreatedSuccess: "Invoice created successfully!",
      recordPaymentNow: "Record Payment Now",
    },
    
    // Checkout step in sample wizard
    checkout: {
      title: "Checkout",
      subtitle: "Collect payment for this sample",
      collectNow: "Create Sample & Collect Payment",
      skip: "Skip Billing",
      missingPrices: "Cannot proceed: some templates are missing prices",
      depositLabel: "Intake Deposit",
      finalLabel: "Final Payment",
      sampleFee: "Sample Fee",
      invoiceCreated: "Invoice created successfully",
      multiSampleBlocked: "Immediate collection is available for one sample only. Create the samples first, then collect each sample separately from its sample card.",
    },
    
    // Alerts
    alerts: {
      mvpInfo: "Laboratory Module MVP. Create and track samples, manage results with templates.",
      requestsInfo: "Requests mode: Create and track lab test requests from external laboratories.",
    },
    
    // Filter Tabs
    filterTabs: {
      all: "All",
      today: "Today",
      received: "Received",
      unreceived: "Unreceived",
      processing: "Processing",
      completed: "Completed",
      cancelled: "Cancelled",
      retest: "Retest",
    },
    clientGrouped: {
      noClient: "No Client",
      noClientAssigned: "No Client Assigned",
      noClients: "No client groups",
      noClientsDesc: "Samples will appear here grouped by client",
      horses: "horses",
      samples: "samples",
      tests: "tests",
      progress: "Completion",
      showHorses: "Show horses",
      viewBySamples: "View by samples",
      viewByClients: "View by clients",
      sortAsc: "Smallest to Largest",
      sortDesc: "Largest to Smallest",
    },
    
    // Horse Selection (Multi-horse)
    horseSelection: {
      selectType: "Select horse source",
      internal: "Facility Horses",
      internalDesc: "From your registered horses",
      platform: "Platform Search",
      platformDesc: "Search all platform horses",
      walkIn: "Walk-in Entry",
      walkInDesc: "Quick manual entry",
      platformComingSoon: "Platform search coming soon",
      searchHorses: "Search horses...",
      selectAll: "Select all",
      noMatchingHorses: "No horses match your search",
      noHorses: "No horses in your facility",
      addHorse: "Add another horse",
      horsesSelected: "horses selected",
    },
    
    // Lab Horses Registry
    labHorses: {
      title: "Horses Registry",
      searchPlaceholder: "Search by name, microchip, passport, owner...",
      addNew: "Register New Horse",
      addNewDesc: "Register a new horse in the lab registry",
      editHorse: "Edit Horse",
      editHorseDesc: "Update horse information",
      ownerName: "Owner Name",
      ownerNamePlaceholder: "Enter owner name",
      ownerPhone: "Owner Phone",
      ownerPhonePlaceholder: "Enter owner phone",
      ownerEmail: "Owner Email",
      ownerEmailPlaceholder: "Enter owner email",
      ownerInfo: "Owner Info",
      callOwner: "Call Owner",
      registerAndSelect: "Register & Select",
      registerHorse: "Register New Horse",
      noMatchingHorses: "No horses match your search",
      noHorses: "No horses registered yet",
      noDetails: "No additional details",
      linkToPlatform: "Link to Platform Horse (Future)",
      showArchived: "Show archived",
      archived: "Archived",
      active: "Active",
      viewProfile: "View Profile",
      archiveHorse: "Archive Horse",
      restoreHorse: "Restore Horse",
      samplesCount: "Samples",
      resultsCount: "Results",
      nameEn: "Horse Name (English)",
      nameAr: "Horse Name (Arabic)",
      nameArPlaceholder: "أدخل اسم الخيل",
      financialSummary: "Financial Summary",
      noSamples: "No samples for this horse",
      noResults: "No results for this horse",
      noInvoices: "No invoices for this horse",
      totalBilled: "Total Billed",
      totalPaid: "Paid",
      outstanding: "Outstanding",
      lastSampleDate: "Last Sample",
      hasSamples: "Has Samples",
      hasOutstanding: "Has Outstanding",
      invoices: "Invoices",
      printReport: "Print Report",
      exportReport: "Export Report",
      viewClientProfile: "View Client Profile",
      notLinkedToClient: "Not linked to a client",
    },
    
    // Walk-in Horse Form
    walkIn: {
      horseName: "Horse Name",
      horseNamePlaceholder: "Enter horse name",
      passportNumber: "Passport Number",
      passportPlaceholder: "e.g. 123456789",
      microchip: "Microchip",
      microchipPlaceholder: "e.g. 985120012345678",
      breed: "Breed",
      breedPlaceholder: "e.g. Arabian",
      color: "Color",
      colorPlaceholder: "e.g. Bay",
      addHorse: "Add Horse",
    },
    
    // Sample Statuses
    sampleStatus: {
      draft: "Not Received",
      accessioned: "Received",
      processing: "Processing",
      completed: "Completed",
      cancelled: "Cancelled",
    },
    
    // Result Statuses
    resultStatus: {
      draft: "Draft",
      reviewed: "Reviewed",
      final: "Final",
    },
    
    // Result Flags
    flags: {
      normal: "Normal",
      abnormal: "Abnormal",
      critical: "Critical",
    },
    
    // Group State (for grouped views)
    groupState: {
      complete: "Complete",
      inProgress: "In Progress",
      mixed: "Mixed",
      pending: "Pending",
      allFinal: "All Final",
      allReviewed: "All Reviewed",
      allDraft: "All Draft",
    },
    
    // Samples List & Card
    samples: {
      title: "Sample",
      searchPlaceholder: "Search samples...",
      statusFilter: "Status",
      allStatus: "All Status",
      newSample: "New Sample",
      noSamplesFound: "No samples found",
      adjustFilters: "Try adjusting your filters",
      createFirst: "Create your first sample to get started",
      createSample: "Create Sample",
      received: "Received",
      retest: "Retest",
      retestNumber: "Retest #{{count}}",
      resultsProgress: "Results Progress",
      partial: "Partial",
      complete: "Complete",
      viewAllResults: "View All Results",
      unknownHorse: "Unlinked",
      unlinkedHorse: "Unlinked",
      manualEntry: "Manual Entry",
    },
    
    // Sample Actions
    sampleActions: {
      accession: "Receive",
      startProcessing: "Start Processing",
      complete: "Complete Analysis",
      cancel: "Cancel",
      createRetest: "Create Retest",
      edit: "Edit Sample",
      delete: "Delete Sample",
      confirmDelete: "Are you sure you want to delete this sample? This action cannot be undone.",
    },
    
    // Results List & Card
    results: {
      title: "Laboratory Results",
      analyses: "Analyses",
      searchPlaceholder: "Search results...",
      statusFilter: "Status",
      allStatus: "All Status",
      flagsFilter: "Flags",
      allFlags: "All Flags",
      newResult: "New Result",
      enterResult: "Enter Result",
      noResults: "No laboratory results yet",
      noResultsFound: "No results found",
      adjustFilters: "Try adjusting your filters",
      createFirst: "Create your first result to get started",
      unknownHorse: "Unknown Horse",
      unknownTest: "Unknown Test",
      unknownTemplate: "Unknown Template",
      unknownSample: "Unknown Sample",
      deleteResult: "Delete Result",
      deleteConfirmation: "Are you sure you want to delete this result? This action cannot be undone.",
      by: "by",
      reviewedBy: "Reviewed by",
      viewBySamples: "By Samples",
      viewByClients: "By Clients",
      samplesCount: "{{count}} samples",
      resultsCount: "{{count}} results",
      finalizedProgress: "Finalized",
      showSamples: "Show samples",
      refRange: "Ref. range",
      noFields: "No fields for this template",
      viewDetails: "View Details",
      status: {
        draft: "Draft",
        reviewed: "Reviewed",
        final: "Final",
      },
      publishToStable: "Publish to Stable",
      published: "Published",
      publishSuccess: "Result published to stable successfully",
      publishError: "Failed to publish result",
      notLinkedToRequest: "This result is not linked to a Stable request and cannot be published",
      noRequest: "No request",
      draftCannotPublish: "Review or finalize this result before publishing",
      publishAll: "Publish All Eligible",
      // Phase 7 — Results Coherence
      resultsOwed: "Results",
      needsResult: "Needs result",
      pendingResult: "Pending result",
      refusedAtIntake: "Refused at intake",
      publishedOfAccepted: "{published} of {total} published",
      publishedOfAcceptedShort: "{published}/{total}",
      resultsProgressTitle: "Results progress",
      cannotPublishNotFinal: "Cannot publish: result is not final",
      cannotResultNotAccepted: "Cannot create result: template not accepted",
      publishAllFinal: "Publish all final",
      publishedCount: "Published {n} results",
      nothingToPublish: "Nothing to publish",
      createResult: "Create result",
      editDraft: "Edit",
      loadingProgress: "Loading progress…",
      testResultsBreakdown: "Test results",
      // Phase 8 — Polish
      noAcceptedTemplatesHint: "Make intake decisions to start authoring results.",
      allRefusedNoResults: "All tests refused — no results expected.",
      duplicateResultExists: "A result already exists for this template and sample.",
    },
    
    // Result Actions
    resultActions: {
      viewDetails: "View Details",
      markReviewed: "Mark as Reviewed",
      finalize: "Finalize",
      delete: "Delete",
      review: "Review",
      approve: "Approve",
    },
    
    // Result Preview Dialog
    preview: {
      title: "Result Preview",
      designTemplate: "Design",
      classic: "Classic",
      modern: "Modern",
      compact: "Compact",
      labReport: "Laboratory Results Report",
      reportDate: "Report Date",
      horse: "Horse",
      sampleId: "Sample ID",
      collectionDate: "Collection Date",
      testType: "Test Type",
      testResults: "Test Results",
      parameter: "Parameter",
      value: "Value",
      unit: "Unit",
      referenceRange: "Reference Range",
      status: "Status",
      interpretation: "Interpretation",
      noInterpretation: "No interpretation provided.",
      createdBy: "Created by",
      reviewedBy: "Reviewed by",
      finalReport: "FINAL REPORT",
      draftReport: "DRAFT",
      reviewedReport: "REVIEWED",
      low: "Low",
      high: "High",
      print: "Print",
      pdf: "PDF",
      printSavePdf: "Print / Save as PDF",
      share: "Share",
      whatsapp: "WhatsApp",
      telegram: "Telegram",
      copyLink: "Copy Link",
      linkCopied: "Link copied",
      pdfDownloaded: "PDF downloaded successfully",
      pdfFailed: "Failed to generate PDF",
      allowPopups: "Please allow popups for printing",
    },
    
    // Share Panel
    share: {
      title: "Share Result",
      notFinalWarning: "Only finalized results can be shared. Please finalize this result first.",
      hideHorseName: "Hide horse name (use alias)",
      showRealName: "Show real horse name",
      expiryDate: "Link expiry date",
      createShareLink: "Create Share Link",
      activeLinks: "Active Links",
      revokedLinks: "Revoked Links",
      active: "Active",
      expired: "Expired",
      revoked: "Revoked",
      alias: "Alias",
      created: "Created",
      expires: "Expires",
      linkCreated: "Share link created and copied to clipboard",
      linkCopiedClipboard: "Link copied to clipboard",
      open: "Open",
      copy: "Copy",
      revoke: "Revoke",
      aliasToggleLabel: "Hide horse name and use an alias",
      aliasInputLabel: "Horse alias",
      aliasPlaceholder: "Enter the alias shown in the shared link",
      aliasHelper: "This alias will appear instead of the real horse name on the shared result page.",
      aliasRequired: "Please enter an alias before creating the link.",
      aliasUnavailable: "The alias is saved on this share link and will not change if the horse record is renamed later.",
      aliasSnapshotHelper: "The alias is saved on this share link and will not change if the horse record is renamed later.",
      createdBy: "Created by",
      realName: "Real name",
      nameMode: "Name mode",
      createdOn: "Created on",
      expiresOn: "Expires on",
      revokedOn: "Revoked on",
      pickDate: "Pick a date",
      clearDate: "Clear",
    },

    reportShare: {
      title: "Share Report",
      selectedAnalyses: "Selected analyses",
      selectAll: "Select all",
      clear: "Clear",
      selectAtLeastOne: "Select at least one analysis.",
      preferredLanguage: "Preferred report language",
      linkIncludesNAnalyses: "Includes {count} analyses",
      createReportShareLink: "Create report share link",
      activeLinks: "Active report links",
      revokedLinks: "Revoked report links",
      shareSelectedAnalyses: "Share selected analyses",
      linkCreated: "Report share link created",
      linkCopied: "Report link copied to clipboard",
      linkFailed: "Failed to create report share link",
    },
    sharedResult: {
      labReport: "Lab Report",
      print: "Print",
      pdf: "PDF",
      unableToView: "Unable to View Result",
      linkInvalid: "This link is invalid, expired, revoked, or the result is not yet finalized.",
      linkError: "Link invalid, expired, revoked, or result not final",
      loadFailed: "Failed to load the shared result",
      labResultsReport: "Laboratory Results Report",
      horse: "Horse",
      testType: "Test Type",
      reportDate: "Report Date",
      status: "Status",
      testResults: "Test Results",
      parameter: "Parameter",
      value: "Value",
      unit: "Unit",
      reference: "Reference",
      noResultData: "No result data available",
      interpretation: "Interpretation",
      noInterpretation: "No interpretation provided.",
      pdfSuccess: "PDF downloaded successfully",
      pdfFailed: "Failed to generate PDF",
      allowPopups: "Please allow popups for printing",
    },
    
    // Credits Panel
    credits: {
      title: "Lab Credits",
      balance: "Balance",
      currentBalance: "Current Balance",
      sampleCost: "Sample Cost",
      balanceAfter: "Balance After",
      insufficientCredits: "Insufficient credits. Please purchase more credits to continue.",
    },
    
    // Create Sample Dialog
    createSample: {
      title: "Create Sample",
      createSample: "Create Sample",
      createSamples: "Create Samples",
      createRetest: "Create Retest",
      steps: {
        client: "Client",
        horses: "Horses",
        basic: "Basic Info",
        templates: "Templates",
        details: "Details",
        checkout: "Checkout",
        billing: "Invoice",
        credits: "Credits",
        review: "Review",
      },
      horse: "Horse",
      selectHorse: "Select horse",
      collectionDate: "Collection Date",
      selectDate: "Select date",
      sampleId: "Sample ID",
      dailyNumber: "Daily Sample #",
      dailyNumberPlaceholder: "e.g. 1",
      dailyNumberHint: "Auto-generated, or enter manually",
      autoNextNumber: "Auto Next",
      useNext: "Use Next",
      dailyNumberRange: "Numbers #{{start}} to #{{end}}",
      autoGenerated: "Auto-generated if empty",
      generate: "Generate",
      selectTemplates: "Select Required Templates",
      selected: "selected",
      noTemplates: "No templates available. You can create them in Laboratory Settings.",
      templatesHint: "Optional - Select templates for easier result entry",
      client: "Client (Optional)",
      selectClient: "Select client",
      noClient: "No client",
      notes: "Notes",
      notesPlaceholder: "Sample collection notes, special handling instructions...",
      reviewTitle: "Review Sample Information",
      selectedTemplates: "Selected Templates",
      relatedOrder: "Linked Order",
      creditDeduction: "1 credit will be deducted from your balance",
      unknown: "Unknown",
      retestOf: "Retest Of",
      templates: "Templates",
      noTemplatesSelected: "No templates selected",
      creating: "Creating...",
      creatingRetest: "Creating retest",
      freeRetest: "Free Retest",
      creatingRetestFor: "Creating retest #{{count}} for sample {{id}}",
      fields: "fields",
      perSampleDailyNumbers: "Daily Numbers per Sample",
      autoFillSequential: "Auto-fill Sequential",
      // New LAB wizard translations
      customizeTemplatesPerHorse: "Customize templates for each horse",
      createInvoice: "Create Invoice",
      invoicePreview: "Invoice Preview",
      skipInvoiceInfo: "You can create an invoice later from the sample details.",
      missingPricesWarning: "Some templates don't have prices. You can add them in the invoice later.",
      noClientForInvoice: "No client selected. You can add the client to the invoice later.",
      invoiceStatus: "Invoice Status",
      willCreateInvoice: "Will create invoice",
      noInvoice: "No invoice",
      // Phase 7: Receive & Number Now
      receiveNow: "Receive & Number Now",
      receiveNowDesc: "Sample is physically received. A daily number will be assigned.",
      deferNumbering: "Save as Draft",
      deferNumberingDesc: "Number will be assigned when sample is received later.",
      receiveStatus: "Receipt Status",
      willReceiveNow: "Will receive & number now",
      willDefer: "Draft (number later)",
      missing: {
        selectClient: "Select a client",
        enterWalkInClientName: "Enter the walk-in client name",
        enterNoClientReason: "Enter a reason for not linking a client (min 5 characters)",
        addHorseOrSubject: "Add at least one horse or sample subject",
        selectAtLeastOneTemplate: "Select at least one test/template",
        selectTemplatesPerHorse: "Select templates for each selected horse",
        setCollectionDate: "Set the collection date",
        selectClientForInvoice: "Select a client before creating an invoice",
        completeRequiredPrices: "Complete the required prices",
        insufficientCredits: "Insufficient credits",
      },
    },
    
    // Client Mode Selector
    clientMode: {
      existing: "Existing Client",
      walkin: "Walk-in Client",
      none: "No Client",
      // New LAB wizard client modes
      registered: "Registered Client",
      registeredDesc: "Select an existing client from database",
      newClient: "New Client",
      newClientDesc: "Create a new client and register in the system",
      noneDesc: "Continue without linking the sample to a client",
      createNewClient: "Create New Client",
      clientCreated: "Client created",
      // No client reason
      noClientReason: "Why is the sample not linked to a client?",
      noClientReasonPlaceholder: "Enter reason here...",
      noClientReasonRequired: "Please provide a reason for not linking a client",
    },
    
    // Walk-in Client Form
    walkInClient: {
      name: "Client Name",
      namePlaceholder: "Enter client name",
      phone: "Phone Number",
      phonePlaceholder: "+966 5XX XXX XXXX",
      email: "Email Address",
      emailPlaceholder: "client@example.com",
      notes: "Notes",
      notesPlaceholder: "Additional client notes...",
    },
    
    // Create Result Dialog
    createResult: {
      title: "Enter Results",
      steps: {
        selectSample: "Select Sample",
        selectTemplate: "Select Template",
        enterResults: "Enter Results",
        review: "Review",
        continue: "Continue",
      },
      selectProcessingSample: "Select a sample that is currently being processed.",
      noProcessingSamples: "No samples in processing stage. Start processing a sample first.",
      analysisProgress: "Analysis Progress",
      selectTemplateForResults: "Choose a template for entering results.",
      noTemplatesForSample: "No templates specified for this sample.",
      completed: "Completed",
      enterResultsBelow: "Enter the results below",
      requiredField: "Required",
      normalRange: "Normal range",
      overallFlags: "Overall Flags",
      interpretation: "Interpretation",
      interpretationPlaceholder: "Notes about this result...",
      reviewResults: "Review Results",
      template: "Template",
      sample: "Sample",
      flags: "Flags",
      saveResult: "Save Result",
      updateResult: "Update Result",
      saving: "Saving...",
      savedSuccessfully: "Result saved. More templates remaining.",
      continueNext: "Continue to Next",
      finishLater: "Finish Later",
      allComplete: "All templates completed!",
      missing: {
        sample: "Select a sample",
        template: "Select a result template",
        requiredFields: "Complete required result fields",
        stepInfo: "Complete the required information for this step",
      },
    },
    
    // Bottom Navigation
    bottomNav: {
      samples: "Samples",
      results: "Results",
      compare: "Compare",
      templates: "Templates",
      settings: "Settings",
    },
    
    // Service Catalog
    catalog: {
      title: "Service Catalog",
      subtitle: "Manage your lab's test offerings",
      addService: "Add Service",
      editService: "Edit Service",
      searchPlaceholder: "Search services...",
      noServices: "No Services Yet",
      noServicesDesc: "Add your first lab service to your catalog",
      noServicesAvailable: "No Services Available",
      noServicesForLab: "{{lab}} has no active services listed",
      selectLabFirst: "Select a laboratory to view their service catalog",
      noAccess: "Access Restricted",
      noAccessDesc: "You need a partnership with this lab to view their services",
      servicesFrom: "Services from",
      nameEn: "Name (English)",
      nameAr: "Name (Arabic)",
      code: "Code",
      category: "Category",
      sampleType: "Sample Type",
      turnaround: "Turnaround (hours)",
      price: "Price",
      currency: "Currency",
      descPlaceholder: "Describe what this test involves...",
      activeDesc: "Available for ordering",
      inactive: "Inactive",
      created: "Service created",
      updated: "Service updated",
      createFailed: "Failed to create service",
      updateFailed: "Failed to update service",
      // Phase 10: Linked Templates
      linkedTemplates: "Linked Templates",
      selectedCount: "{{count}} selected",
      clearSelection: "Clear selection",
      searchCategoriesPlaceholder: "Search categories...",
      noMatches: "No analyses match your filters",
      atLeastOneAnalysis: "Please select at least one analysis.",
      selectedAnalysesTotal: "Selected Analyses Total",
      addTemplate: "Add",
      removeTemplate: "Remove",
      required: "Required",
      searchTemplates: "Search templates...",
      noAvailableTemplates: "No available templates",
      // Phase 10: Publish / Link
      publishLink: "Publish / Link",
      createNewService: "Create New Service",
      linkToExisting: "Link to Existing Service",
      selectService: "Select Service",
      // Phase 13: Pricing
      pricing: "Pricing",
      pricingMode: "Pricing Mode",
      sumTemplates: "Sum of Templates",
      overridePrice: "Override Price",
      discountType: "Discount Type",
      discountNone: "None",
      discountPercentage: "Percentage",
      discountFixed: "Fixed",
      discountValue: "Discount Value",
      // Request actions
      createSampleFromRequest: "Create Sample from Request",
      restartRetest: "Restart / Retest",
      // Fast-path summary cards
      summaryClient: "Client",
      summaryHorse: "Horse",
      summaryTemplates: "Templates",
      summaryEdit: "Edit",
      summaryNotAvailable: "Not available",
      summaryFromRequest: "From Request",
      summaryAutoResolved: "Auto-resolved",
      summaryCount: "{{count}} templates",
      // Deactivate
      deactivateTitle: "Deactivate this service?",
      deactivateDesc: "This service will no longer be selectable in new requests. You can reactivate it later.",
      deactivate: "Deactivate",
      // Hard delete
      deleteTitle: "Delete service permanently?",
      deleteDesc: "This will remove the service. If it has existing requests or invoices, deletion may be blocked to protect history.",
      deleteConfirm: "Delete permanently",
      deleteBlocked: "Cannot delete: this service is used in existing requests. Deactivate it instead.",
      deleted: "Service deleted",
      deleteFailed: "Failed to delete service",
      // Table columns
      name: "Name",
      active: "Active",
      actions: "Actions",
    },
    
    // Toast Messages (for hooks)
    toasts: {
      // Samples
      sampleCreated: "Sample created successfully",
      sampleUpdated: "Sample updated successfully",
      sampleDeleted: "Sample deleted successfully",
      sampleNotFound: "Sample not found in this organization",
      sampleMarkedReceived: "Sample marked as received",
      sampleMarkedUnreceived: "Sample marked as unreceived",
      unableToMarkUnreceived: "Unable to mark as unreceived. This action may not be allowed.",
      retestCreated: "Retest sample created successfully",
      retestOnlyCompleted: "Can only create retest for completed samples",
      maxRetestsReached: "Maximum retests (3) reached for this sample",
      failedToLoadSamples: "Failed to load samples",
      failedToCreateSample: "Failed to create sample",
      failedToUpdateSample: "Failed to update sample",
      failedToDeleteSample: "Failed to delete sample",
      failedToMarkReceived: "Failed to mark sample as received",
      failedToCreateRetest: "Failed to create retest",
      
      // Results
      resultCreated: "Result created successfully",
      resultUpdated: "Result updated successfully",
      resultDeleted: "Result deleted successfully",
      resultNotFound: "Result not found in this organization",
      resultReviewed: "Result reviewed successfully",
      resultFinalized: "Result finalized successfully",
      cannotReviewNotDraft: "Cannot review: Result must be in draft status",
      cannotFinalizeNotReviewed: "Cannot finalize: Result must be reviewed first",
      resultAlreadyFinal: "Result is already finalized",
      failedToLoadResults: "Failed to load results",
      failedToCreateResult: "Failed to create result",
      failedToUpdateResult: "Failed to update result",
      failedToDeleteResult: "Failed to delete result",
      failedToReviewResult: "Failed to review result",
      failedToFinalizeResult: "Failed to finalize result",
      
      // Shares
      shareLinkCreated: "Share link created",
      shareLinkRevoked: "Share link revoked",
      shareNotFound: "Share not found in this organization",
      onlyFinalCanShare: "Only final results can be shared",
      failedToLoadShares: "Failed to load shares",
      failedToCreateShare: "Failed to create share link",
      failedToRevokeShare: "Failed to revoke share link",
      
      // General
      noActiveOrganization: "No active organization",
      notAuthenticated: "Not authenticated",
      duplicateDailyNumber: "Sample number {{number}} is already used for horse: {{horseName}}",
      
      // Horse Links (UHP)
      horseLinkCreated: "Horse linked to client successfully",
      horseLinkFailed: "Failed to link horse to client",
    },
    
    // Filters
    filters: {
      fromDate: "From Date",
      toDate: "To Date",
      clearDates: "Clear Dates",
      client: "Client",
      horse: "Horse",
      status: "Status",
      statuses: "Statuses",
      searchClient: "Search clients...",
      searchHorse: "Search horses...",
    },
    
    // Table Headers
    table: {
      number: "#",
      client: "Client",
      horse: "Horse",
      sampleId: "Sample ID",
      status: "Status",
      collectionDate: "Collection Date",
      templates: "Templates",
      results: "Results",
      actions: "Actions",
      resultDate: "Result Date",
      template: "Template",
      flags: "Flags",
      createdBy: "Created By",
    },
    
    
    // Progress Stepper
    progress: {
      received: "Received",
      accessioned: "Accessioned",
      processing: "Processing",
      readyForResults: "Ready for Results",
      sent: "Sent",
    },
    
    // Templates
    templates: {
      title: "Templates",
      selectTemplates: "Select Templates",
      selectedCount: "{{count}} templates selected",
      confirmSelection: "Confirm Selection",
      add: "Add Template",
      edit: "Edit Template",
      name: "Name",
      price: "Price",
      includedTests: "Included Tests",
      noFieldsDefined: "No fields defined for this template",
      viewDetails: "View Template Details",
      templates: "templates",
      duplicateTemplate: "Duplicate Template",
      duplicateDesc: "Create a copy of",
      newName: "New Name",
      newNamePlaceholder: "Enter new template name",
      duplicate: "Duplicate",
      deleteTemplate: "Delete Template",
      deleteConfirm: "Are you sure you want to delete",
      searchPlaceholder: "Search templates...",
      filterByType: "Filter by type",
      filterByStatus: "Filter by status",
      allTypes: "All Types",
      allStatus: "All Status",
      noTemplatesFound: "No templates found",
      adjustFilters: "Try adjusting your search or filters",
      createFirst: "Create your first template",
      seedDefaults: "Seed Default Templates",
      createTemplate: "Create Template",
      // Additional keys for LabTemplatesManager
      noTemplates: "No templates",
      noTemplatesDesc: "Create your first template to get started",
      addDefaults: "Add Defaults",
      addTemplate: "Add Template",
      createCustom: "Create Custom",
      editTemplate: "Edit Template",
      newTemplate: "New Template",
      noPrice: "No Price",
      rules: "rules",
      // Bilingual labels (explicit)
      nameEnRequired: "Name (English) *",
      nameEn: "Name (English)",
      fieldNameEn: "Field name (English)",
      fieldNameAr: "Field name (Arabic)",
      groupNameEn: "Group name (English)",
      groupNameAr: "Group name (Arabic)",
      discountNameEn: "Discount name (English)",
      discountNameAr: "Discount name (Arabic)",
      ruleNameEn: "Rule name (English)",
      ruleNameAr: "Rule name (Arabic)",
      // Header actions
      preview: "Preview",
      previewDesc: "Review the template before saving",
      activeInHeader: "Active",
      // Stats / KPI widgets
      totalTemplates: "Total",
      activeTemplates: "Active",
      inactiveTemplates: "Inactive",
      // Confirmations (in-editor deletions)
      confirmDeleteFieldTitle: "Delete field?",
      confirmDeleteFieldDesc: "This will remove the field from the template. You can't undo this action.",
      confirmDeleteGroupTitle: "Delete group?",
      confirmDeleteGroupDesc: "This will remove the group and unassign its fields. You can't undo this action.",
      confirmDeleteDiscountTitle: "Delete discount?",
      confirmDeleteDiscountDesc: "This will remove the discount rule. You can't undo this action.",
      confirmDeleteRuleTitle: "Delete rule?",
      confirmDeleteRuleDesc: "This will remove the diagnostic rule. You can't undo this action.",
      confirmCancel: "Cancel",
      confirmDelete: "Delete",
      // Collapsed summaries
      requiredBadge: "Required",
      optionalBadge: "Optional",
      noGroupBadge: "No group",
      // Placeholders
      placeholderTemplateName: "e.g., Complete Blood Count",
      placeholderTemplateNameAr: "Name in Arabic",
      placeholderDescription: "Template description...",
      placeholderDescriptionAr: "Arabic description...",
      placeholderFieldName: "Field name",
      placeholderFieldNameAr: "Field name (Arabic)",
      placeholderDiscountName: "Discount name",
      placeholderDiscountNameAr: "Discount name (Arabic)",
      placeholderRuleName: "Rule name",
      placeholderRuleNameAr: "Rule name (Arabic)",
      optionsRequired: "Options are required for this field type",
      fileNotSupported: "File uploads handled separately",
      min: "Min",
      max: "Max",
      minQty: "Min Qty",
      nameRequired: "Template name is required",
      nameAr: "Name (Arabic)",
      nameArPlaceholder: "Enter Arabic name",
      description: "Description",
      descriptionEn: "Description (English)",
      descriptionPlaceholder: "Enter description",
      descriptionAr: "Description (Arabic)",
      descriptionArPlaceholder: "Enter Arabic description",
      type: "Type",
      selectType: "Select type",
      category: "Category",
      categoryPlaceholder: "Enter category",
      categoryAr: "Category (Arabic)",
      categoryArPlaceholder: "Enter Arabic category",
      version: "Version",
      active: "Active",
      activeDesc: "Template is available for use",
      groups: "Groups",
      addGroup: "Add Group",
      editGroup: "Edit Group",
      groupName: "Group Name",
      groupNamePlaceholder: "Enter group name",
      noGroups: "No groups defined",
      deleteGroup: "Delete Group",
      deleteGroupConfirm: "Are you sure? Fields in this group will become unassigned.",
      fields: "Fields",
      addField: "Add Field",
      editField: "Edit Field",
      fieldName: "Field Name",
      fieldNamePlaceholder: "Enter field name",
      fieldType: "Field Type",
      selectFieldType: "Select field type",
      unit: "Unit",
      unitPlaceholder: "Enter unit (e.g., mg/dL)",
      unitAr: "Unit (Arabic)",
      unitArPlaceholder: "Enter Arabic unit",
      required: "Required",
      requiredDesc: "This field must be filled",
      assignToGroup: "Assign to Group",
      noGroup: "No Group",
      noFields: "No fields defined",
      deleteField: "Delete Field",
      deleteFieldConfirm: "Are you sure you want to delete this field?",
      options: "Options",
      addOption: "Add Option",
      optionPlaceholder: "Enter option",
      noOptions: "No options defined",
      normalRanges: "Normal Ranges",
      minValue: "Min Value",
      maxValue: "Max Value",
      pricing: "Pricing",
      basePrice: "Base Price",
      currency: "Currency",
      enableDiscounts: "Enable Discounts",
      enableDiscountsDesc: "Allow discounts to be applied to this template pricing",
      discounts: "Discounts",
      addDiscount: "Add Discount",
      discountName: "Discount Name",
      discountNamePlaceholder: "Enter discount name",
      discountType: "Type",
      percentage: "Percentage",
      fixed: "Fixed Amount",
      bulk: "Bulk Discount",
      discountValue: "Value",
      minQuantity: "Min Quantity",
      finalPrice: "Final Price",
      noDiscounts: "No discounts defined",
      diagnosticRules: "Diagnostic Rules",
      addRule: "Add Rule",
      editRule: "Edit Rule",
      ruleName: "Rule Name",
      ruleNamePlaceholder: "Enter rule name",
      condition: "Condition",
      conditionPlaceholder: "Enter condition (e.g., {{FieldName}} > 10)",
      insertField: "Insert Field",
      availableTokens: "Available field tokens",
      insertToken: "Insert token",
      unknownTokens: "Unknown field tokens",
      addFieldsForTokens: "Add fields first to use tokens in diagnostic rules.",
      interpretation: "Interpretation",
      normal: "Normal",
      abnormal: "Abnormal",
      critical: "Critical",
      warning: "Warning",
      message: "Message",
      messageEn: "Message (English)",
      messagePlaceholder: "Enter message",
      messageAr: "Message (Arabic)",
      messageArPlaceholder: "Enter Arabic message",
      noRules: "No diagnostic rules defined",
      deleteRule: "Delete Rule",
      deleteRuleConfirm: "Are you sure you want to delete this rule?",
      saving: "Saving...",
      created: "Template created successfully",
      updated: "Template updated successfully",
      duplicated: "Template duplicated successfully",
      deleted: "Template deleted successfully",
      rangeChip: "Range",
      noRangeChip: "No range",
      optionsCount: "options",
      nav: {
        identity: "Identity",
        overview: "Overview",
        groups: "Groups",
        fields: "Fields",
        pricing: "Pricing",
        rules: "Rules",
        sectionsLabel: "Sections",
        jumpTo: "Jump to section",
      },
      validation: {
        missingName: "Enter template name",
        fieldMissingOptions: "select fields need options",
        invalidRange: "normal ranges are invalid",
        unknownTokens: "unknown diagnostic tokens",
        requiredIssues: "Required",
        warnings: "Warnings",
        fixInSection: "Go to section",
      },
    },
  },
  movement: {
    title: "Horse Movements",
    subtitle: "Track entries, exits, and transfers between locations",
    tabs: {
      movements: "Movements",
      locations: "Locations",
      settings: "Settings",
    },
    direction: {
      from: "From {{from}}",
      to: "To {{to}}",
      fromTo: "From {{from}} to {{to}}",
      unspecified: "unspecified",
    },
    types: {
      in: "Arrival",
      out: "Departure",
      transfer: "Inter-Branch Transfer",
      manual: "Manual",
      inHelper: "Record a horse entering a branch — new from outside, returning, or coming back from a temporary outing.",
      outHelper: "Record a horse leaving care: going to an outside destination, sent to a connected partner, or going out temporarily and expected back.",
      transferHelper: "Move a horse between two internal branches. Do not use this to assign a housing unit — use Housing Tasks for that.",
    },
    guards: {
      sameBranchTransfer: "This is the same branch. Use Housing Tasks or Assign Housing Unit to reassign within the same branch.",
      selectMovementType: "Select a movement type",
      selectArrivalSource: "Select an arrival source",
      selectHorse: "Select a horse",
      horseName: "Horse name is required",
      horseGender: "Horse gender is required",
      fromLocation: "Source branch is required",
      toLocation: "Destination branch is required",
      externalDestination: "External destination is required",
      connectedPartner: "Connected partner is required",
      connectedDirectionUnsupported: "Connected partner is only supported for departures",
      departureSubtype: "Departure type is required",
    },
    missingFields: {
      movementType: "Movement type",
      arrivalSource: "Arrival source",
      horse: "Horse",
      sourceLocation: "Source branch",
      destinationLocation: "Destination branch",
      externalDestination: "External destination",
      connectedDestination: "Connected destination",
      arrivalBranch: "Arrival branch",
      departureSubtype: "Departure type",
      newHorseName: "New horse name",
    },
    class: {
      arrival: "Arrival",
      admission_checkin: "Admission Check-in",
      checkout_departure: "Final Checkout",
      temporary_out: "Temporary Out",
      return_from_temporary_out: "Return from Temporary Out",
      connected_outgoing: "Connected Outgoing",
      inter_branch_transfer: "Inter-Branch Transfer",
      unit_assignment: "Housing Unit Assignment",
      unit_reassignment: "Housing Unit Reassignment",
      unknown: "Movement",
    },
    reason: {
      system: {
        boardingAdmissionCheckin: "Boarding admission check-in",
        boardingAdmissionCheckout: "Boarding admission checkout",
        internalUnitReassignment: "Internal housing unit reassignment",
        boardingContractScheduledArrival: "Scheduled from Boarding Contract",
      },
    },
    note: {
      system: {
        initialUnitPlacement: "Initial housing unit assignment",
        internalUnitMove: "Internal housing unit reassignment",
        internalUnitReassignment: "Internal housing unit reassignment",
      },
    },
    detail: {
      title: "Movement Detail",
      internalNote: "Internal Note",
      reason: "Reason",
      time: "Time",
      linkedAdmission: "Linked to boarding admission",
      viewAdmission: "View Admission",
      admissionLinkApprox: "Best-match admission within 24h.",
      admissionLinkDirect: "Linked admission file.",
      noMatchingAdmission: "No matching admission file was found for this movement.",
    },
    destination: {
      internal: "Internal",
      external: "Not Registered on Dayli Horse",
      connected: "Registered on Dayli Horse",
      externalLocation: "External Location",
      selectExternal: "Select external location",
      addNew: "Add New Location",
      locationName: "Location Name",
      city: "City",
      connectedEntity: "Connected Entity",
      noConnectedDestinations: "No connected stables or clinics yet",
      noConnectedDestinationsHint: "Search for and request a partnership with another stable or clinic to enable connected movements.",
      requestPartnership: "Request Partnership",
      linkAnotherOrganization: "Link Another Organization",
      partnershipRequested: "Partnership request sent",
      orUseExternal: "Or use External destination for non-partner locations",
      pendingRequests: "Pending Requests",
      awaitingResponse: "Awaiting response",
      types: {
        stable: "Stable",
        clinic: "Clinic",
        lab: "Laboratory",
        venue: "Venue",
        farm: "Farm",
        other: "Other",
      },
    },
    connected: {
      noConnection: "No accepted connection exists with this entity",
      noPermission: "You do not have permission to send connected movements.",
      noPermissionHint: "Ask a workspace admin to grant the connected movement permission.",
      recordFailed: "Could not record the connected movement. Please try again or contact support.",
    },
    incoming: {
      title: "Incoming Arrivals",
      empty: "No incoming arrivals",
      emptyDesc: "Incoming horses from connected entities will appear here",
      from: "From",
      scheduledAt: "Scheduled",
      statusPending: "Pending",
      statusCompleted: "Completed",
      statusCancelled: "Cancelled",
      confirmArrival: "Confirm Arrival",
      cancelAction: "Decline",
      confirmTitle: "Confirm Horse Arrival",
      confirmDesc: "This will mark the horse as arrived at your facility. You can then proceed with the admission process.",
      confirmed: "Arrival confirmed successfully",
      confirmFailed: "Failed to confirm arrival",
      confirmFailedFallback: "Could not confirm arrival. Please choose a receiving branch and try again.",
      cancelTitle: "Decline Incoming Horse",
      cancelDesc: "Are you sure you want to decline this incoming horse?",
      cancelled: "Incoming movement declined",
      cancelFailed: "Failed to decline movement",
      fromContract: "Boarding Contract",
      confirmArrivalBranch: {
        title: "Confirm Arrival",
        description: "Select the receiving branch for this horse.",
        receivingBranch: "Receiving branch",
        preferredHint: "Defaulted from contract preference — change if different.",
        soleHint: "Only active branch — change if different.",
        multiHint: "Please select the receiving branch.",
        selectPlaceholder: "Select branch",
        submit: "Confirm Arrival",
        dirtyDiscardTitle: "Discard changes?",
        dirtyDiscardDesc: "Branch selection will be lost.",
        branchRequired: "Receiving branch is required.",
        connectedHint: "Choose the branch that will receive this horse.",
        noActiveBranches: "No active branch found. Add or activate a branch before confirming arrival.",
        emptyStateActionable: "No active receiving branches are available. Add a branch to continue confirming arrival.",
        quickAddCta: "Add new branch",
        addAnotherBranch: "Add another branch",
        quickAddSuccess: "Branch created and selected.",
      },
    },
    lifecycle: {
      statusScheduled: "Scheduled",
      statusDispatched: "Dispatched",
      statusCompleted: "Completed",
      statusCancelled: "Cancelled",
      scheduled: "Movement scheduled successfully",
      dispatched: "Horse dispatched — housing cleared",
      dispatchFailed: "Failed to dispatch movement",
      dispatchFailedFallback: "Could not confirm dispatch. Please try again or contact support.",
      confirmDispatch: "Confirm Dispatch",
      confirmDispatchTitle: "Confirm Horse Dispatch",
      confirmDispatchDesc: "This will mark the horse as physically departed. Housing will be cleared and occupancy will close. This action cannot be undone.",
      scheduledFor: "Scheduled for",
      dispatchedAt: "Dispatched at",
      completedAt: "Completed at",
      scheduleForLater: "Schedule for later",
      scheduleForLaterDesc: "Horse stays in housing until dispatch is confirmed",
      scheduledDateTime: "Scheduled departure",
      confirmArrival: "Confirm Arrival",
      retryArrival: "Retry Arrival",
      confirmArrivalTitle: "Confirm Arrival",
      retryArrivalTitle: "Retry Arrival Completion",
      confirmArrivalDescNoUnit: "This will mark the scheduled arrival as actually arrived. No housing unit was selected, so the equine will remain arrived but unplaced until the next workflow step. No boarding admission is created automatically.",
      confirmArrivalDescWithUnit: "This will mark the scheduled arrival as actually arrived and place the equine into the selected housing unit. No boarding admission is created automatically.",
      retryArrivalDesc: "The arrival was started but did not finish. The equine is currently in transit. Confirming again will complete the arrival without re-running the dispatch step.",
      arrivalConfirmed: "Arrival confirmed",
      arrivalFailed: "Failed to confirm arrival",
      arrivalHalfFailed: "Arrival started but did not finish. The equine is currently in transit. Tap Confirm Arrival again to complete the process.",
      confirmInternalTransfer: "Confirm Internal Transfer",
      completeInternalTransfer: "Complete Internal Transfer",
      confirmInternalTransferTitle: "Confirm Internal Transfer",
      completeInternalTransferTitle: "Complete Internal Transfer",
      confirmInternalTransferDesc: "This will confirm the internal transfer of the equine to the destination branch. The boarding admission stays active. No checkout is performed.",
      confirmInternalTransferDescNoUnit: "No destination unit was selected, so the equine will need placement after arrival. Housing can assign a unit later.",
      transferConfirmed: "Internal transfer confirmed",
      transferFailed: "Failed to confirm internal transfer",
      transferHalfFailed: "Transfer started but did not finish. The equine is currently in transit. Tap Complete Internal Transfer again to finish the transfer.",
      retryCompleteTransferDesc: "The transfer was started but did not finish. The equine is currently in transit. Confirming again will complete the transfer without re-running the dispatch step.",
      opStatus: {
        housed: "Housed",
        needsAdmission: "Needs Admission",
        needsPlacement: "Needs Placement",
        temporarilyOut: "Temporarily Out",
        inTransit: "In Transit",
        scheduled: "Scheduled",
        departed: "Departed",
        unknown: "Not Currently Housed",
      },
      summary: {
        title: "Operational status",
        needsAdmissionDesc: "The equine has arrived but no boarding admission is open. Start an admission to place it under care.",
        needsPlacementDesc: "An admission is active but no housing unit is assigned. Assign a stall, paddock, or area.",
        inTransitDesc: "Movement was dispatched but not yet completed. Confirm the arrival or transfer to finish.",
        temporarilyOutDesc: "The equine is recorded as temporarily off-site. Record its return when it comes back.",
      },
    },
    timeline: {
      title: "Movement History",
      admissionLinked: "Admission",
      noMovements: "No movement history yet",
    },
    nextStep: {
      title: "Arrival Confirmed",
      description: "The horse has been recorded as arrived. What would you like to do next?",
      done: "Done",
      startAdmission: "Start Admission",
      assignLater: "Assign Housing Later",
      assignLaterToast: "You can start an admission for this horse anytime from the Admissions tab.",
    },
    filters: {
      all: "All",
      today: "Today",
      thisWeek: "This Week",
      allLocations: "All Locations",
      allTypes: "All Types",
      allStatuses: "All Statuses",
      searchPlaceholder: "Search horses...",
    },
    list: {
      noMovements: "No movements found",
      adjustFilters: "Try adjusting your filters",
      recordFirst: "Record your first horse movement",
      recordMovement: "Record Movement",
    },
    locations: {
      title: "Locations",
      activeLabel: "active locations",
      addLocation: "Add Location",
      editLocation: "Edit Location",
      name: "Location Name",
      namePlaceholder: "Enter location name",
      address: "Address",
      addressPlaceholder: "Enter address (optional)",
      city: "City",
      cityPlaceholder: "Enter city (optional)",
      noLocations: "No locations found",
      addFirst: "Add your first location",
      created: "Location created successfully",
      updated: "Location updated successfully",
      activated: "Location activated",
      deactivated: "Location deactivated",
    },
    wizard: {
      step1Title: "Select Movement Direction",
      step1Desc: "Choose whether the horse is arriving, departing, or transferring between branches.",
      step2Title: "Select Horse",
      step2Desc: "Choose the horse to move",
      step3Title: "Select Location",
      step3Desc: "Choose the location",
      step4Title: "Additional Details",
      step4Desc: "Add reason and notes (optional)",
      step5Title: "Review",
      step5Desc: "Confirm movement details",
      stepHousingTitle: "Select Housing",
      stepHousingDesc: "Optionally assign to a stall or paddock",
    },
    arrival: {
      sourceTitle: "Arrival Source",
      sourceDesc: "How is this horse arriving?",
      existingHorse: "Existing Horse",
      existingHorseDesc: "Horse already registered, returning or re-entering",
      newHorse: "New Horse from Outside",
      newHorseDesc: "Register a new horse during arrival",
      newHorseTitle: "Quick Horse Intake",
      newHorseSubtitle: "Enter minimum details. Complete the profile later.",
      searchHorses: "Search by name, passport, microchip...",
      noEligibleHorses: "No eligible horses found",
      noEligibleHorsesHint: "All horses are already housed or admitted",
      sourceNotes: "Origin / Source Notes",
      sourceNotesPlaceholder: "Where is this horse coming from?",
      incompleteProfileHint: "This will create a horse record marked as incomplete. You can complete the full profile later.",
      horseCreated: "Horse {{name}} registered successfully",
      autoDetected: "Auto-detected",
      newLabel: "New",
    },
    transfer: {
      sameBranchHint: "Same-branch transfer — select target facility/unit in the next step.",
      sameBranchBlocked: "Source and destination must be different branches. To assign or change a housing unit inside the same branch, use Housing Tasks.",
      unitLater: "No housing unit selected. After confirmation, the horse will appear under Needs Stall Assignment at the destination branch.",
      departureUseTransferHint: "Moving between your own branches? Use Inter-Branch Transfer instead.",
      sourceBranch: "Source Branch",
      destinationBranch: "Destination Branch",
      branch: "Branch",
      fromUnit: "From Unit",
      toUnit: "To Unit",
      housingNote: "Housing note",
      movementNote: "Movement note",
      needsStallAssignmentHint: "Needs Stall Assignment at destination",
      unitTransition: "Unit",
    },
    form: {
      recordMovement: "Record Movement",
      movementDate: "Movement date",
      movementType: "Movement Type",
      selectHorse: "Horse",
      fromLocation: "From Location",
      toLocation: "To Location",
      noOtherLocations: "No other branches available",
      reason: "Reason",
      reasonPlaceholder: "e.g. Training, Vet visit, Competition...",
      notes: "Notes",
      notesPlaceholder: "Additional notes...",
      internalLocationNote: "Internal Location",
      internalLocationNotePlaceholder: "e.g. Stable 3, Paddock B...",
      area: "Section",
      unit: "Unit",
      selectArea: "Select section",
      selectUnit: "Select unit",
      skipHousing: "Skip (no unit assignment)",
      destinationType: "Destination Type",
      internal: "Internal (Branch)",
      external: "External Location",
      externalLocation: "External Location",
      selectExternalLocation: "Select external location",
      addNewExternal: "Add New Location",
      externalName: "Location Name",
      externalCity: "City",
      externalType: "Type",
      externalTypes: {
        stable: "Stable",
        clinic: "Clinic",
        venue: "Venue",
        farm: "Farm",
        other: "Other",
      },
    },
    validation: {
      inRequiresToLocation: "Entry requires destination location",
      outRequiresFromLocation: "Exit requires departure location",
      transferRequiresBoth: "Transfer requires both locations",
      internalNoteRequired: "Internal note is required for same-location transfer",
      unitFull: "Unit is at full capacity",
      singleOccupied: "Unit is single-occupancy and already occupied",
      destinationRequired: "Destination location is required",
      originRequired: "Origin location is required",
    },
    labels: {
      currentHousing: "Current Housing",
      noHousing: "No assigned unit",
    },
    horseSection: {
      title: "Location & Movement",
      currentLocation: "Current Location",
      noLocation: "No location assigned",
      homeLocation: "Home Location",
      recentMovements: "Recent Movements",
      noMovements: "No movements recorded yet",
      viewAll: "View All",
    },
    housing: {
      selectLocationFirst: "Select a destination first",
      noAreasInBranch: "No sections in this branch",
      noUnitsInArea: "No units in this section",
      occupied: "Occupied",
      available: "Available",
    },
    record: {
      title: "Record Movement",
      steps: {
        type: "Type",
        horse: "Horse",
        location: "Location",
        details: "Details",
        review: "Review",
      },
      selectType: "Select movement type",
      entry: "Entry",
      entryDesc: "Horse arriving at a location",
      exit: "Exit",
      exitDesc: "Horse leaving a location",
      transfer: "Transfer",
      transferDesc: "Horse moving between locations",
      selectHorse: "Select Horse",
      searchHorses: "Search horses...",
      noHorses: "No horses found",
      fromLocation: "From Location",
      toLocation: "To Location",
      selectLocation: "Select location",
      noOtherLocations: "No other branches available",
      reason: "Reason",
      reasonPlaceholder: "e.g. Training, Vet visit, Competition...",
      notes: "Notes",
      notesPlaceholder: "Additional notes...",
      movementDate: "Movement Date",
      reviewInfo: "Review Movement",
      horse: "Horse",
      type: "Type",
      from: "From",
      to: "To",
      date: "Date",
      confirm: "Confirm Movement",
      recording: "Recording...",
      success: "Movement recorded successfully",
      failed: "Failed to record movement",
    },
    demo: {
      title: "Demo Mode",
      description: "Load sample movements and locations for testing. Demo data is tagged and can be safely removed.",
      load: "Load Demo Data",
      remove: "Remove Demo Data",
      loaded: "Demo data loaded successfully",
      removed: "Demo data removed",
      alreadyLoaded: "Demo data already exists",
    },
    toasts: {
      noActiveOrganization: "No active organization",
      movementCreated: "Movement recorded successfully",
      movementRecorded: "Movement recorded successfully",
      failedToCreate: "Failed to record movement",
      failedToRecord: "Failed to record movement",
      failedToCreateHorse: "Failed to create horse record",
      horseRequired: "A horse is required to record a movement",
      horseCreatedMovementFailed: "Horse was created but the movement could not be saved. You can find the horse in the Horses list (Incomplete profiles).",
      permissionDenied: "Permission denied",
      locationCreated: "Branch created successfully",
      locationUpdated: "Branch updated successfully",
    },
    cancel: {
      action: "Cancel movement",
      scheduledTitle: "Cancel scheduled movement",
      scheduledDesc: "This will cancel the planned movement. Current housing is not affected.",
      dispatchedTitle: "Cancel in-transit movement",
      dispatchedDesc: "The system will attempt to restore the origin housing/location when possible. If the origin unit is no longer available, the cancellation will fail.",
      reasonLabel: "Reason for cancellation (optional)",
      reasonPlaceholder: "e.g. duplicate entry, plan changed...",
      confirmButton: "Cancel movement",
      success: "Movement cancelled",
      historyTitle: "Cancelled movement",
      reasonHistoryLabel: "Cancellation reason",
      cancelledAtLabel: "Cancelled at",
      cancelledByLabel: "Cancelled by",
      noReasonProvided: "No reason was provided.",
    },
    subtype: {
      departureTypeLabel: "Departure type",
      checkoutDeparture: "Final checkout",
      checkoutDepartureHint: "Closes the boarding stay and may require financial clearance.",
      temporaryOut: "Temporary out",
      temporaryOutHint: "Keeps the horse's stay active and allows a later return.",
      chooseRequired: "Choose departure type",
    },
    return: {
      recordReturn: "Record return",
      title: "Return from temporary out",
    },
    route: {
      from: "From",
      to: "To",
    },
  },
  housing: {
    title: "Housing Management",
    subtitle: "Manage stalls, paddocks, and horse housing assignments",
    tabs: {
      branches: "Branches",
      facilities: "Facilities",
      units: "Units",
      areas: "Sections",
      admissions: "Admissions",
      plans: "Plans",
      movement: "Arrivals & Departures",
      incoming: "Incoming",
      overview: "Overview",
      arrivalsAndDepartures: "Arrivals & Departures",
    },
    arrivalsAndDepartures: {
      arrivals: "Arrivals",
      departures: "Departures",
      transfers: "Inter-Branch Transfers",
      incoming: "Incoming",
      pending: "Pending",
      completed: "Completed",
      arrivalsToday: "arrivals today",
      departuresToday: "departures today",
      arrivingToday: "Arriving today",
      departingToday: "Departing today",
      transfersToday: "Transfers today",
      needsAction: "Needs action",
    },
    branchScope: {
      selectBranch: "Select Branch",
      allBranches: "All Branches",
      noBranches: "No branches configured yet",
      overviewDesc: "Select a branch to view its facilities, horses, and occupancy.",
      facilities: "Facilities",
      horses: "Horses",
      occupancy: "Occupancy",
      vacant: "vacant",
      noUnitsYet: "No units yet",
      facilitiesSection: "Facilities",
      noFacilitiesYet: "No facilities added yet",
      unitsOccupied: "occupied",
      unassignedHorses: "Unassigned Horses",
      needsAttention: "Housing Tasks",
      needsPlacement: "Needs Stall Assignment",
      needsAdmission: "Open Admission Required",
      needsPlacementHelper: "Admission is open, but no housing unit has been assigned yet.",
      needsAdmissionHelper: "This horse is at the branch but has no active admission record.",
      unassignedGroup: "Unassigned",
      unassignedGroupHelper: "Local horses with no branch and no active admission. Start an admission to place them.",
      noNeedsPlacement: "No horses need stall assignment",
      noNeedsAdmission: "No horses need an open admission",
      placeInUnit: "Assign Housing Unit",
      startAdmission: "Open Admission",
      placeInUnitDialogTitle: "Place horse in a unit",
      placeInUnitDialogDesc: "Assign the active admission to a stall, paddock, or area at the current branch.",
      placeInUnitArea: "Area",
      placeInUnitUnit: "Unit",
      placeInUnitNoUnits: "No available units in this branch. Add or free a unit in Facilities to continue.",
      placeInUnitNoFacility: "No facility in this branch yet.",
      placeInUnitAddFacility: "Add facility",
      placeInUnitNoUnitsInFacility: "No available units in this facility.",
      placeInUnitAddUnits: "Add units",
      placeInUnitOpenFacilities: "Open Facilities",
      placeInUnitPickFacility: "Facility",
      placeInUnitNoAdmission: "No active admission found for this horse.",
      placeInUnitSuccess: "Horse placed in unit",
      placeInUnitSelectArea: "Select an area…",
      placeInUnitSelectUnit: "Select a unit…",
      placeInUnitFull: "Full",
      placeInUnitCurrentBranch: "Branch",
    },
    branchActions: {
      edit: "Edit Branch",
      editDesc: "Update branch name and location details.",
      delete: "Delete Branch",
      deleteTitle: "Delete Branch",
      deleteConfirm: "Are you sure you want to delete \"{name}\"? This action will deactivate the branch.",
      cannotDelete: "This branch cannot be deleted because it has active dependencies:",
      blockHorses: "This branch has {n} horse(s). Relocate or remove them first.",
      blockFacilities: "This branch has {n} active facility(ies). Remove or deactivate them first.",
      blockAdmissions: "This branch has active boarding admissions. Complete or cancel them first.",
      resolveFirst: "Resolve all dependencies before deleting this branch.",
      deleted: "Branch deactivated successfully",
      missing: {
        name: "Enter branch name",
      },
    },
    branchWizard: {
      title: "Create Branch",
      createBranch: "Create Branch",
      success: "Branch created successfully",
      step1Desc: "Enter the branch name and location details.",
      step2Desc: "Add initial facilities to this branch (optional).",
      stepIdentity: "Identity",
      stepFacilities: "Facilities",
      branchName: "English Name",
      branchNamePlaceholder: "e.g. Main Stable",
      branchNameAr: "Arabic Name",
      branchNameArPlaceholder: "e.g. الإسطبل الرئيسي",
      city: "City",
      cityPlaceholder: "e.g. Riyadh",
      address: "Address",
      addressPlaceholder: "Street address",
      facilitiesHint: "Add the facilities you want to create in this branch. You can add more later.",
      facilityNamePlaceholder: "Facility",
      addFacility: "Add Another Facility",
      skipFacilities: "Skip",
      skipFacilitiesConfirm: {
        title: "Skip facility setup?",
        description: "Your branch '{branchName}' has already been saved. Closing now will discard the facility details you entered, but the branch itself will be kept.",
        confirm: "Discard facility & close",
        keepEditing: "Keep editing",
      },
      branchKeptToast: "Branch saved. You can add facilities later from Facilities.",
    },
    facilityTypes: {
      barn: "Barn",
      paddock: "Paddock",
      arena: "Arena",
      isolation: "Isolation",
      pasture: "Pasture",
      wash_area: "Wash Area",
      round_pen: "Round Pen",
      storage: "Storage",
    },
    create: {
      title: "Create Facility",
      description: "Set up a new facility. The fields below adapt to the facility type you select.",
      namePlaceholder: "e.g. Main Barn",
      nameArPlaceholder: "مثال: الجناح الرئيسي",
      unitSetup: "Room / Unit Setup",
      stallSetup: "Stall Setup",
      wardSetup: "Ward Room Setup",
      unitSetupDesc: "Configure the rooms that will be created inside this facility. Click a cell to assign its function.",
      unitCount: "Number of Units",
      codePrefix: "Code Prefix",
      startNumber: "Start Number",
      layout: "Layout",
      layoutSingle: "Single Row",
      layoutTwoSided: "Two-Sided",
      aisle: "Aisle",
      previewTitle: "Preview",
      clickToAssign: "Click cell to assign function",
      clickToToggle: "Click cell to change type",
      roomDefault: "Stall",
      roomPatient: "Room",
      roomStorage: "Storage",
      roomIsolation: "Isolation",
      roomIsolationDefault: "Isolation Bay",
      addUnitsTitle: "Add Rooms",
      addUnitsDesc: "Add more rooms to {name}.",
      addUnitsSubmit: "Add Rooms",
      addUnitsTooltip: "Add more rooms",
      sideA: "Side A",
      sideB: "Side B",
      startSide: "Start from",
      startFromSideA: "Side A first",
      startFromSideB: "Side B first",
      editUnitsInfo: "rooms configured",
      openAreaSetup: "Open Area Details",
      openAreaSetupDesc: "Configure the capacity and characteristics of this open area.",
      approxCapacity: "Approximate Horse Capacity",
      capacityPlaceholder: "e.g. 12",
      activityNote: "This is an activity/service space. No housing units will be created.",
      storageNote: "This is a storage facility. No housing units will be created.",
      noZonesCapacity: "Capacity: {capacity} horses",
      noZonesDefault: "No zones configured yet. Add zones to manage group placements.",
    },
    openArea: {
      areaSizeLabel: "Area Size (m²)",
      areaSizePlaceholder: "e.g. 500",
      shadeLabel: "Shade / Shelter",
      shadeNone: "No shade",
      shadePartial: "Partial shade",
      shadeFull: "Full shelter",
      waterAvailable: "Water available",
      horsesPresent: "horses",
      horsesInArea: "{count} horses in this area",
      emptyState: "No horses currently assigned to this area.",
    },
    activity: {
      setupTitle: "Activity / Service Details",
      setupDesc: "Configure the specifications for this facility.",
      dimensions: "Dimensions (L×W)",
      dimensionsPlaceholder: "e.g. 60×20",
      diameter: "Diameter (m)",
      diameterPlaceholder: "e.g. 15",
      coveredLabel: "Cover",
      covered: "Covered",
      uncovered: "Uncovered",
      footingLabel: "Footing",
      footing_sand: "Sand",
      footing_grass: "Grass",
      footing_rubber: "Rubber",
      footing_dirt: "Dirt",
      footing_synthetic: "Synthetic",
      washPoints: "Wash Points",
      washPointsCount: "{count} wash points",
      waterTypeLabel: "Water Type",
      water_cold: "Cold water",
      water_hot_cold: "Hot & cold water",
      ready: "Ready",
      noSpecs: "No specifications configured yet.",
    },
    facilities: {
      title: "Facilities",
      addFacility: "Add Facility",
      addFacilityDesc: "Create a new facility in the selected branch.",
      editFacility: "Edit Facility",
      editFacilityDesc: "Update facility details.",
      name: "English Name",
      namePlaceholder: "Enter facility name in English",
      nameAr: "Arabic Name",
      nameArPlaceholder: "Enter facility name in Arabic (optional)",
      code: "Code",
      codePlaceholder: "e.g. BARN-1, PDK-A",
      facilityType: "Facility Type",
      branch: "Branch",
      selectBranch: "Select branch",
      noFacilities: "No facilities found",
      addFirst: "Add your first facility",
      created: "Facility created successfully",
      updated: "Facility updated successfully",
      activated: "Facility activated",
      deactivated: "Facility deactivated",
      active: "active facilities",
      occupancy: "Occupied",
      activitySpace: "Activity Space",
      activitySpaceDesc: "This facility is used for activities and training. It does not contain housing units.",
      serviceSpace: "Service Space",
      serviceSpaceDesc: "This facility is used for services. It does not contain housing units.",
      storageSpace: "Storage",
      storageSpaceDesc: "This facility is used for storage and infrastructure.",
      manageSubdivisions: "Manage Internal Structure",
      manageStalls: "Manage Stalls",
      manageZones: "Manage Zones",
      manageBays: "Manage Bays",
      manageSections: "Manage Sections",
      manageSubunits: "Manage Subdivisions",
      editTooltip: "Edit facility",
      toggleTooltip: "Toggle active status",
      vacateConfirmTitle: "Vacate Unit",
      vacateConfirmDesc: "Remove {horse} from {unit}? The horse will become unassigned.",
      reassignWarningTitle: "Move Horse",
      reassignWarningDesc: "{horse} is currently in {fromUnit}. Assigning here will move the horse to {toUnit}.",
    },
    areas: {
      title: "Facilities",
      addArea: "Add Facility",
      editArea: "Edit Facility",
      name: "Facility Name",
      namePlaceholder: "Enter facility name",
      nameAr: "Arabic Name",
      nameArPlaceholder: "Enter Arabic name (optional)",
      code: "Code",
      codePlaceholder: "e.g. A1, BARN-1",
      branch: "Branch",
      selectBranch: "Select branch",
      noAreas: "No facilities found",
      addFirst: "Add your first facility",
      created: "Facility created successfully",
      updated: "Facility updated successfully",
      activated: "Facility activated",
      deactivated: "Facility deactivated",
      active: "active facilities",
    },
    units: {
      title: "Housing Units",
      addUnit: "Add Unit",
      editUnit: "Edit Unit",
      name: "Unit Name",
      namePlaceholder: "Enter unit name",
      nameAr: "Arabic Name",
      nameArPlaceholder: "Enter Arabic name (optional)",
      code: "Code",
      codePlaceholder: "e.g. S-01, P-05",
      unitType: "Unit Type",
      selectType: "Select unit type",
      area: "Section",
      selectArea: "Select section",
      capacity: "Capacity",
      occupancy: "Occupancy Mode",
      selectOccupancy: "Select occupancy mode",
      notes: "Notes",
      notesPlaceholder: "Additional notes...",
      noUnits: "No units found",
      addFirst: "Add your first housing unit",
      created: "Unit created successfully",
      updated: "Unit updated successfully",
      activated: "Unit activated",
      deactivated: "Unit deactivated",
      viewDetails: "View Details",
      active: "active units",
      editRoom: "Edit room",
      roomCode: "Room Code",
      roomName: "Room Name",
      roomFunction: "Room Function",
      renameOccupiedTitle: "Room is currently occupied",
      renameOccupiedDesc: "Room {unit} currently houses {count} horse(s). Changing its code or name will affect how this room is identified. Are you sure?",
      confirmRename: "Rename anyway",
      reclassifyOccupiedTitle: "Change function of occupied room",
      reclassifyOccupiedDesc: "Room {unit} currently has {count} horse(s). Changing the room function to \"{type}\" may affect operations. Are you sure?",
      reclassifyBlockedStorage: "Cannot convert to storage while occupied. Remove the current occupant(s) first.",
      confirmReclassify: "Change function",
      sectionRoomInfo: "Room Information",
      sectionCurrentOccupant: "Current Occupant",
      sectionRoomHistory: "Room History",
      viewBoardingDetails: "View Boarding Details",
      noHistoryYet: "No previous occupants",
      stayDuration: "{from} — {to}",
      roomStatusActions: "Room Status",
      types: {
        stall: "Stall",
        paddock: "Paddock",
        room: "Room",
        cage: "Cage",
        pasture: "Pasture",
        barn: "Barn",
        isolation: "Isolation",
        foaling: "Foaling Stall",
        other: "Other",
      },
      occupancyModes: {
        single: "Single",
        shared: "Shared",
        group: "Group",
      },
      status: {
        vacant: "Vacant",
        occupied: "Occupied",
        full: "Full",
        maintenance: "Maintenance",
        outOfService: "Out of Service",
        available: "Available",
      },
      setMaintenance: "Set as Maintenance",
      setOutOfService: "Set as Out of Service",
      setAvailable: "Set as Available",
      maintenanceWarning: "This unit will be marked as under maintenance. No horses can be assigned until it is set back to available.",
      outOfServiceWarning: "This unit will be marked as out of service. No horses can be assigned until it is reactivated.",
      unitUnavailable: "This unit is currently unavailable for assignment",
      currentUnit: "Current unit",
      differentBranch: "Different branch",
      sameBranch: "Same branch",
      searchPlaceholder: "Search units...",
      filterByType: "Filter by type",
      filterByStatus: "Filter by status",
      allTypes: "All types",
      allStatuses: "All statuses",
      isolation: "Isolation",
      isolationLabel: "Isolation",
      storageLabel: "Storage",
    },
    search: {
      placeholder: "Search horse or unit…",
      placeholderV2: "Search horse, unit or facility",
      noResults: "No matches found",
    },
    scope: {
      showingAllBranches: "Showing facilities from: All branches",
      showingBranch: "Showing facilities from: {branch}",
      summary: "{branches} branches · {facilities} facilities",
    },
    filter: {
      all: "All",
      vacant: "Vacant",
      occupied: "Occupied",
      full: "Full",
      storage: "Storage",
      isolation: "Isolation",
      maintenance: "Maintenance",
      outOfService: "Out of Service",
      helpers: {
        vacant: "Available and contains no horses",
        occupied: "Contains horses but has not reached capacity",
        full: "Reached capacity",
        storage: "Storage room or unit",
        isolation: "Isolation unit",
        maintenance: "Currently under maintenance",
        outOfService: "Out of service — not assignable",
      },
    },
    lifecycle: {
      activeBranches: "Active Branches",
      deactivatedBranches: "Deactivated Branches",
      archivedBranches: "Archived Branches",
      allBranches: "All Branches",
      activeFacilities: "Active Facilities",
      deactivatedFacilities: "Deactivated Facilities",
      archivedFacilities: "Archived Facilities",
      allFacilities: "All Facilities",
      activeUnits: "Active Units",
      deactivatedUnits: "Deactivated Units",
      archivedUnits: "Archived Units",
      allUnits: "All Units",
      deactivate: "Deactivate",
      deactivateTitle: "Deactivate {entity}",
      deactivateMsg: "This {entity} will be temporarily suspended from operations. You can reactivate it later.",
      reactivate: "Reactivate",
      archive: "Archive",
      archiveTitle: "Archive {entity}",
      archiveMsg: "This {entity} will be retired from active views but all historical data will be preserved. You can restore it later if needed.",
      archiveInstead: "Archive Instead",
      restore: "Restore from Archive",
      deletePermanently: "Delete Permanently",
      deleteTitle: "Delete {entity}",
      deleteConfirmMsg: "Are you sure you want to permanently delete this {entity}? This action cannot be undone.",
      cannotDeleteMsg: "This {entity} cannot be permanently deleted because it has operational history:",
      suggestArchive: "Use Archive instead to retire this from active views while preserving all historical data.",
      suggestDeactivate: "Use Deactivate for temporary suspension — you can reactivate later.",
      archivedBadge: "Archived",
      deactivatedBadge: "Deactivated",
      entity: {
        branch: "Branch",
        facility: "Facility",
        unit: "Unit",
      },
      blockers: {
        hasHorses: "Contains {n} horse(s)",
        hasFacilities: "Contains {n} facility(ies)",
        hasAdmissions: "Has active boarding admissions",
        hasUnits: "Contains {n} unit(s)",
        hasOccupants: "Has current occupants",
        hasHistory: "Has occupancy or admission history",
        hasInvoices: "Has linked invoices",
      },
      noArchived: "No archived items in this view",
      noDeactivated: "No deactivated items in this view",
    },
    vacancy: {
      vacant: "vacant",
      available: "available",
    },
    stats: {
      title: "Housing Overview",
      total: "Total Units",
      vacant: "Vacant",
      occupied: "Occupied",
      full: "Full",
      horses: "Horses Housed",
    },
    settings: {
      title: "Housing Settings",
      description: "Configure housing module and manage demo data",
    },
    unitTypes: {
      stall: "Stall",
      paddock: "Paddock",
      room: "Room",
      cage: "Cage",
      pasture: "Pasture",
      barn: "Barn",
      isolation: "Isolation",
      foaling: "Foaling Stall",
      other: "Other",
    },
    occupancyModes: {
      single: "Single",
      shared: "Shared",
      group: "Group",
    },
    occupants: {
      title: "Current Occupants",
      assignHorse: "Assign Horse",
      admitHorse: "Admit Horse",
      admitFirst: "Admit a horse to this unit",
      selectHorse: "Select Horse",
      noOccupants: "No horses in this unit",
      assignFirst: "Assign a horse to this unit",
      vacate: "Vacate",
      vacateTooltip: "Remove horse from this unit",
      assigned: "Horse assigned successfully",
      vacated: "Horse vacated successfully",
      moved: "Horse moved successfully",
      orphanRemoved: "Orphan placement removed",
      since: "Since",
      full: "Full",
      available: "Available",
      alreadyOccupied: "This unit is already occupied",
      unitFull: "This unit is at full capacity",
      activeAdmission: "Active Admission",
      noAdmission: "No Admission",
      noUnit: "no unit",
      crossBranchTitle: "Cross-Branch Admission",
      crossBranchDesc: "{horse} has an active admission at {branch}. Use Arrivals & Departures to transfer the horse first.",
      removeOrphan: "Remove Placement",
      removeOrphanTitle: "Remove Orphan Placement",
      removeOrphanDesc: "Remove {horse} from {unit}? This is a repair action for a placement that has no backing admission.",
      createAdmission: "Create Admission",
    },
    details: {
      title: "Unit Details",
      info: "Information",
      occupants: "Occupants",
      area: "Section",
      capacity: "Capacity",
      status: "Status",
    },
    demo: {
      title: "Demo Mode",
      description: "Load sample sections, units, and assignments for testing. Demo data is tagged and can be safely removed.",
      load: "Load Demo Data",
      remove: "Remove Demo Data",
      loaded: "Demo data loaded successfully",
      removed: "Demo data removed",
      alreadyLoaded: "Demo data already exists",
      needsHorses: "Please create at least 2 horses first, then try again.",
    },
    toasts: {
      noActiveOrganization: "No active organization",
      areaCreated: "Area created successfully",
      areaUpdated: "Area updated successfully",
      unitCreated: "Unit created successfully",
      unitUpdated: "Unit updated successfully",
      horseAssigned: "Horse assigned successfully",
      horseVacated: "Horse vacated successfully",
      failedToCreate: "Failed to create",
      failedToUpdate: "Failed to update",
    },
    quickCreate: {
      desc: "Quickly register a horse for housing. You can complete the full profile later in the Horse Registry.",
      completeHint: "This creates a draft record. You can add microchip, passport, pedigree, and other details later from the Horse Registry.",
      addNewHorse: "+ Add New Horse",
      noHorsesYet: "No horses registered yet",
      noHorsesDesc: "Create a horse to get started with housing.",
      addNewClient: "+ Add New Client",
      noClientsYet: "No clients registered yet",
      noClientsDesc: "Create a client to assign to this admission.",
      addNewPackage: "+ Add New Package",
      noPackagesYet: "No packages available yet",
      noPackagesDesc: "Create a package to define standard rates and services.",
      packageDesc: "Quickly create a package for this admission. You can add included services later from the Services page.",
      missing: {
        name: "Enter package name",
        planType: "Select a package type",
        billingCycle: "Select a billing cycle",
        price: "Enter a valid base price",
      },
    },
    emergency: {
      addContact: "Add emergency contact",
      removeContact: "Remove contact",
      contactHeading: "Contact #{{n}}",
      emptyState: "No emergency contacts yet. Add at least one trusted person to reach in case of emergencies.",
      nameEn: "Name (English)",
      nameAr: "Name (Arabic)",
      nameEnPlaceholder: "Full name in English",
      nameArPlaceholder: "الاسم الكامل بالعربية",
      relationship: "Relationship",
      relationshipPlaceholder: "Owner, family, trainer…",
      moreSuffix: "+{{n}} more",
      contact: {
        phones: "Phones",
        addPhone: "Add phone",
        noPhonesAdded: "No phones added",
        whatsapp: "WhatsApp",
        primaryPhone: "Primary",
        setAsPrimary: "Set as primary",
      },
      phoneLabels: {
        mobile: "Mobile",
        work: "Work",
        home: "Home",
        other: "Other",
      },
      validation: {
        nameRequired: "Contact name is required",
        namesRequiredAr: "Arabic and English names are required",
        phoneRequired: "Add at least one phone number per contact",
      },
    },
    admissions: {

      title: "Boarding Admissions",
      newAdmission: "New Admission",
      searchPlaceholder: "Search by horse or client...",
      empty: "No admissions found",
      createFirst: "Create First Admission",
      status: {
        active: "Active",
        checkoutPending: "Pending Checkout",
        checkedOut: "Checked Out",
        checkout_pending: "Checkout pending",
        checked_out: "Checked out",
        draft: "Draft",
        cancelled: "Cancelled",
      },
      subFilters: {
        noInvoice: "Billable, No Invoice",
        outstanding: "Outstanding",
        needsPlacement: "Needs Placement",
        needsPrice: "Needs Price",
        accruedUnbilled: "Accrued, Unbilled",
        settled: "Settled",
      },
      filterPanel: {
        secondaryAccrued: "This is not an issued invoice. It shows boarding that can be invoiced.",
        countLabel: "Showing",
        descriptions: {
          all: "All admissions across every lifecycle state.",
          active: "Admissions currently active in the stable.",
          needs_placement: "Active admissions that have not been assigned to a housing unit.",
          checkout_pending: "Admissions awaiting checkout completion.",
          checked_out: "Admissions that have already been checked out.",
          draft: "Admissions saved as drafts and not yet activated.",
          needs_price: "Admissions that do not have a direct price or a usable boarding-plan price.",
          accrued_unbilled: "Admissions with a known boarding price, but no boarding invoice has been linked yet.",
          outstanding: "Admissions with linked boarding invoices that still have an unpaid balance.",
          settled: "Admissions with linked boarding invoices that are fully paid or cleared.",
        },
        titles: {
          all: "All",
          active: "Active",
          needs_placement: "Needs Placement",
          checkout_pending: "Pending Checkout",
          checked_out: "Checked Out",
          draft: "Draft",
          needs_price: "Needs Price",
          accrued_unbilled: "Accrued, Unbilled",
          outstanding: "Outstanding",
          settled: "Settled",
        },
      },
      price: {
        fromPlan: "from plan",
        notSet: "Price not set",
      },
      assignUnit: "Assign Unit",
      table: {
        horse: "Horse",
        owner: "Owner",
        client: "Client",
        branch: "Branch",
        unit: "Unit",
        status: "Status",
        admitted: "Admitted",
        stay: "Stay",
        rate: "Rate",
      },
      list: {
        since: "Since",
        facility: "Facility",
        stayDuration: "Stay",
        daysUnit: "d",
        billingStatus: "Billing",
        noBilling: "No rate set",
      },
      checks: {
        horseRequired: "Horse is required",
        branchRequired: "Branch is required",
        noClient: "No client/payer assigned",
        noUnit: "No housing unit assigned",
        noEmergency: "No emergency contact",
        noRate: "No rate configured",
        noTeam: "No staff assigned to this horse",
      },
      financial: {
        summary: "Financial Summary",
        accrued: "Accrued Value",
        invoiced: "Invoiced",
        unbilled: "Unbilled",
      },
      invoiceStatuses: {
        draft: "Draft",
        reviewed: "Reviewed",
        approved: "Approved",
        shared: "Shared",
        paid: "Paid",
        partial: "Partial",
        overdue: "Overdue",
        cancelled: "Cancelled",
        issued: "Issued",
      },
      detail: {
        title: "Admission Detail",
        owner: "Owner",
        client: "Client",
        branch: "Branch",
        area: "Section",
        facility: "Facility",
        unit: "Unit",
        horseName: "Horse",
        admittedAt: "Admitted",
        expectedDeparture: "Expected Departure",
        checkedOutAt: "Checked Out",
        rate: "Rate",
        instructions: "Instructions",
        emergencyContact: "Emergency Contact",
        emergencyContactHelp: "For emergencies only. Operational staff are assigned separately.",
        assignedTeam: "Assigned Team",
        assignedTeamEmpty: "No staff assigned",
        assignStaff: "Assign Staff",
        manageTeam: "Manage",
        statusHistory: "Status History",
        by: "by",
        days: "days",
        assignUnit: "Assign Unit",
        moveUnit: "Move Unit",
        addContact: "Add Contact",
        notAssigned: "Not assigned",
        addDeparture: "Set Departure",
        setRate: "Set Rate",
        assignClient: "Assign Client",
        assignClientDialogTitle: "Assign Client for {{name}}",
        searchClientsPlaceholder: "Search clients by name, phone, email…",
        noClientsYet: "No clients available. Create one in the Clients section first.",
        setPriceDialogTitle: "Set Boarding Price for {{name}}",
        setPriceHelp: "Set daily and/or monthly rate. Both can be used together.",
        emergencyContactDialogTitle: "Emergency Contacts for {{name}}",
        addNewClient: "Add new client",

        movements: "Related Movements",
        checkin: "Check-in",
        checkout: "Checkout",
      },
      warnings: {
        noClient: "No client assigned",
        noUnit: "No housing unit assigned",
      },
      reasons: {
        boarding: "Boarding",
        training: "Training",
        medical: "Medical",
        breeding: "Breeding",
        temporary: "Temporary",
      },
      wizard: {
        title: "New Admission",
        stepHorse: "Horse",
        stepClient: "Client",
        stepHousing: "Housing",
        stepRates: "Rates",
        stepDetails: "Details",
        stepReview: "Review",
        overrideHint: "You can override below",
        housingPrefilled: "Housing location has been prefilled from the selected unit.",
        selectHorse: "Select Horse",
        arrivalDate: "Arrival Date",
        arrivalDateHint: "Leave empty for today. Set a past date for historical admissions.",
        selectClient: "Client / Payer (optional)",
        noClient: "No client (assign later)",
        branch: "Branch",
        selectBranch: "Select branch",
        area: "Section (optional)",
        selectArea: "Select section",
        unit: "Unit / Stall (optional)",
        selectUnit: "Select unit",
        unitFull: "Full",
        unitFullToast: "This unit is already full. Please choose another unit.",
        ineligibleActiveAdmission: "Active admission",
        ineligibleAlreadyHoused: "Already housed",
        ineligibleHorseStatus: "Not eligible",
        horseIneligibleToast: "A new admission cannot be created for this horse because it already has an active admission or is already housed.",
        ineligibleDepartedFromTenant: "Departed from this stable",
        ineligibleTransferredAway: "Transferred away",
        ineligibleHistoricalOnly: "Historical only",
        horseDepartedToast: "A new admission cannot be created for this horse here because it has departed from this stable. It must return through a new incoming or transfer movement first.",
        noEligibleHorses: "No horses are currently available for a new admission.",
        dailyRate: "Daily Rate",
        monthlyRate: "Monthly Rate",
        billingCycle: "Billing Cycle",
        cycleDaily: "Daily",
        cycleWeekly: "Weekly",
        cycleMonthly: "Monthly",
        cycleYearly: "Yearly",
        cycleOnetime: "One-time",
        currency: "Currency",
        expectedDeparture: "Expected Departure",
        reason: "Reason",
        selectReason: "Select reason",
        emergencyContact: "Emergency Contact",
        emergencyContactPlaceholder: "Phone number or contact info",
        specialInstructions: "Special Instructions",
        specialInstructionsPlaceholder: "Any special care requirements...",
        confirmAdmission: "Confirm Admission",
      },
      completePricing: {
        cta: "Complete pricing",
        title: "Complete pricing",
        description: "Set the client, package, and rate to complete the commercial context for this admission. This does not create a contract or an invoice.",
        bannerMessage: "Pricing has not been set for this admission.",
        client: "Client / payer",
        clientOptionalHelp: "Optional. You can complete pricing now and assign a client later.",
        addClient: "Add client",
        package: "Package",
        noPackageManual: "No package / manual rates",
        manualRates: "Rates",
        dailyRate: "Daily rate",
        monthlyRate: "Monthly rate",
        currency: "Currency",
        billingCycle: "Billing cycle",
        rateRequired: "Enter at least a daily or monthly rate.",
      },
      billing: {
        createInvoiceTitle: "Generate Boarding Invoice",
        createInvoice: "Generate Invoice",
        boardingInvoice: "Boarding",
        invoiceCreated: "Invoice created successfully",
        invoiceFailed: "Failed to create invoice",
        invoiceItemFailed: "Failed to create invoice line item",
        estimatedHint: "Estimated: {{days}} days at {{rate}}",
        linkedInvoices: "Linked Invoices",
        noLinkedInvoices: "No invoices linked yet",
        generateInvoice: "Generate Invoice",
        viewInvoices: "View Invoices",
        paid: "Paid",
        billed: "Billed",
        notBilled: "Not Billed",
        totalBilled: "Total Billed",
        billedPeriods: "Already Billed Periods",
        remainingBillable: "Remaining Billable (pre-tax)",
        fullyBilled: "This admission has been fully billed. No remaining amount to invoice.",
        overlapWarning: "Selected period overlaps with an already-billed period",
        overlapBlock: "Cannot create invoice — selected period overlaps with an already-billed period",
        fullMonth: "full month",
        segmentsBreakdown: "Billing breakdown",
        noPriceGuard: "Set pricing before creating an invoice. This admission has no rate.",
        noPriceGuardAction: "Complete pricing",
      },
      checkout: {
        initiateTitle: "Initiate Checkout",
        initiateDesc: "This will move the admission to pending checkout for review.",
        confirmTitle: "Confirm Checkout",
        confirmDesc: "This will check out the horse and release the housing unit.",
        initiateCheckout: "Initiate Checkout",
        confirmCheckout: "Confirm Checkout",
        willBeReleased: "will be released",
        notes: "Checkout Notes",
        notesPlaceholder: "Any notes about the checkout...",
        financialReviewPlaceholder: "Financial review will be enhanced in a future update",
        financialBlock: "Checkout blocked due to outstanding balance",
        contactManager: "Please contact a manager or owner to override this block.",
        overrideRequired: "Outstanding balance requires manager override",
        outstandingAmount: "Outstanding amount",
        confirmOverride: "Override and Proceed",
        overrideAccepted: "Financial override confirmed",
        unbilledSuggestion: "There is unbilled boarding value. Consider generating an invoice before checkout.",
        unbilledAmount: "Unbilled amount",
      },
    },
    careNotes: {
      title: "Care Notes",
      empty: "No care notes yet",
      addNote: "Add Note",
      editNote: "Edit Note",
      priority: "Priority",
      titleLabel: "Title",
      titlePlaceholder: "Brief title...",
      bodyLabel: "Note",
      bodyPlaceholder: "Enter care note details...",
      types: {
        feed_plan: "Feed Plan",
        medication: "Medication",
        care_note: "Care Note",
        restriction: "Restriction",
        training: "Training",
        vet_note: "Vet Note",
        general: "General",
      },
      priorities: {
        low: "Low",
        normal: "Normal",
        high: "High",
        urgent: "Urgent",
      },
    },
    checkout: {
      financial: {
        admissionScope: "Admission Billing",
        clientScope: "Client Account",
        billed: "Billed",
        paid: "Paid",
        balance: "Balance",
        deposit: "Deposit",
        outstanding: "Outstanding",
        creditLimit: "Credit Limit",
        available: "Available",
        creditExceeded: "Credit limit exceeded — override required",
        accrued: "Accrued",
        unbilledWarning: "Unbilled boarding value",
      },
    },
    dashboard: {
      activeBoarders: "Active Boarders",
      upcomingDepartures: "Upcoming Departures",
      pendingCheckouts: "Pending Checkouts",
    },
    horseProfile: {
      activeAdmission: "Active Admission",
      viewDetails: "View Details",
      activeCareNotes: "active care notes",
    },
  },
  vetVisits: {
    title: "Vet Visits",
    newVisit: "New Visit",
    scheduleVisit: "Schedule Visit",
    horsesCount: "{{count}} horse(s)",
    horsesSelected: "{{count}} horse(s) selected",
    noHorsesAvailable: "No horses available",
    estimated: "Est:",
    actual: "Actual:",
    types: {
      routine: "Routine Checkup",
      emergency: "Emergency",
      follow_up: "Follow-up",
      inspection: "Inspection",
    },
    status: {
      scheduled: "Scheduled",
      confirmed: "Confirmed",
      in_progress: "In Progress",
      completed: "Completed",
      cancelled: "Cancelled",
      no_show: "No Show",
    },
    fields: {
      title: "Visit Title",
      visitType: "Visit Type",
      scheduledDate: "Scheduled Date",
      time: "Time",
      vetName: "Veterinarian Name",
      vetPhone: "Veterinarian Phone",
      horses: "Horses to Examine",
      notes: "Notes",
      findings: "Findings",
      recommendations: "Recommendations",
      estimatedCost: "Estimated Cost",
      actualCost: "Actual Cost",
    },
    placeholders: {
      title: "e.g., Annual checkup, Emergency call...",
      pickDate: "Pick a date",
      vetName: "Dr. Ahmed...",
      phone: "+966...",
      notes: "Additional notes...",
    },
    actions: {
      confirm: "Confirm Visit",
      start: "Start Visit",
      complete: "Complete Visit",
      cancel: "Cancel Visit",
      reschedule: "Reschedule",
    },
    messages: {
      created: "Visit scheduled successfully",
      updated: "Visit updated",
      confirmed: "Visit confirmed",
      completed: "Visit completed",
      cancelled: "Visit cancelled",
      deleted: "Visit deleted",
    },
  },
  vet: {
    title: "Vet & Health",
    subtitle: "Manage treatments, vaccinations, and follow-ups",
    newTreatment: "New Treatment",
    editTreatment: "Edit Treatment",
    createTreatment: "Create Treatment",
    updateTreatment: "Update Treatment",
    treatmentDetails: "Treatment Details",
    vaccinationPrograms: "Vaccination Programs",
    noRecords: "No health records yet",
    addFirstTreatment: "Add First Treatment",
    recentTreatments: "Recent Treatments",
    upcomingVaccinations: "Upcoming Vaccinations",
    openFollowups: "Open Follow-ups",
    noTreatments: "No Treatments",
    scheduleVaccination: "Schedule Vaccination",
    filterByHorse: "Filter by horse",
    allHorses: "All Horses",
    dueLabel: "Due",
    tabs: {
      treatments: "Treatments",
      vaccinations: "Vaccinations",
      visits: "Visits",
      followups: "Follow-ups",
      programs: "Vaccination Programs",
    },
    emptyMessages: {
      treatments: "No treatments yet. Create your first treatment to get started.",
      horseTreatments: "No treatment records for this horse.",
      vaccinations: "No vaccinations scheduled",
      visits: "No visits scheduled. Schedule a vet visit to get started.",
      followups: "No follow-ups scheduled",
    },
    form: {
      horse: "Horse",
      category: "Category",
      title: "Title",
      titlePlaceholder: "e.g., Annual vaccination, Dental checkup",
      description: "Description",
      descriptionPlaceholder: "Brief description of the treatment",
      priority: "Priority",
      serviceMode: "Service Mode",
      provider: "Veterinarian / Clinic",
      providerPlaceholder: "Select provider (optional)",
      providerNamePlaceholder: "Or enter provider name",
      scheduledFor: "Scheduled For",
      selectDate: "Select date",
      selectHorse: "Select horse",
      selectCategory: "Select category",
      selectPriority: "Select priority",
      selectMode: "Select mode",
      notes: "Notes",
      notesPlaceholder: "Additional notes...",
    },
    programs: {
      addVaccine: "Add Vaccine",
      editProgram: "Edit Vaccination Program",
      newProgram: "New Vaccination Program",
      name: "English Name",
      nameAr: "Arabic Name",
      interval: "Interval (days)",
      intervalPlaceholder: "e.g., 365",
      minAge: "Min Age (days)",
      minAgePlaceholder: "e.g., 90",
      repeatEvery: "Repeat every {days} days",
      minAgeLabel: "Min age: {days} days",
      deleteConfirm: "Are you sure you want to delete this vaccination program?",
    },
    vaccination: {
      vaccine: "Vaccine",
      dueDate: "Due Date",
      selectProgram: "Select vaccine program",
      horse: "Horse",
      serviceMode: "Service Mode",
      notes: "Notes",
    },
    detail: {
      requestedAt: "Requested",
      scheduledFor: "Scheduled",
      completedAt: "Completed",
      assignedTo: "Assigned To",
      provider: "Provider",
      serviceMode: "Service Mode",
      medications: "Medications",
      noMedications: "No medications recorded",
      followups: "Follow-ups",
      noFollowups: "No follow-ups scheduled",
      addMedication: "Add Medication",
      medicationName: "Medication Name",
      dose: "Dose",
      frequency: "Frequency",
      duration: "Duration (days)",
      startDate: "Start Date",
      endDate: "End Date",
    },
    followupType: {
      followup: "Follow-up",
      recheck: "Recheck",
      medication_refill: "Medication Refill",
      wound_check: "Wound Check",
      suture_removal: "Suture Removal",
      lab_result: "Lab Result",
      blood_test: "Blood Test",
    },
    timeLabels: {
      overdue: "Overdue",
      dueToday: "Due Today",
      tomorrow: "Tomorrow",
      due: "Due",
      administered: "Administered",
    },
    billing: {
      createInvoiceTitle: "Create Invoice from Treatment",
      createVaccinationInvoiceTitle: "Create Invoice from Vaccination",
      service: "Service",
      selectService: "Select service",
      client: "Client",
      selectClient: "Select client",
      searchClients: "Search clients...",
      noClientsFound: "No clients found",
      clientNameManual: "Client Name (manual)",
      totalAmount: "Total Amount",
      createInvoice: "Create Invoice",
      invoiceCreated: "Invoice created successfully",
      invoiceFailed: "Failed to create invoice",
      generateInvoice: "Generate Invoice",
      invoiced: "Invoiced",
      notInvoiced: "Not Invoiced",
      linkedInvoice: "Linked Invoice",
      serviceIncludedInPlan: "This service is included in the horse's boarding plan",
      includedInPlanTitle: "Service Included in Plan",
      includedInPlanDescription: "This service is covered by the «{{planName}}» plan. Amount has been set to zero automatically. You may override if you want to charge.",
      includedOverrideWarning: "⚠ Amount manually overridden despite service being included in plan",
      zeroChargeIncluded: "Invoice will be created as a zero-charge record (included service)",
      createZeroInvoice: "Create Zero-Charge Invoice",
      providerCostRef: "Provider Cost",
      recordStableCost: "Record as Stable Cost",
      stableCostRecorded: "Internal cost recorded successfully",
      markupHelper: {
        title: "Provider Cost Comparison",
        providerCost: "Provider Cost",
        billedAmount: "Billed to Client",
        exact: "Exact",
        passThrough: "Pass-through",
        withMarkup: "With Markup",
        belowCost: "Below Cost",
        applyPassThrough: "Apply Exact Pass-Through",
      },
    },
    domain: {
      vet: "Veterinary",
    },
  },
  organizationSettings: {
    title: "Organization Settings",
    modules: "Modules",
    modulesDescription: "Enable or disable features for your organization",
    labMode: "Laboratory Mode",
    labModeDescription: "Configure how you use lab services",
    labModes: {
      none: "None - I don't need lab services",
      requests: "Requests Only - Request tests from external labs",
      full: "Full - I have an internal lab and manage samples/results",
    },
    vet: {
      title: "Veterinary & Health",
      description: "Track treatments, vaccinations, and health records",
    },
    housing: {
      title: "Housing",
      description: "Manage units, stalls, and sections",
    },
    movement: {
      title: "Movement",
      description: "Track horse movements between locations",
    },
    breeding: {
      title: "Breeding",
      description: "Manage breeding programs and pregnancies",
    },
    saved: "Settings saved successfully",
    ownerOnly: "Only organization owners can change these settings",
    taxPricing: {
      title: "Currency, Tax & Pricing",
      description: "Organization-level currency, tax, and pricing defaults",
      currency: "Currency",
      currencyHint: "The currency used for invoices and financial displays",
      taxRate: "Default Tax Rate (%)",
      taxRateHint: "This rate applies to new invoices unless overridden",
      pricingMode: "How are your service prices set?",
      pricingModeHint: "Determines how entered prices are interpreted when tax is calculated",
      exclusive: "Prices are before tax (tax added on top)",
      inclusive: "Prices already include tax",
      save: "Save Settings",
      saving: "Saving...",
      savedSuccess: "Settings saved successfully",
      savedError: "Failed to save settings",
    },
  },
  schedule: {
    title: "Schedule",
    subtitle: "Upcoming events and appointments",
    filters: {
      module: "Module",
      dateRange: "Date Range",
      search: "Search events",
    },
    views: {
      list: "List",
      calendar: "Calendar",
    },
    empty: "No scheduled events found",
    emptyDesc: "Events will appear here when you schedule visits, vaccinations, or breeding activities",
    today: "Today",
    upcoming: "Upcoming",
    searchPlaceholder: "Search events...",
    modules: {
      vet: "Veterinary",
      vet_followups: "Vet Followups",
      vetFollowups: "Vet Followups",
      vaccinations: "Vaccinations",
      breeding: "Breeding",
      movement: "Movement",
      academy: "Academy",
      orders: "Orders",
      lab: "Laboratory",
      laboratory: "Laboratory",
    },
    loadMore: "Load more",
    calendar: {
      monthView: "Month",
      weekView: "Week",
      dayView: "Day",
      today: "Today",
      reschedule: "Reschedule",
      confirmReschedule: "Reschedule Event",
      dragToReschedule: "Drag to reschedule",
    },
  },
  records: {
    title: "Records",
    subtitle: "Activity log and documents",
    tabs: {
      activity: "Activity",
      documents: "Documents",
    },
    filters: {
      module: "Module",
      eventType: "Event Type",
      dateRange: "Date Range",
      search: "Search activity",
    },
    empty: "No activity records found",
    goToFiles: "Go to File Manager",
    loadMore: "Load more",
    events: {
      created: "Created",
      updated: "Updated",
      statusChanged: "Status Changed",
      deleted: "Deleted",
    },
    eventTypes: {
      created: "Created",
      updated: "Updated",
      status_changed: "Status Changed",
      deleted: "Deleted",
    },
  },
  files: {
    title: "File Manager",
    subtitle: "Manage all media assets",
    searchPlaceholder: "Search files...",
    stats: {
      totalFiles: "Total Files",
      totalSize: "Total Size",
      images: "Images",
      videos: "Videos",
    },
    filters: {
      entityType: "Entity Type",
      visibility: "Visibility",
      mimeType: "File Type",
      sort: "Sort By",
    },
    sort: {
      newest: "Newest First",
      oldest: "Oldest First",
      largest: "Largest First",
      smallest: "Smallest First",
    },
    views: {
      grid: "Grid",
      list: "List",
    },
    actions: {
      view: "View",
      delete: "Delete",
      changeVisibility: "Change Visibility",
      download: "Download",
      selectAll: "Select All",
      deleteSelected: "Delete Selected",
    },
    visibility: {
      private: "Private",
      tenant: "Organization",
      shared_link: "Shared Link",
      sharedLink: "Shared Link",
      customerSpecific: "Client Only",
    },
    entityTypes: {
      horse: "Horse",
      invoice: "Invoice",
      expense: "Expense",
      labResult: "Lab Result",
    },
    empty: "No files found",
    emptyDesc: "Upload files from horse profiles, invoices, or other modules",
    viewComingSoon: "File preview coming soon",
    confirmDelete: "Are you sure you want to delete this file?",
    confirmBulkDelete: "Are you sure you want to delete the selected files?",
    selected: "selected",
    uploadedBy: "Uploaded by",
  },
  finance: {
    title: "Finance",
    subtitle: "Invoices, expenses, and ledger",
    tax: {
      exclTax: "excl. tax",
      inclTax: "incl. tax",
      estimated: "estimated",
      exclusive: "excl. tax",
      inclusive: "incl. tax",
      label: "Tax",
    },
    overview: "Overview",
    customerBalances: {
      title: "Customer Balances",
      description: "Monitor ledger-based client balances and credit",
      balance: "Balance",
      available: "Available",
      owes: "Owes",
      credit: "Credit",
      settled: "Settled",
    },
    tabs: {
      invoices: "Invoices",
      expenses: "Expenses",
      payables: "Payables",
      internalCosts: "Stable Costs",
      ledger: "Ledger",
      payments: "Payments",
    },
    internalCosts: {
      title: "Internal Stable Costs",
      noRecords: "No internal cost records yet",
      searchPlaceholder: "Search costs...",
      source: "Source",
      serviceMode: "Service Mode",
      cost: "Cost",
      modeInternal: "Internal",
      modeExternal: "External Provider",
      stats: {
        totalRecords: "Total Records",
        totalCost: "Total Cost",
        categories: "Categories",
      },
      sources: {
        vetTreatment: "Vet Treatment",
        vaccination: "Vaccination",
        breeding_attempt: "Breeding Attempt",
        foaling: "Foaling",
      },
    },
    traceability: {
      financialStatus: "Financial Status",
      invoiced: "Invoiced",
      stableCostRecorded: "Stable Cost Recorded",
      payableCreated: "Supplier Payable Created",
      zeroCharge: "Zero-Charge",
      clientInvoiced: "Client Invoiced",
      notInvoiced: "Not Invoiced",
      viewSource: "View Source",
      horseName: "Horse",
      sourceLabel: {
        vet_treatment: "Vet Treatment",
        vaccination: "Vaccination",
        breeding_attempt: "Breeding Attempt",
        foaling: "Foaling",
        boarding: "Boarding",
      },
    },
    payables: {
      title: "Supplier Payables",
      create: "New Payable",
      createDesc: "Record an incoming bill from a supplier or service provider",
      noPayables: "No payables recorded yet",
      supplierName: "Supplier Name",
      supplierPlaceholder: "Enter supplier name",
      amount: "Amount",
      paid: "Paid",
      remaining: "Remaining",
      dueDate: "Due Date",
      sourceType: "Source",
      reference: "Reference",
      referencePlaceholder: "Invoice/reference number",
      paymentAmount: "Payment Amount",
      approve: "Approve",
      recordPayment: "Record Payment",
      searchPlaceholder: "Search payables...",
      statuses: {
        received: "Received",
        reviewed: "Reviewed",
        approved: "Approved",
        partiallyPaid: "Partially Paid",
        paid: "Paid",
        cancelled: "Cancelled",
        disputed: "Disputed",
      },
      sources: {
        lab_invoice: "Laboratory",
        clinic: "Clinic",
        transport: "Transport",
        feed: "Feed & Supplies",
        other: "Other",
      },
      stats: {
        pending: "Pending",
        outstanding: "Outstanding",
        totalPaid: "Total Paid",
        settled: "Settled",
      },
      toasts: {
        created: "Payable recorded successfully",
        paymentRecorded: "Payment recorded successfully",
        deleted: "Payable deleted",
      },
    },
    pos: {
      title: "Point of Sale",
      subtitle: "Manage sales and transactions",
      walkIn: "Walk-in Customer",
      priceMissing: "No price",
      customer: "Customer",
      quickCheckout: "Quick Checkout",
      comingSoon: "Coming Soon",
      comingSoonDesc: "Point of Sale features are being developed and will be available soon.",
      plannedFeatures: "Planned POS Features",
      checkout: {
        success: "Checkout complete",
        error: "Checkout failed",
        missingPrices: "Some items have no price",
      },
      session: {
        openTitle: "Open POS Session",
        closeTitle: "Close POS Session",
        open: "Open Session",
        close: "Close Session",
        closed: "Closed",
        openingCash: "Opening Cash",
        expectedCash: "Expected Cash",
        actualCash: "Actual Cash Count",
        variance: "Variance",
        varianceNote: "Explain variance...",
        openFirst: "Please open a session first",
        openToSell: "Open a session to start selling",
      },
      cart: {
        title: "Cart",
        empty: "Cart is empty",
        subtotal: "Subtotal",
        discount: "Discount",
        tax: "Tax",
        total: "Total",
        view: "View Cart",
      },
      payment: {
        method: "Payment Method",
        cash: "Cash",
        card: "Card",
        transfer: "Transfer",
        debt: "Debt",
        credit: "Credit",
      },
      actions: {
        addMore: "Add More",
        pay: "Pay",
        completeSale: "Complete Sale",
        print: "Print",
        viewInvoice: "View Invoice",
        newSale: "New Sale",
      },
      receipt: {
        success: "Sale Complete",
        title: "Receipt",
      },
      features: {
        sessions: "Session Management",
        sessionsDesc: "Open and close POS sessions with cash reconciliation",
        quickSale: "Quick Sales",
        quickSaleDesc: "Fast checkout for walk-in customers",
        payments: "Multiple Payments",
        paymentsDesc: "Accept cash, card, or bank transfers",
        receipts: "Receipts & Print",
        receiptsDesc: "Generate and print professional receipts",
        customers: "Customer Tracking",
        customersDesc: "Link sales to customers for better tracking",
        inventory: "Inventory",
        inventoryDesc: "Track stock levels and product catalog",
      },
    },
    categories: {
      title: "Financial Categories",
      subtitle: "Manage income and expense categories",
      addCategory: "Add Category",
      editCategory: "Edit Category",
      deleteConfirm: "Delete Category",
      deleteConfirmDesc: "This action cannot be undone.",
      incomeCategories: "Income Categories",
      expenseCategories: "Expense Categories",
      empty: "No categories yet",
      emptyDesc: "Create custom categories to organize your finances",
      name: "Name",
      namePlaceholder: "Category name",
      nameAr: "Arabic Name",
      nameArPlaceholder: "Arabic name (optional)",
      type: "Type",
      income: "Income",
      expense: "Expense",
      accountCode: "Account Code",
      accountCodePlaceholder: "e.g. 4100",
      parent: "Parent Category",
      noParent: "No parent (top-level)",
      parentLabel: "Parent",
      description: "Description",
      descriptionPlaceholder: "Optional description",
      manage: "Manage categories",
      unmapped: "Unmapped",
      unmappedFull: "Unmapped Category",
      unavailable: "Category Unavailable",
    },
    categorization: {
      income: "Income (Revenue)",
      expense: "Expense (Cost)",
      category: "Financial Category",
      selectCategory: "Select category",
      accountCode: "Account Code",
      accountCodePlaceholder: "e.g. 4200",
      taxCategory: "Tax Category",
      selectTax: "Select tax category",
      baseAmount: "Base Amount",
      tax: "Tax",
      total: "Total",
    },
    invoices: {
      title: "Invoices",
      create: "Create Invoice",
      number: "Invoice Number",
      client: "Client",
      issueDate: "Issue Date",
      dueDate: "Due Date",
      amount: "Amount",
      status: "Status",
      items: "Items",
      addItem: "Add Item",
      addFromCatalog: "From Services",
      addFromPackage: "From Packages",
      searchService: "Search services...",
      searchPackage: "Search packages...",
      servicesIncluded: "services",
      catalogLinked: "Service",
      packageSource: "Package",
      periodStart: "Period Start",
      periodEnd: "Period End",
      description: "Description",
      quantity: "Quantity",
      unitPrice: "Unit Price",
      total: "Total",
      subtotal: "Subtotal",
      tax: "Tax",
      discount: "Discount",
      selectHorse: "Select horse (optional)",
      noHorse: "No horse (client-level)",
      domain: {
        select: "Category (optional)",
        general: "General",
        training: "Training",
        transport: "Transport",
      },
      empty: "No invoices yet",
      emptyDesc: "Create your first invoice to get started",
      pending: "Pending",
      paid: "Paid",
      overdue: "Overdue",
      statuses: {
        draft: "Draft",
        reviewed: "Reviewed",
        approved: "Approved",
        shared: "Shared",
        issued: "Issued",
        paid: "Paid",
        partial: "Partially Paid",
        overdue: "Overdue",
        cancelled: "Cancelled",
        unknown: "Unknown Status",
      },
      view: "View Invoice",
      downloadPDF: "Download PDF",
      print: "Print",
      send: "Send to Client",
      markPaid: "Mark as Paid",
      createDraft: "Create Draft",
      selectClient: "Select a client",
      lineItems: "Line Items",
      taxRate: "Tax Rate",
      totalAmount: "Total Amount",
      notes: "Notes",
      notesPlaceholder: "Payment terms, thank you message, etc.",
      itemDescription: "Item description...",
      addManualItem: "Add Manual Item",
      manualEntry: "Manual",
      emptyLineItems: "No items added yet",
      emptyLineItemsHint: "Use the buttons below to add manual items or select from the service catalog",
      servicesDialogTitle: "Select Services",
      packagesDialogTitle: "Select Packages",
      servicesDialogEmpty: "No active services are available for this account.",
      packagesDialogEmpty: "No packages are available for this account yet.",
      servicesDialogSearch: "Search by name, code, or category",
      packagesDialogSearch: "Search packages",
      apply: "Apply",
      selectedCount: "{{count}} selected",
      missingPrice: "Missing price",
      totalsByCurrency: "Totals by currency",
      allCategories: "All categories",
      includedServices: "Included services",
      packageTotal: "Package total",
      packageLineHint: "One financial line — included services show as details at 0.00",
      addNewClient: "+ Add new client",
      addNewHorse: "+ Add new horse",
      searchClient: "Search clients...",
      searchHorse: "Search horses...",
      noClientsFound: "No clients found",
      noHorsesFound: "No horses found",
      noHorseClientLevel: "No horse (client-level)",
      clientLevelCharges: "Client-Level Charges",
      included: "Included",
      horseGroupHeading: "Horse",
      horseGroupLabel: "Horse",
      unassignedHorse: "Unassigned horse",
      pdfInvoiceTitle: "Invoice",
      pdfBillTo: "Bill To",
      pdfThankYou: "Thank you for your business!",
      pdf: {
        continuationSuffix: " — Continued",
        pageOf: "Page {current} of {total}",
      },
      dateToday: "Today",
      dateClear: "Clear",
      taxBadgeTaxable: "VAT",
      taxBadgeExempt: "Tax-exempt",
      financialSummary: "Financial Summary",
      noItemsError: "Please add at least one item",
      searchPlaceholder: "Search invoices...",
      tryDifferentFilters: "Try different filters",
      deleteConfirm: "Delete Invoice",
      deleteConfirmDesc: "This action cannot be undone. This will permanently delete the invoice.",
      deleteConfirmTitle: "Delete Invoice?",
      deleted: "Invoice deleted",
      markedAsPaid: "Invoice marked as paid",
      sentSuccess: "Invoice sent successfully",
      selectPaymentMethod: "Select a payment method to record this payment.",
      paymentMethod: "Payment Method",
      pdfDownloaded: "PDF downloaded",
      pdfFailed: "Failed to generate PDF",
      printFailed: "Failed to print invoice",
      printOptionsDesc: "Choose what to include before exporting this invoice.",
      includePaymentHistory: "Include payment history",
      includePaymentHistoryDesc: "Add a detailed table of recorded payments to the exported invoice.",
      includePaymentHistoryUnavailable: "No payments have been recorded on this invoice yet.",
      paymentStatusLabel: "Payment Status",
      statusUnpaid: "Unpaid",
      statusPartial: "Partially Paid",
      statusPaid: "Paid in Full",
      paidToDate: "Paid to Date",
      colMethod: "Method",
      colEffectiveDate: "Effective Date",
      colRecordedAt: "Recorded At",
      colAmount: "Amount",
      issued: "Issued",
      due: "Due",
      noClient: "No client",
      detailsTitle: "Invoice Details",
      close: "Close",
      review: "Review",
      reviewDesc: "Mark this invoice as internally reviewed.",
      approve: "Approve Invoice",
      approveDesc: "Approving will activate this invoice financially. It will enter the ledger and affect client balances.",
      share: "Share with Client",
      shareDesc: "Make this invoice visible to the counterparty.",
      cancelInvoice: "Cancel Invoice",
      cancelInvoiceDesc: "This will void the invoice and reverse its financial effect. This action cannot be undone.",
      cancelDate: "Cancellation date",
      cancelReason: "Cancellation reason",
      cancelReasonPlaceholder: "Explain why this invoice is being cancelled",
      cancelReasonRequired: "Enter a cancellation reason",
      cancelledSuccess: "Invoice cancelled and reversed",
      reviewedSuccess: "Invoice marked as reviewed",
      approvedSuccess: "Invoice approved and posted to ledger",
      sharedSuccess: "Invoice shared with counterparty",
      approveErrors: {
        generic: "Could not approve the invoice.",
        sourceSnapshotStale:
          "A saved line no longer matches its service in the catalog. Open the invoice, refresh the affected line, then approve again.",
        totalsStale:
          "The invoice totals no longer match its saved lines. Open the invoice, save it to recompute totals, then approve again.",
        notApprovable: "This invoice can no longer be approved from its current state.",
        itemsInvalid: "One or more invoice lines are invalid. Fix them and save before approving.",
        itemsEmpty: "Add at least one line to the invoice before approving.",
        payloadUnknownKey:
          "The invoice could not be approved because the approval data is incompatible with the current system version. Refresh the page and try again. If the issue continues, contact support.",
      },
      cannotDeleteApproved: "Cannot delete an approved invoice. Use Cancel instead.",
      horse: "Horse",
      sample: "Sample",
      service: "Service",
      category: "Category",
      selectCustomerFirst: "Select a customer first to view linked horses",
      noHorsesLinkedToCustomer: "No horses are linked to this customer",
      loadingHorses: "Loading horses…",
      unableToLoadHorses: "Unable to load horses",
      retry: "Retry",
      missing: {
        client: "Select a client",
        issueDate: "Enter an issue date",
        dueDate: "Enter a due date",
        lineItems: "Add at least one valid line item",
        lineItemDescription: "Each line needs a description",
        lineItemQuantity: "Quantity must be greater than zero",
        lineItemUnitPrice: "Unit price cannot be negative",
      },
    },
    expenses: {
      title: "Expenses",
      approveFailed: "Failed to approve expense",
      create: "Add Expense",
      category: "Category",
      description: "Description",
      amount: "Amount",
      date: "Date",
      vendor: "Vendor",
      status: "Status",
      receipt: "Receipt",
      uploadReceipt: "Upload Receipt",
      empty: "No expenses yet",
      emptyDesc: "Track your expenses by adding the first one",
      total: "Total Expenses",
      thisMonth: "This Month",
      pending: "Pending",
      categories: {
        feed: "Feed & Nutrition",
        veterinary: "Veterinary",
        equipment: "Equipment",
        labor: "Labor",
        utilities: "Utilities",
        maintenance: "Maintenance",
        transportation: "Transportation",
        insurance: "Insurance",
        other: "Other",
      },
      statuses: {
        pending: "Pending",
        approved: "Approved",
        paid: "Paid",
        rejected: "Rejected",
      },
      selectCategory: "Select a category",
      descriptionPlaceholder: "What was this expense for?",
      vendorPlaceholder: "Vendor or supplier name",
      notes: "Notes",
      notesPlaceholder: "Additional notes...",
      noVendor: "No vendor",
      viewReceipt: "View Receipt",
      approve: "Approve",
      reject: "Reject",
      searchPlaceholder: "Search expenses...",
      tryDifferentFilters: "Try different filters",
      deleteConfirm: "Delete Expense",
      deleteConfirmDesc: "This action cannot be undone.",
      invalidFileType: "Only images and PDFs are allowed",
      fileTooLarge: "File is too large (max 10MB)",
      uploadFailed: "Failed to upload receipt",
      takePhoto: "Take Photo",
      missing: {
        category: "Select an expense category",
        amount: "Enter a valid amount",
      },
    },
    ledger: {
      title: "Customer Ledger",
      selectClient: "Select a client",
      balance: "Balance",
      entries: "Entries",
      createEntry: "Add Entry",
      entryType: "Type",
      reference: "Reference",
      customers: "Customers",
      receivable: "Receivable",
      overdue: "Overdue",
      empty: "No ledger entries",
      emptyDesc: "Ledger entries will appear when you create invoices or record payments",
      entryTypes: {
        invoice: "Invoice",
        payment: "Payment",
        credit: "Credit",
        adjustment: "Adjustment",
      },
      collected: "Collected",
      debit: "Debit",
      credit: "Credit",
    },
    payments: {
      record: "Record Payment",
      recordPayment: "Record Payment",
      recordPaymentDesc: "Record one or more payments for this invoice",
      invoiceTotal: "Invoice Total",
      invoiceItems: "Invoice Items",
      paidSoFar: "Paid So Far",
      outstanding: "Outstanding",
      paymentDetails: "Payment Details",
      paymentDate: "Payment date",
      method: "Payment Method",
      amount: "Amount",
      reference: "Reference",
      referencePlaceholder: "Check #, Txn ID...",
      addPaymentMethod: "Add Payment Method",
      payFullAmount: "Pay Full Amount",
      payFullOutstanding: "Pay Full Outstanding",
      paymentAmount: "Payment Amount",
      paymentMethodDetails: "Payment Method Details",
      totalPayment: "Total Payments",
      outstandingAfter: "Remaining After Payment",

      willBeFullyPaid: "Will be fully paid",
      alreadyPaid: "This invoice is already fully paid",
      overpaymentError: "Payment amount exceeds outstanding balance",
      paymentRecorded: "Payment recorded successfully",
      viewPayments: "View Payments",
      paymentHistory: "Payment History",
      noPayments: "No payments recorded yet",
      fullyPaid: "Invoice fully paid",
      recorded: "Payment recorded",
      groupedItems: {
        horseHeader: "Horse",
        itemCount: "items",
        itemCountOne: "item",
        subtotal: "Subtotal",
        clientLevel: "Client-Level Charges",
      },
      pdfPaymentSession: {
        sessionLabel: "Payment Session",
        methodsHeading: "Payment Methods",
        distributionHeading: "Payment Distribution",
        horseColumn: "Horse",
        clientLevelLabel: "Client-Level",
        historicalLabel: "Historical Payments",
        sessionTotal: "Session Total",
      },
      missing: {
        date: "Select the payment date",
        amount: "Enter at least one payment amount greater than zero",
        method: "Select a payment method",
        invalidAmount: "Amount must be a positive number",
      },
      errors: {
        unknown: "Failed to record payment",
        idempotencyConflict: "This payment attempt conflicts with an earlier submission. Reopen the dialog and try again.",
        overAllocation: "One of the payments exceeds the invoice's outstanding amount.",
        notPayable: "This invoice is not in a payable state.",
        crossClient: "Selected invoices don't belong to the same client, tenant, or currency.",
        accountMissing: "No active payment account is configured for this workspace.",
        allocationRequired: "This invoice covers multiple horses or a client-level line. Splitting a payment across them requires the allocation editor, which arrives in the next update.",
        historyUnresolved: "This invoice has legacy allocations that must be reconciled before new payments can be recorded.",
        permissionDenied: "You don't have permission to record payments.",
        methodInvalid: "Selected payment method is not supported.",
        duplicateAllocation: "The same invoice and payment method were provided twice.",
      },
      allocation: {
        title: "Payment Distribution",
        horseLabel: "Horse",
        clientLevel: "Client-level items",
        remaining: "Remaining",
        allocated: "Paid Amount",
        remainingAfter: "Remaining",
        pretax: "Services / Pretax",
        tax: "Tax",
        totalDue: "Total Due",
        distributeEqually: "Distribute Equally",
        resetProposal: "Reset",
        remainderMismatch: "Allocations must equal the payment amount and stay within each remaining balance.",
        balanced: "Allocation matches payment amount",
      },

    },
    multiInvoicePayment: {
      title: "Record Client Payment",
      description: "Record one atomic payment across multiple outstanding invoices for this client.",
      openAction: "Record Client Payment",
      openActionShort: "Record Payment",
      selectClient: "Select a Client",
      selectClientDescription: "Pick the client you want to record a payment for.",
      searchPlaceholder: "Search clients…",
      continue: "Continue",
      eligibleInvoices: "Outstanding Invoices",

      selectedInvoices: "selected",
      noEligibleInvoices: "This client has no outstanding invoices in the tenant currency.",
      distributeOldestFirst: "Distribute Oldest First",
      allocatedToInvoices: "Allocated to Invoices",
      allocatedToInvoice: "Paid Now",
      remainingToAllocate: "Remaining to Allocate",
      difference: "Difference",
      record: "Record Payment",
      recordPayments: "Record Payments",
      recordClientPayment: "Record Client Payment",
      pageIndicator: "{{current}} of {{total}}",
      success: "Payment recorded across selected invoices",
      loadingComposition: "Loading invoice composition",
      allocationForInvoice: "Allocate across horses for invoice",
      allocatedToInvoiceLabel: "Amount Allocated to Invoice",
      remainingAfterPayment: "Remaining After Payment",
      itemsCount: "Invoice Items ({{count}})",
      showItems: "Show Invoice Items",
      hideItems: "Hide Invoice Items",
      internalDistribution: "Distribute Payment Within Invoice",
      returnToAutomatic: "Return to Automatic Distribution",
      amountDerivedNotice: "Amount is determined by the invoice distribution",
      packageIncludedItems: "Included Package Items",
      totalPayments: "Total Payments",
      allocatedToInvoicesShort: "Allocated to Invoices",
      remainingToAllocateShort: "Remaining to Allocate to Invoices",
      payInFull: "Pay in Full",
      summary: {
        eligibleCount: "Eligible Invoices",
        totalOutstanding: "Total Outstanding",
        selectedCount: "Selected Invoices",
        selectedOutstanding: "Selected Outstanding",
      },
      errors: {
        perInvoiceOver: "One or more invoice allocations exceed the outstanding balance:",
        duplicateTender: "Two payment methods share the same reference. Use distinct references or merge them.",
        tooManyRows: "This payment would create too many allocation rows for a single session.",
        needsAllocationEditor: "One of the selected invoices needs the single-invoice allocation editor. Open it from the invoice details.",
        overAllocationBy: "Allocated amount exceeds total payment by",
        paymentsExceedOutstandingBy: "Total payment exceeds outstanding on selected invoices by",
        notSupported: "This invoice's composition is not yet supported by multi-invoice payments. Open it individually.",
        overMaxInvoice: "Amount exceeds this invoice's outstanding balance",
        overMaxBucket: "Amount exceeds this bucket's remaining balance",
      },

    },

    invoiceStats: {
      totalInvoices: "Total Invoices",
      pendingInvoices: "Pending Invoices",
      paidAmount: "Paid Amount",
      overdueAmount: "Overdue Amount",
      paidInvoices: "Paid Invoices",
      overdueInvoices: "Overdue Invoices",
    },

    paymentMethods: {
      cash: "Cash",
      card: "Card",
      transfer: "Bank Transfer",
      check: "Check",
      credit: "Credit",
      mixed: "Mixed",
    },
    status: {
      draft: "Draft",
      sent: "Sent",
      shared: "Shared",
      issued: "Issued",
      reviewed: "Reviewed",
      approved: "Approved",
      partial: "Partially Paid",
      partially_paid: "Partially Paid",
      paid: "Paid",
      overdue: "Overdue",
      cancelled: "Cancelled",
      pending: "Pending",
      rejected: "Rejected",
    },

    creditLimit: {
      exceeded: "Credit limit exceeded. Cannot create invoice.",
      limit: "Credit Limit",
      currentOutstanding: "Current Outstanding",
      available: "Available",
      willExceed: "This invoice will exceed credit limit",
    },
  },
  dashboard: {
    widgets: {
      upcomingSchedule: "Upcoming Schedule",
      recentActivity: "Recent Activity",
      financialSummary: "Financial Summary",
      viewAll: "View All",
      revenue: "Revenue",
      expenses: "Expenses",
      outstanding: "Outstanding",
      netThisMonth: "Net This Month",
    },
    welcome: "Welcome!",
    welcomeName: "Welcome, {{name}}!",
    welcomeMessage: "Here's what's happening at {{name}} today.",
    todayAt: "Here's what's happening today at {{org}}.",
    personalSubtitle: "Here's your personal activity overview for today.",
    discoverServices: "Discover our services and explore what Dayli Horse has to offer.",
    noTenantMessage: "Discover our services and explore what Dayli Horse has to offer.",
    gettingStarted: "Get Started with Dayli Horse",
    getStarted: "Get Started with Dayli Horse",
    gettingStartedDesc: "Create your organization to manage horses, track health records, and collaborate with your team.",
    getStartedDesc: "Create your organization to manage horses, track health records, and collaborate with your team. Or explore our services first!",
    createOrganization: "Create Organization",
    searchPlaceholder: "Search horses, records...",
    totalHorses: "Total Horses",
    healthCheckups: "Health Checkups",
    teamMembers: "Team Members",
    thisMonth: "This Month",
    activeRecords: "Active records",
    addFirstHorse: "Add your first horse",
    addYourFirstHorse: "Add Your First Horse",
    scheduledThisWeek: "Scheduled this week",
    statsComingSoon: "Stats coming soon",
    yourHorses: "Your Horses",
    viewAll: "View All",
    noHorsesYet: "No horses yet",
    loadingHorses: "Loading horses...",
    availableServices: "Available Services",
    exploreServices: "Explore our services and start your 14-day free trial",
    stableManagement: "Stable Management",
    stableManagementDesc: "Manage your stable, horses, and team",
    horseOwner: "Horse Owner",
    horseOwnerDesc: "Track your horses' health and records",
    veterinaryClinic: "Veterinary Clinic",
    veterinaryClinicDesc: "Manage appointments and medical records",
    vetClinic: "Veterinary Clinic",
    vetClinicDesc: "Manage appointments and medical records",
    trainingAcademy: "Training Academy",
    trainingAcademyDesc: "Schedule training sessions and track progress",
    startFreeTrial: "Start Free Trial (14 days)",
    overview: "Overview",
    upcoming: "Upcoming",
    noUpcomingEvents: "No upcoming events",
    completePublicProfile: "Complete Your Public Profile",
    completePersonalProfile: "Complete Your Personal Profile",
    completePersonalProfileDesc: "Add your personal details so others can recognize you.",
    completePublicProfileDesc: "Set up your public profile to appear in the directory.",
    publicProfileDesc: "Set up your public profile to appear in the directory and let customers find and book your services.",
    setUpNow: "Set Up Now",
    completeNow: "Complete Now",
    publicProfileLive: "Your Public Profile is Live!",
    publicProfileLiveDesc: "Customers can now find you in the directory and book your services.",
    editProfile: "Edit Profile",
    viewPublicPage: "View Public Page",
    healthy: "Healthy",
    unknownBreed: "Unknown breed",
    quickActions: "Quick Actions",
    myProfileDesc: "Manage your personal identity",
    communityDesc: "Connect with the equestrian community",
    myBookingsDesc: "View and manage your bookings",
    myPaymentsDesc: "Track your payment history",
  },
  horses: {
    title: "Horses",
    horses: "horses",
    backToHorses: "Back to Horses",
    unknownBreed: "Unknown breed",
    addHorse: "Add Horse",
    noHorses: "No horses found",
    addFirstHorse: "Add your first horse to get started",
    owner: {
      unassigned: "Unassigned",
      plus: "+ {count}",
    },
    tabs: {
      all: "All",
      inside: "In Stable",
      intakeDraft: "Pending Arrival",
      incomplete: "Incomplete Profile",
      outside: "No Location",
      hosted: "Hosted",
      current: "Current",
      local: "Local",
      historical: "Historical",
      localAll: "All Local",
      noLocationSub: "No Location",
    },
    notFound: "Horse not found",
    horseDeleted: "Horse deleted",
    horseDeletedDesc: "{{name}} has been removed.",
    deleteError: "Failed to delete horse",
    loadError: "Failed to load horse details",
    addSuccess: "{{name}} has been added successfully",
    addError: "Failed to add horse",
    gender: {
      male: "Male",
      female: "Female",
    },
    breedingRoles: {
      broodmare: "Broodmare",
      breeding_stallion: "Breeding Stallion",
    },
    status: {
      active: "Active",
      inactive: "Inactive",
      draft: "Draft",
      intake_draft: "Pending Arrival",
    },
    wizard: {
      addTitle: "Add New Horse",
      editTitle: "Edit Horse",
      step: "Step",
      of: "of",
      steps: {
        registration: "Registration Check",
        basic: "Basic Info",
        details: "Location & Details",
        physical: "Physical Specs",
        pedigree: "Pedigree",
        ownership: "Ownership",
        media: "Media",
      },
      next: "Next",
      back: "Back",
      save: "Save Horse",
      saveChanges: "Save Changes",
      saving: "Saving...",
      // Registration step
      checkRegistration: "Check Registration",
      checkRegistrationDesc: "Check if the horse is already in the platform by microchip or passport number.",
      alreadyRegistered: "Is this horse already registered in the system?",
      searchExisting: "Search Existing Horse",
      registerNew: "Register New Horse",
      newHorse: "No, this is a new horse",
      newHorseDesc: "Register a new horse from scratch",
      existingHorse: "Yes, search existing horse",
      existingHorseDesc: "Find by name, microchip, passport, or UELN",
      searchPlaceholder: "Search by name, microchip, or passport...",
      noSearchResults: "No horses found matching",
      minSearchChars: "Type at least 2 characters to search",
      // Basic info step
      name: "Name (English)",
      namePlaceholder: "Enter horse name",
      nameAr: "Name (Arabic)",
      nameArPlaceholder: "Enter name in Arabic",
      gender: "Gender",
      male: "Male",
      female: "Female",
      birthDate: "Birth Date",
      registrationNumber: "Registration Number",
      avatarUrl: "Photo URL",
      breed: "Breed",
      selectBreed: "Select breed",
      chooseBreed: "Choose breed",
      searchBreedPlaceholder: "Search for a breed...",
      addNewBreed: "+ Add new breed",
      noBreedsFound: "No breeds found",
      color: "Color",
      selectColor: "Select color",
      chooseColor: "Choose color",
      searchColorPlaceholder: "Search for a color...",
      addNewColor: "+ Add new color",
      noColorsFound: "No colors found",
      ageCategory: "Age Category",
      selectAgeCategory: "Select category",
      microchip: "Microchip Number",
      passport: "Passport Number",
      ueln: "UELN",
      // Details step
      branch: "Branch",
      selectBranch: "Select branch",
      stable: "Stable",
      selectStable: "Select stable",
      housingUnit: "Housing Unit",
      selectHousingUnit: "Select unit",
      housingNotes: "Housing Notes",
      status: "Status",
      active: "Active",
      inactive: "Inactive",
      pregnant: "Pregnant",
      pregnancyMonths: "Pregnancy Months",
      gelded: "Gelded",
      breedingRole: "Breeding Role",
      broodmare: "Broodmare",
      // Physical step
      height: "Height (cm)",
      weight: "Weight (kg)",
      maneMarks: "Mane Marks",
      bodyMarks: "Body Marks",
      legsMarks: "Legs Marks",
      distinctiveMarks: "Distinctive Marks Notes",
      // Pedigree step
      sire: "Sire (Father)",
      dam: "Dam (Mother)",
      selectHorse: "Select horse",
      orEnterManually: "or enter manually",
      maternalGrandmother: "Maternal Grandmother",
      maternalGrandfather: "Maternal Grandfather",
      paternalGrandmother: "Paternal Grandmother",
      paternalGrandfather: "Paternal Grandfather",
      breeder: "Breeder",
      selectBreeder: "Select breeder",
      // Ownership step
      owners: "Owners",
      addOwner: "Add Owner",
      selectOwner: "Select owner",
      percentage: "Ownership Percentage",
      primaryOwner: "Primary Owner",
      totalMustBe100: "Total percentage must equal 100%",
      onePrimaryRequired: "One primary owner is required",
      // Media step
      images: "Images",
      videos: "Videos",
      externalLinks: "External Links",
      uploadImages: "Upload Images",
      addLink: "Add Link",
      dropImagesHere: "Drop images here or click to browse",
      dropVideosHere: "Drop videos here or click to browse",
      uploading: "Uploading...",
      imageFormats: "JPG, PNG, WebP, GIF",
      videoFormats: "MP4, WebM, MOV (max 50MB)",
      noLinksYet: "No links added yet",
      linkPlaceholder: "https://example.com",
      uploadOrDragImages: "Upload or drag-and-drop images",
      uploadOrDragVideos: "Upload or drag-and-drop videos (max 50MB)",
      addRelatedLinks: "Add related links (pedigree, social, etc.)",
      // Additional ownership step keys
      ownershipDescription: "Add owners with percentage shares",
      noOwnersYet: "No owners added yet. You can skip this step or add owners.",
      ownershipValidation: "{{total}}% total {{primaryText}}",
      noPrimaryOwner: "No primary owner",
      onePrimary: "1 primary owner",
      multiplePrimary: "{{count}} primary owners",
      ownerNumber: "Owner {{number}}",
      createNewOwner: "Create new owner",
      addAnotherOwner: "+ Additional owner",
      chooseOwner: "Choose owner",
      searchOwnerPlaceholder: "Search owners...",
      noOwnersFound: "No owners found",
      // Details step additional keys
      isGelded: "Is Gelded?",
      geldedDesc: "Mark if this horse has been castrated",
      broodmareDesc: "Mark if this mare is used for breeding",
      isPregnant: "Is Pregnant?",
      // Physical step placeholders
      heightPlaceholder: "e.g., 160",
      weightPlaceholder: "e.g., 500",
      maneMarksPlaceholder: "Describe mane characteristics...",
      bodyMarksPlaceholder: "Describe body markings...",
      legsMarksPlaceholder: "Describe leg markings...",
      additionalNotesPlaceholder: "Any other distinctive marks...",
      // Pedigree step
      motherSection: "Mother (Dam)",
      fatherSection: "Father (Sire)",
      motherNameEn: "Mother Name (English)",
      motherNameAr: "Mother Name (Arabic)",
      fatherNameEn: "Father Name (English)",
      fatherNameAr: "Father Name (Arabic)",
      // Basic info additional
      selectGender: "Select gender",
      maleDesc: "Male",
      femaleDesc: "Female",
      birthTime: "Birth Time (Optional)",
      currentAge: "Current Age",
      type: "Type",
    },
    // Master data dialog translations
    masterData: {
      color: {
        title: "Add New Color",
        nameEn: "Name (English)",
        nameAr: "Name (Arabic)",
      },
      breed: {
        title: "Add New Breed",
        nameEn: "Name (English)",
        nameAr: "Name (Arabic)",
      },
      owner: {
        unassigned: "Unassigned",
        plus: "+ {count}",
        title: "Add New Owner",
        name: "Name",
        nameEn: "Name (English)",
        nameAr: "Name (Arabic)",
        phone: "Phone",
        email: "Email",
        ownerType: "Owner Type",
        ownerTypeIndividual: "Individual",
        ownerTypeOrganization: "Organization",
        basicInfo: "Basic Information",
        ownerPhones: "Phone Numbers",
        ownerEmail: "Email",
        representativeSection: "Owner Representative",
        representativeName: "Representative Name (English)",
        representativeNameAr: "Representative Name (Arabic)",
        representativeTitle: "Title / Role",
        representativeEmail: "Representative Email",
        representativePhones: "Representative Phones",
        repNameRequiredEn: "Representative English name is required",
        repNameRequiredAr: "Representative Arabic name is required",
      },
      breeder: {
        title: "Add New Breeder",
        name: "Name",
        nameAr: "Name (Arabic)",
        phone: "Phone",
        email: "Email",
      },
      branch: {
        title: "Add New Branch",
        name: "Branch Name",
        address: "Address",
      },
      stable: {
        title: "Add New Stable",
        name: "Stable Name",
      },
      housingUnit: {
        title: "Add New Housing Unit",
        code: "Unit Code (e.g., A1, Stall-5)",
        unitType: "Type (stall/room/paddock)",
      },
      missingFields: "Missing required fields",
      pleaseFillIn: "Please fill in",
      createdSuccess: "Created successfully",
      hasBeenAdded: "Item has been added",
      errorCreating: "Error creating item",
      duplicateName: "This name already exists. Please use a different name or choose the existing value.",
      errorNetwork: {
        title: "Connection error",
        description: "Could not reach the server. Check your connection and try again.",
      },
      bilingualNameHint: "Required for database identity, search, and alphabetical sorting.",
      delete: {
        breed: {
          title: "Delete breed",
          description: "Are you sure you want to delete this breed? This action cannot be undone.",
          confirm: "Delete",
          success: "Breed deleted successfully",
          blockedUsed: "This breed cannot be deleted because it is used by horse records.",
          blockedSeed: "This breed is a protected baseline value and cannot be deleted.",
          notFound: "Breed could not be found.",
          error: "Could not delete breed. Please try again.",
        },
        color: {
          title: "Delete color",
          description: "Are you sure you want to delete this color? This action cannot be undone.",
          confirm: "Delete",
          success: "Color deleted successfully",
          blockedUsed: "This color cannot be deleted because it is used by horse records.",
          blockedSeed: "This color is a protected baseline value and cannot be deleted.",
          notFound: "Color could not be found.",
          error: "Could not delete color. Please try again.",
        },
        usedByNames: "Used by: {{names}}{{more}}",
        moreSuffix: " +{{count}} more",
      },
      edit: {
        action: "Edit",
        breedTitle: "Edit breed",
        colorTitle: "Edit color",
        breedSuccess: "Breed updated successfully",
        colorSuccess: "Color updated successfully",
        duplicate: "A breed or color with this name already exists.",
        invalidName: "Please enter a valid English name.",
        notFound: "This item could not be found.",
        error: "Could not update. Please try again.",
      },
    },
    owners: {
      typeIndividual: "Individual",
      typeOrganization: "Organization",
      repShort: "Rep",
      contact: {
        phones: "Phone Numbers",
        addPhone: "+ Add phone",
        noPhonesAdded: "No phone numbers added",
        whatsapp: "WhatsApp",
        primaryPhone: "Primary",
        setAsPrimary: "Set as primary",
      },
      phoneLabels: {
        mobile: "Mobile",
        work: "Work",
        home: "Home",
        other: "Other",
      },
    },
    errors: {
      noTenant: "No active workspace selected. Please pick a workspace and try again.",
      saveFailed: {
        title: "Could not save horse",
        description: "Something went wrong while saving. Please try again.",
      },
      deleteFailed: {
        title: "Could not delete horse",
        description: "Something went wrong while deleting this horse. Please try again.",
      },
      passportDuplicate: {
        title: "Passport number already exists",
        description: "Another horse is already registered with this passport number. Please enter a different number or review the existing horse record.",
      },
      microchipDuplicate: {
        title: "Microchip number already exists",
        description: "Another horse is already registered with this microchip number. Please verify the number or review the existing horse record.",
      },
      uelnDuplicate: {
        title: "UELN already exists",
        description: "Another horse is already registered with this UELN. Please verify the number or review the existing horse record.",
      },
      noWriteAuthority: {
        title: "You cannot edit this horse's identity",
        description: "Only the current verified owner (or an authorized workspace) can edit this horse's identity from this account.",
      },
      ambiguousAuthority: {
        title: "Owner authority needs verification",
        description: "This horse's ownership authority is not clearly resolved yet. Please verify the owner before editing.",
      },
      unsupportedField: {
        title: "This field cannot be edited here",
        description: "One of the submitted fields is not part of horse identity edit. Please remove it and try again.",
      },
      invalidField: {
        title: "Please check the highlighted value",
        description: "One of the values is invalid. Please review and try again.",
      },
      emptyPayload: {
        title: "No changes to save",
        description: "There are no changes to save yet.",
      },
      malformedPayload: {
        title: "Invalid save request",
        description: "The save request was not formatted correctly. Please try again.",
      },
      // Phase 1.e.f.8.1.4.d.3.fix.1.r1 — Critical Identity Field Edit
      // Governance Correction. Field-specific messages that point users
      // to future correction workflows instead of a generic save failure.
      hostIdentityDenied: {
        title: "Identity edit not permitted",
        description: "You do not have permission to edit this horse's identity data. This horse is hosted under your account, but identity edits require owner authority.",
      },
      ownershipTransferred: {
        title: "Ownership transferred",
        description: "You are no longer the owner of this horse. Ownership has been transferred to another owner.",
      },
      genderLocked: {
        title: "Sex is not editable here",
        description: "Gender cannot be changed from normal identity editing. Use the gender correction flow when available.",
      },
      breedLocked: {
        title: "Breed is not editable here",
        description: "Breed cannot be changed from normal identity editing. Breed changes require a dedicated correction flow.",
      },
      birthDateLocked: {
        title: "Date of birth is locked",
        description: "Date of birth cannot be changed from this flow after it has been set. A dedicated birth-date correction flow will handle this later.",
      },
      birthTimeLocked: {
        title: "Birth time is not editable here",
        description: "Birth time cannot be edited from identity editing. It will be managed together with the future birth-date correction flow.",
      },
      restrictedField: {
        title: "This field cannot be changed here",
        description: "This field cannot be changed from normal identity editing.",
      },
      // Phase 1.e.f.8.1.4.d.3.fix.1.r1.qa1.local — Local Record Custodial
      // Completion error mapping. These messages are shown to Stable staff
      // when the custodial completion RPC rejects a write.
      localRecordPermissionDenied: {
        title: "You cannot complete this record",
        description: "You do not have permission to complete this local horse record. Ask a workspace owner or manager to complete it.",
      },
      localRecordOwnerExists: {
        title: "Owner information already exists",
        description: "This horse already has owner information, so it cannot be completed as a local record. Ownership changes require the owner authority workflow.",
      },
      localRecordReviewRequired: {
        title: "Owner-truth review required",
        description: "This record needs an owner-truth review before local completion is available again.",
      },
      localRecordNotInTenant: {
        title: "Not part of this workspace",
        description: "This horse is not part of the active workspace and cannot be completed here.",
      },
      localRecordStatusDenied: {
        title: "Record is not eligible",
        description: "This record is archived, retired, transferred out or otherwise not eligible for local completion.",
      },
      localRecordFieldRestricted: {
        title: "This field cannot be completed here",
        description: "Sensitive identity fields such as sex, breed or reproductive role cannot be edited from local completion.",
      },
      localRecordFieldNotMissing: {
        title: "This field is already filled",
        description: "This field is no longer missing on the record and cannot be overwritten from local completion.",
      },
      localRecordBirthDateAlreadySet: {
        title: "Date of birth is already recorded",
        description: "Date of birth is already set on this horse. It cannot be changed from local completion.",
      },
      localRecordNoSafeFields: {
        title: "Nothing to complete",
        description: "There are no safe missing fields to complete on this record.",
      },
      localRecordCompletionUnavailable: {
        title: "Local completion is not available",
        description: "This record cannot be completed locally right now.",
      },
    },

    localRecord: {
      dialog: {
        title: "Complete local record",
        description: "Fill in the missing safe fields for this Stable-local horse record. Sensitive identity fields (sex, breed, reproductive role) are handled by dedicated workflows.",
        submit: "Save",
        nothingMissing: "There are no safe missing fields to complete on this record.",
      },
      completed: {
        title: "Record updated",
        description: "The local horse record has been updated.",
      },
    },

    toast: {
      created: {
        title: "Horse created",
        description: "{{name}} has been added successfully.",
      },
      updated: {
        title: "Horse updated",
        description: "{{name}} has been updated successfully.",
      },
      ownershipWarning: {
        title: "Horse saved, but ownership was not linked",
        description: "The horse was saved successfully, but ownership could not be linked. You can update ownership later from the horse profile.",
      },
    },
    // Table view column headers
    table: {
      name: "Name",
      breed: "Breed",
      type: "Type",
      age: "Age",
      color: "Color",
      owner: "Owner",
      status: "Status",
    },
    filters: {
      gender: "Gender",
      status: "Status",
      breed: "Breed",
      color: "Color",
      allGenders: "All Genders",
      male: "Male",
      female: "Female",
      allStatus: "All Status",
      allBreeds: "All Breeds",
      allColors: "All Colors",
      clearFilters: "Clear Filters",
      filterHorses: "Filter Horses",
      searchPlaceholder: "Search horses...",
    },
    age: {
      year: "year",
      years: "years",
      month: "month",
      months: "months",
      week: "week",
      weeks: "weeks",
      day: "day",
      days: "days",
      unknownAge: "Unknown age",
    },
    labRequests: {
      title: "Lab Requests",
      noRequests: "No lab requests for this horse",
    },
    profile: {
      physicalSpecs: "Physical Specifications",
      height: "Height",
      weight: "Weight",
      noPhysicalSpecs: "No physical specs recorded",
      identification: "Identification",
      microchip: "Microchip",
      passport: "Passport",
      ueln: "UELN",
      noIdentification: "No identification recorded",
      pedigree: "Pedigree",
      sire: "Sire (Father)",
      dam: "Dam (Mother)",
      noPedigree: "No pedigree information recorded",
      location: "Location",
      branch: "Branch",
      stable: "Stable",
      noLocation: "No location assigned",
      mediaGallery: "Media Gallery",
      notes: "Notes",
      breed: "Breed",
      color: "Color",
      owner: "Owner",
      incompleteTitle: "Incomplete Profile",
      completeProfile: "Complete Profile",
      tabs: {
        overview: "Overview",
        housingMovement: "Housing & Movement",
        pedigreeBreeding: "Pedigree & Breeding",
        more: "More",
      },
      projection: {
        source: {
          snapshot: "Snapshot",
          redacted: "Redacted",
        },
        reason: {
          owner_only: "Owner only",
          deferred: "Deferred",
        },
      },
    },
    mediaGallery: {
      zoomIn: "Zoom In",
      zoomOut: "Zoom Out",
      close: "Close",
      previous: "Previous",
      next: "Next",
      noMedia: "No media",
      videoError: "Failed to load video",
      imageError: "Failed to load image",
    },
    ownership: {
      // CurrentOwnership.tsx
      currentOwnership: "Current Ownership",
      totalOwnership: "Total Ownership",
      remaining: "remaining",
      totalShouldBe100: "Total should be 100% ({{remaining}}% remaining)",
      noOwnersAssigned: "No owners assigned yet",
      primary: "Primary",
      editOwnership: "Edit Ownership",
      percentage: "Percentage (%)",
      addOwner: "Add Owner",
      owner: "Owner",
      selectOwner: "Select owner",
      primaryOwner: "Primary Owner",
      maxPlaceholder: "Max: {{max}}%",
      removeOwner: "Remove Owner?",
      removeOwnerConfirm: "Remove {{owner}} from {{horse}}'s ownership? This action cannot be undone.",
      removing: "Removing...",
      transfer: "Transfer",
      
      // TransferOwnershipDialog.tsx
      transferOwnership: "Transfer Ownership",
      from: "From",
      current: "Current",
      transferTo: "Transfer To",
      existingOwner: "Existing Owner",
      newOwner: "New Owner",
      selectRecipient: "Select Recipient",
      selectRecipientPlaceholder: "Select recipient",
      noOtherOwners: "No other owners",
      noAvailableOwners: "No available owners",
      percentageToTransfer: "Percentage to Transfer",
      effectiveDate: "Effective Date",
      transferring: "Transferring...",
      transferSuccess: "Transfer successful",
      transferSuccessDesc: "{{percentage}}% transferred to {{recipient}}",
      transferFailed: "Transfer failed",
      recipient: "recipient",
      
      // OwnershipTimeline.tsx
      history: "Ownership History",
      noChangesRecorded: "No ownership changes recorded yet",
      actions: {
        added: "Owner Added",
        updated: "Ownership Updated",
        removed: "Owner Removed",
        transferred: "Ownership Transferred",
      },
      unknownOwner: "Unknown Owner",
      fromPercentage: "(from {{percentage}}%)",
      
      // Toast messages
      ownerAdded: "Owner added successfully",
      ownerUpdated: "Ownership updated",
      ownerRemoved: "Owner removed",
      
      // Validation
      fillAllFields: "Please fill all fields",
      invalidPercentage: "Invalid percentage",
      cannotExceed100: "Cannot exceed 100%. Remaining: {{remaining}}%",
      maxAllowed: "Cannot exceed 100%. Max allowed: {{max}}%",
      selectRecipientError: "Please select a recipient",
      invalidPercentageMax: "Invalid percentage. Max: {{max}}%",
    },
    deleteConfirm: {
      title: "Delete {{name}}?",
      description: "This action cannot be undone. This will permanently delete the horse and all associated data.",
      deleting: "Deleting...",
    },
  },
  permissions: {
    accessDenied: "Access Denied",
    accessDeniedDesc: "You don't have permission to access this feature. Contact your administrator.",
    title: "Permissions",
    bundles: "Permission Bundles",
    bundlesDesc: "Create reusable permission templates",
    createBundle: "Create Bundle",
    editBundle: "Edit Bundle",
    deleteBundle: "Delete Bundle",
    duplicateBundle: "Duplicate Bundle",
    confirmDeleteBundle: "Are you sure you want to delete this bundle?",
    bundleDeleted: "Bundle deleted",
    bundleDuplicated: "Bundle duplicated",
    bundleEditorDesc: "Select permissions to include in this bundle",
    viewBundle: "View Bundle",
    viewBundleDesc: "View bundle details and permissions",
    selectedPermissionsList: "Selected Permissions",
    noPermissionsInBundle: "No permissions in this bundle",
    bundleName: "Bundle Name",
    bundleNamePlaceholder: "e.g. Lab Technician",
    description: "Description",
    descriptionPlaceholder: "Optional description...",
    selectPermissions: "Select Permissions",
    searchPermissions: "Search permissions...",
    permissionsSelected: "permissions selected",
    ownerOnly: "Owner Only",
    members: "Members",
    membersDesc: "Manage member permissions",
    selectMember: "Select a member",
    assignedBundles: "Assigned Bundles",
    assignedBundlesDesc: "Toggle bundles for this member",
    noBundles: "No bundles created yet",
    overrides: "Permission Overrides",
    overridesDesc: "Grant or revoke individual permissions",
    granted: "Grant",
    revoked: "Revoke",
    inherit: "Inherit",
    cannotDelegate: "You cannot delegate this permission",
    clickToToggle: "Click to toggle",
    auditLog: "Audit Log",
    auditLogDesc: "Recent permission changes",
    noAuditLogs: "No permission changes recorded",
    noMembers: "No Members",
    noMembersDesc: "Invite team members to manage their permissions",
    to: "to",
    actions: {
      granted: "Granted",
      revoked: "Revoked",
      bundle_assigned: "Bundle Assigned",
      bundle_removed: "Bundle Removed",
      override_removed: "Override Removed",
    },
    delegationScopes: "Delegation Scopes",
    delegationScopesDesc: "Control which permissions this manager can delegate to others",
    allowDelegate: "Allow delegation",
    // Module names for permissions
    modules: {
      horses: "Horses",
      laboratory: "Laboratory",
      finance: "Finance",
      hr: "HR / Team",
      housing: "Housing",
      movement: "Movement",
      breeding: "Breeding",
      vet: "Vet & Health",
      academy: "Academy",
      services: "Services",
      files: "Files",
      schedule: "Schedule",
      settings: "Settings",
      admin: "Admin",
    },
    noDescriptionAvailable: "—",
    // Permission action labels
    labels: {
      // Finance permissions
      "finance.invoice.view": "View invoices",
      "finance.invoice.create": "Create invoices",
      "finance.invoice.edit": "Edit draft invoices",
      "finance.invoice.delete": "Delete invoices",
      "finance.invoice.send": "Send invoices",
      "finance.invoice.markPaid": "Mark invoices as paid",
      "finance.invoice.print": "Print/download invoices",
      // Laboratory horses permissions
      "laboratory.horses.view": "View lab horses registry",
      "laboratory.horses.create": "Register new lab horses",
      "laboratory.horses.edit": "Edit lab horse information",
      "laboratory.horses.archive": "Archive/restore lab horses",
      "laboratory.horses.export": "Export lab horse reports",
      // Laboratory samples permissions
      "laboratory.samples.create": "Create lab samples",
      "laboratory.samples.edit": "Edit lab samples",
      "laboratory.samples.delete": "Delete lab samples",
    },
  },
  sharedMedia: {
    invalidLink: "Invalid or Expired Link",
    invalidLinkDesc: "This link may have expired, been revoked, or is invalid.",
    download: "Download",
    previewNotAvailable: "Preview not available",
  },
  // Orders module
  orders: {
    title: "Horse Orders",
    subtitle: "Manage veterinary, training, grooming and other service orders",
    provider: "Provider",
    newOrder: "New Order",
    createOrder: "Create Order",
    deleteOrder: "Delete Order",
    deleteOrderConfirm: "Are you sure you want to delete this order? This action cannot be undone.",
    noOrdersFound: "No Orders Found",
    createFirstOrderPrompt: "Create your first order to start tracking horse services",
    noOrdersCreatedYet: "No orders have been created yet",
    noOrganizationSelected: "No Organization Selected",
    createOrJoinOrganization: "Please create or join an organization to manage orders.",
    manageOrderTypes: "Manage Order Types",
    serviceCapabilities: "Service Capabilities",
    orderTimeline: "Order Timeline",
    timeline: {
      title: "Activity History",
      noActivity: "No activity recorded yet",
      created: "Order created",
      statusChanged: "Status changed from {{from}} to {{to}}",
      updated: "Order details updated",
      deleted: "Order deleted",
    },
    actions: {
      viewDetails: "View Details",
      edit: "Edit Order",
      markAs: "Mark as",
      billOrder: "Bill Order",
    },
    table: {
      horse: "Horse",
      type: "Type",
      status: "Status",
      priority: "Priority",
      mode: "Mode",
      scheduled: "Scheduled",
      actions: "Actions",
    },
    filters: {
      searchOrders: "Search orders...",
      allStatuses: "All Statuses",
      allPriorities: "All Priorities",
      allHorses: "All Horses",
      status: "Status",
      priority: "Priority",
    },
  },
  // Breeding module
  breeding: {
    title: "Breeding & Reproduction",
    dashboardTitle: "Breeding & Reproduction",
    dashboardSubtitle: "Manage breeding records, pregnancies, embryo transfers, and semen inventory",
    unknownMare: "Unknown Mare",
    unknownStallion: "Unknown Stallion",
    assigned: "Assigned",
    markSuccessful: "Mark Successful",
    markUnsuccessful: "Mark Unsuccessful",
    addNew: "Add New",
    noOrgSelected: "No Organization Selected",
    noOrgDesc: "Please create or join an organization to manage breeding records.",
    createOrg: "Create Organization",
    // Tab labels
    tabs: {
      records: "Breeding Records",
      pregnancies: "Pregnancies",
      foalings: "Foalings",
      contracts: "Contracts",
      embryo: "Embryo Transfers",
      inventory: "Semen Inventory",
    },
    // Breeding method labels
    methods: {
      natural: "Natural Cover (Live Cover)",
      ai_fresh: "Artificial Insemination (Fresh)",
      ai_frozen: "Artificial Insemination (Frozen)",
    },
    // Legacy attemptTypes kept for backward compat
    attemptTypes: {
      natural: "Natural Cover (Live Cover)",
      aiFresh: "Artificial Insemination (Fresh)",
      aiFrozen: "Artificial Insemination (Frozen)",
      embryoTransfer: "Embryo Transfer",
    },
    // Outcome labels
    outcome: "Outcome",
    outcomes: {
      pending: "Pending",
      successful: "Successful",
      unsuccessful: "Unsuccessful",
    },
    // Source mode
    source: "Source",
    sourceMode: {
      internal: "Internal",
      connected: "Connected Partner",
      external: "External Provider",
      connectedUnderActivation: "Under activation",
      connectedUnderActivationHelp: "Connected partner selection is not available yet. Use Internal or External for now.",
    },
    performedBy: "Performed By",
    providerName: "Provider Name",
    externalProvider: "External Provider",
    connectedPartner: "Connected Partner",
    // Dialog titles
    addBreedingRecord: "Add Breeding Record",
    selectMare: "Choose mare",
    selectStallion: "Choose stallion",
    selectMethod: "Choose method",
    noMaresAvailable: "No eligible mares registered yet",
    noStallionsAvailable: "No eligible stallions registered yet",
    createMare: "Add new mare",
    createStallion: "Add new stallion",
    addPregnancy: "Add Pregnancy Record",
    addEmbryoTransfer: "Add Embryo Transfer",
    addSemenBatch: "Add Semen Batch",
    hasActivePregnancy: "Has active pregnancy",
    onlyMaresWithoutActive: "Only mares without an active pregnancy can be selected. A mare cannot have more than one active pregnancy at a time.",
    errors: {
      activePregnancyExists: {
        title: "Cannot create pregnancy",
        description: "This mare already has an active pregnancy. Close or end the existing pregnancy before creating a new one.",
      },
    },
    // Pregnancy exams
    pregnancyExams: "Pregnancy Exams",
    addExam: "Add Exam",
    noExams: "No pregnancy exams recorded yet.",
    examMethod: "Method",
    examOutcome: "Outcome",
    examMethods: {
      ultrasound: "Ultrasound",
      palpation: "Palpation",
      blood_test: "Blood Test",
      other: "Other",
    },
    examOutcomes: {
      confirmed_pregnant: "Confirmed Pregnant",
      confirmed_open: "Confirmed Open",
      inconclusive: "Inconclusive",
    },
    // Pregnancy statuses (user-facing)
    pregnancyStatus: {
      open: "Suspected",
      pregnant: "Confirmed Pregnant",
      open_by_abortion: "Open (Abortion)",
      closed: "Closed",
    },
    // Breeding record detail
    detail: {
      title: "Breeding Record Details",
      mare: "Mare",
      stallion: "Stallion",
      method: "Breeding Method",
      date: "Date",
      sourceMode: "Source",
      performedBy: "Performed By",
      providerName: "Provider",
      notes: "Notes",
      externalStallion: "External Stallion",
    },
    // Pregnancy detail
    pregnancyDetail: {
      title: "Pregnancy Details",
      startDate: "Conception Date",
      expectedDue: "Expected Due Date",
      daysPregnant: "Days Pregnant",
      status: "Status",
      verification: "Verification",
      stallion: "Attributed Stallion",
      sourceAttempt: "Source Breeding Record",
      ended: "Ended",
      endReason: "End Reason",
    },
    // Cards
    startedOn: "Started",
    dueOn: "Due",
    days: "days",
    fromRecord: "From record on",
    // Semen
    semen: {
      dosesAvailable: "Doses Available",
      collected: "Collected",
      type: "Type",
      fresh: "Fresh",
      frozen: "Frozen",
      motility: "Motility %",
      concentration: "Concentration (M/ml)",
      qualityNotes: "Quality Notes",
      sourceProvenance: "Provenance",
    },
    // Embryo transfer
    embryoTransfer: {
      donorMare: "Donor Mare",
      recipientMare: "Recipient Mare",
      flushDate: "Flush Date",
      transferDate: "Transfer Date",
      embryoGrade: "Embryo Grade",
      embryoCount: "Embryo Count",
      grades: {
        excellent: "Excellent",
        good: "Good",
        fair: "Fair",
        poor: "Poor",
      },
    },
    // Actions
    actions: {
      markTransferred: "Mark Transferred",
      markCompleted: "Mark Completed",
      markFailed: "Mark Failed",
      markFoaled: "Mark as Foaled",
      markAbortion: "Mark Abortion",
      closeNotPregnant: "Close - Not Pregnant",
      addCheck: "Add Exam",
    },
    // Empty states
    empty: {
      records: "No breeding records yet. Add your first breeding record to get started.",
      pregnancies: "No pregnancy records yet.",
      transfers: "No embryo transfers recorded yet.",
      batches: "No semen batches in inventory yet.",
      horseRecords: "No breeding records for this horse.",
      foalings: "No foaling records yet. Record a foaling from an active pregnancy.",
      foalingsHelp: "Foalings are recorded from an existing pregnancy. Open the Pregnancies tab and choose \"Mark as Foaled\" from an eligible pregnancy.",
      goToPregnancies: "Go to Pregnancies",
      contracts: "No breeding contracts yet. Create your first contract to get started.",
    },
    // Horse profile section
    horseSection: {
      title: "Breeding History",
      titleMare: "Breeding & Reproduction",
      titleStallion: "Breeding Service Records",
      recentRecords: "Recent Breeding Records",
      serviceRecords: "Service Records",
      activePregnancy: "Active Pregnancy",
      noActivePregnancy: "No active pregnancy",
      viewAll: "View All in Breeding Dashboard",
      notEligible: "This horse is not currently designated for breeding.",
      foalingHistory: "Foaling History",
    },
    // Pedigree
    pedigree: {
      grandparents: "Grandparents",
      paternalGrandsire: "Paternal Grandsire",
      paternalGranddam: "Paternal Granddam",
      maternalGrandsire: "Maternal Grandsire",
      maternalGranddam: "Maternal Granddam",
    },
    // Offspring
    offspring: {
      title: "Offspring",
      empty: "No registered offspring.",
    },
    // Foaling
    foaling: {
      recordTitle: "Record Foaling",
      detailTitle: "Foaling Details",
      date: "Foaling Date",
      time: "Foaling Time",
      outcome: "Outcome",
      outcomes: {
        live: "Live Foal",
        stillborn: "Stillborn",
        non_viable: "Non-Viable",
        other: "Other",
      },
      foalSex: "Foal Sex",
      selectSex: "Select sex",
      foalName: "Foal Name",
      foalNamePlaceholder: "Enter foal name",
      foalNameAr: "Foal Name (Arabic)",
      foalNameArPlaceholder: "أدخل اسم المهر بالعربية",
      foalColor: "Foal Color",
      foalColorPlaceholder: "Enter color",
      registerAsHorse: "Register foal as a new horse record",
      registerFoal: "Register Foal",
      registerFoalTitle: "Register Foal as Horse",
      registerFoalDesc: "Create a horse record for this foal with automatic pedigree linkage.",
      register: "Register",
      registered: "Registered",
      registeredHorse: "Registered Horse Record",
      registeredFoalRecord: "Registered Foal Record",
      pedigreeAutoLinked: "Sire and dam will be automatically linked from the breeding record.",
      birthDateAuto: "Birth date will be set from foaling date",
      additionalDetails: "Additional Details",
      notesPlaceholder: "Notes about the foaling...",
      save: "Record Foaling",
      alive: "Alive",
      deceased: "Deceased",
      markDeceased: "Mark as Deceased",
      markAlive: "Mark as Alive",
      registry: {
        title: "Foal Registry Status",
        notification: "Notification",
        blood: "Blood Sample",
        microchip: "Microchip",
        registration: "Registration",
        statuses: {
          pending: "Pending",
          done: "Done",
          na: "N/A",
        },
      },
    },
    // End reasons for pregnancy closure
    endReasons: {
      foaled: "Foaled",
      abortion: "Abortion",
      not_pregnant: "Not Pregnant",
      other: "Other",
    },
    // Demo data notice
    demoNotice: "Showing demo data. Add your first record to see real data.",
    billing: {
      generateInvoice: "Generate Invoice",
      createInvoiceTitle: "Create Breeding Invoice",
      service: "Service",
      selectService: "Select a service...",
      client: "Client",
      selectClient: "Select a client...",
      searchClients: "Search clients...",
      noClientsFound: "No clients found",
      clientNameManual: "Client Name",
      totalAmount: "Total Amount",
      createInvoice: "Create Invoice",
      invoiceCreated: "Invoice created successfully",
      invoiceFailed: "Failed to create invoice",
      sourceTypes: {
        breeding_attempt: "Breeding Record",
        pregnancy_check: "Pregnancy Exam",
        foaling: "Foaling",
        embryo_transfer: "Embryo Transfer",
      },
    },
    contracts: {
      contract: "Contract",
      createTitle: "Create Breeding Contract",
      editTitle: "Edit Breeding Contract",
      detailTitle: "Contract Details",
      contractNumber: "Contract Number",
      type: "Contract Type",
      types: {
        natural_cover: "Natural Cover",
        pregnancy_exam: "Pregnancy Exam",
        foaling_assistance: "Foaling Assistance",
        embryo_transfer: "Embryo Transfer",
        custom: "Custom",
      },
      statuses: {
        draft: "Draft",
        active: "Active",
        completed: "Completed",
        cancelled: "Cancelled",
        expired: "Expired",
      },
      externalParty: "External Party",
      pricingMode: "Pricing Mode",
      pricingModes: {
        fixed: "Fixed",
        per_event: "Per Event",
        package: "Package",
      },
      unitPrice: "Unit Price",
      totalPrice: "Total Price",
      startDate: "Start Date",
      endDate: "End Date",
      changeStatus: "Change Status",
    },
  },
  // Status translations for badges
  status: {
    draft: "Draft",
    pending: "Pending",
    scheduled: "Scheduled",
    in_progress: "In Progress",
    completed: "Completed",
    cancelled: "Cancelled",
    approved: "Approved",
    rejected: "Rejected",
    unknown: "Unknown",
    successful: "Successful",
    unsuccessful: "Unsuccessful",
    open: "Suspected",
    pregnant: "Pregnant",
    open_by_abortion: "Open (Abortion)",
    closed: "Closed",
    planned: "Planned",
    transferred: "Transferred",
    failed: "Failed",
    unverified: "Unverified",
    verified: "Verified",
    active: "Active",
    inactive: "Inactive",
    confirmed: "Confirmed",
    overdue: "Overdue",
    due: "Due",
    administered: "Administered",
    accessioned: "Accessioned",
    processing: "Processing",
    reviewed: "Reviewed",
    final: "Final",
    done: "Done",
    skipped: "Skipped",
  },
  // Severity/Priority translations
  severity: {
    low: "Low",
    medium: "Medium",
    high: "High",
    urgent: "Urgent",
    critical: "Critical",
    normal: "Normal",
  },
  // Scope/Service mode translations
  scope: {
    internal: "Internal",
    external: "External",
    hybrid: "Hybrid",
  },
  // Category translations
  category: {
    treatment: "Treatment",
    procedure: "Procedure",
    checkup: "Checkup",
    dental: "Dental",
    hoof: "Hoof",
    injury: "Injury",
    surgery: "Surgery",
    reproductive: "Reproductive",
    lab: "Lab",
    vaccination: "Vaccination",
    deworming: "Deworming",
    nutrition: "Nutrition",
    farrier: "Farrier",
    general: "General",
    emergency: "Emergency",
    routine: "Routine",
    preventive: "Preventive",
    respiratory: "Respiratory",
    musculoskeletal: "Musculoskeletal",
    other: "Other",
  },
  // Navigation labels
  nav: {
    home: "Home",
    more: "More",
    menu: "Menu",
    settings: "Settings",
    treatments: "Treatments",
    vaccines: "Vaccines",
    visits: "Visits",
    followUps: "Follow-ups",
    back: "Back",
    services: "Services",
  },
  // Global billing keys (for components using billing.collectFinal etc)
  billing: {
    collectIntake: "Collect Intake Payment",
    collectFinal: "Collect Final Payment",
  },
  // Horse Share (External Sharing)
  horseShare: {
    title: "Shared Links",
    create: "Share",
    createTitle: "Share Horse Record",
    createDescription: "Create a shareable link for {{name}}'s medical records.",
    createShare: "Create Share Link",
    pack: "Share Template",
    customize: "Customize sections",
    includeVet: "Include Vet Treatments",
    includeLab: "Include Lab Results",
    includeFiles: "Include Attachments",
    dateFrom: "From Date",
    dateTo: "To Date",
    expiresIn: "Link Expires In",
    expires1Day: "1 Day",
    expires7Days: "7 Days",
    expires30Days: "30 Days",
    expires90Days: "90 Days",
    expiresNever: "Never",
    expiresLabel: "Expires",
    lockToEmail: "Lock to recipient email",
    recipientEmail: "Recipient Email",
    emailLockHint: "Only the specified email can view this link (requires login).",
    shareLink: "Share Link",
    linkCopied: "Link copied to clipboard",
    linkCreatedHint: "Share this link with the recipient. They can view the selected records.",
    noShares: "No active share links",
    activeShares: "Active Shares",
    expiredOrRevoked: "Expired / Revoked",
    statusActive: "Active",
    statusRevoked: "Revoked",
    statusExpired: "Expired",
    customPack: "Custom",
    vetLabel: "Vet",
    labLabel: "Lab",
    filesLabel: "Files",
    revokeTitle: "Revoke Share Link?",
    revokeDescription: "This will permanently disable access to this share link. Recipients will no longer be able to view the records.",
    revoke: "Revoke",
    toasts: {
      shareCreated: "Share link created successfully",
      shareRevoked: "Share link revoked",
      failedToCreate: "Failed to create share link",
      failedToRevoke: "Failed to revoke share link",
    },
    errors: {
      noHorseSelected: "No horse selected",
      not_authenticated: "You must be logged in",
      horse_not_found: "Horse not found",
      permission_denied: "You don't have permission to share this horse",
      share_not_found: "Share link not found",
    },
  },
  // Clients module
  clients: {
    title: "Clients",
    description: "Manage your client database",
    create: "Add Client",
    edit: "Edit Client",
    delete: "Delete Client",
    search: "Search clients...",
    empty: "No clients yet",
    emptyDescription: "Add your first client to get started",
    outstandingBalance: "Outstanding",
    balance: "Balance",
    creditBalance: "Credit Balance",
    availableCredit: "Available Credit",
    creditLimitHelp: "The approved maximum credit available to the customer.",
    availableCreditHelp: "The remaining credit limit after deducting the outstanding amount.",
    backToCustomers: "Back to Customers",
    quickAdd: "Quick Add Client",
    filters: {
      all: "All Clients",
      active: "Active",
      inactive: "Inactive",
      withBalance: "With Balance Due",
    },
    status: {
      active: "Active",
      inactive: "Inactive",
      pending: "Pending",
    },
    types: {
      individual: "Individual",
      organization: "Organization",
      farm: "Farm",
      clinic: "Clinic",
    },
    paymentMethods: {
      cash: "Cash",
      card: "Card",
      transfer: "Bank Transfer",
      check: "Check",
    },
    form: {
      name: "Customer Name",
      namePlaceholder: "Enter customer name",
      nameAr: "Customer Name (Arabic)",
      nameArPlaceholder: "أدخل اسم العميل",
      email: "Email",
      phone: "Phone",
      address: "Address",
      addressPlaceholder: "Enter address",
      type: "Type",
      status: "Status",
      taxNumber: "Tax Number",
      taxNumberPlaceholder: "Enter tax number",
      paymentMethod: "Preferred Payment",
      creditLimit: "Credit Limit",
      notes: "Notes",
      notesPlaceholder: "Additional notes...",
      // Multi-phone support
      phones: "Phone Numbers",
      addPhone: "Add Phone",
      noPhonesAdded: "No phone numbers added",
      whatsapp: "WhatsApp",
      primaryPhone: "Primary",
      setAsPrimary: "Set as Primary",
    },
    phoneLabels: {
      mobile: "Mobile",
      work: "Work",
      home: "Home",
      other: "Other",
    },
    deleteConfirm: {
      title: "Delete Client?",
      description: "This will permanently delete {{name}}. This action cannot be undone.",
    },
    toasts: {
      created: "Client created successfully",
      updated: "Client updated successfully",
      deleted: "Client deleted successfully",
    },
    statement: {
      title: "Account Statement",
      view: "View Statement",
      noEntries: "No transactions found for this period",
      totalDebit: "Total Invoices",
      totalCredit: "Total Amount Paid",
      closingBalance: "Total Due",
      openingBalance: "Opening Balance",
      debit: "Amount Due",
      credit: "Amount Paid",
      balance: "Balance After",
      horsesLabel: "Horses",
      sampleLabel: "Sample",
      itemsLabel: "Items",
      moreHorses: "+{count} more",
      moreItems: "+{count} more",
      showDetails: "Show details",
      hideDetails: "Hide details",
      scopedDebit: "Invoices in Scope",
      scopedCredit: "Paid in Scope",
      scopedBalance: "Outstanding in Scope",
      clientWideOutstanding: "Customer Total Invoices",
      // Slice 2C — Canonical summary card labels
      totalInvoices: "Total Invoices",
      totalPaid: "Total Paid",
      totalOutstanding: "Total Outstanding",
      scopedInvoices: "Invoices in Scope",
      scopedPaid: "Paid in Scope",
      scopedOutstanding: "Outstanding in Scope",
      scopedCreditBalance: "Credit Balance in Scope",
      customerTotalInvoices: "Customer Total Invoices",
      customerTotalOutstanding: "Customer Total Outstanding",
      creditBalance: "Credit Balance",
      scopeExcludesHelper: "Scope totals exclude customer-level activities that are not allocated to a specific horse or service.",
      backToClients: "Back to Customers",
      manageCategories: "Manage Categories",
      sortOldestFirst: "Oldest First",
      sortNewestFirst: "Newest First",
      boardingFrom: "From",
      boardingTo: "To",
      scopeContext: {
        allHorses: "All Horses",
        allCategories: "All Categories",
        horse: "Horse",
        horses: "Horses",
        category: "Category",
      },
      domain: {
        lab: "Laboratory",
        boarding: "Boarding",
        breeding: "Breeding",
        vet: "Veterinary",
        registration: "Registration",
        admin: "Administrative",
        transport: "Transport",
      },
      dateRange: "Date Range",
      print: "Print",
      exportCSV: "Export CSV",
      exportPDF: "Export PDF",
      scope: {
        title: "Statement Scope",
        all: "All",
        dateFrom: "From",
        dateTo: "To",
        selectHorses: "Select Horses",
        selectServices: "Select Services",
        searchHorses: "Search horses...",
        searchServices: "Search services...",
        selectAll: "Select all",
        clear: "Clear",
        generate: "Generate Statement",
        cancel: "Cancel",
        summary: "Selected summary",
        horsesSelected: "{count} horses selected",
        servicesSelected: "{count} services selected",
        domainLabel: "Category",
        domainGeneral: "General / Other",
        horseScope: "Horse Scope",
        allCategories: "All Categories",
        categoriesLabel: "Categories",
        oneCategorySelected: "1 category",
        nCategoriesSelected: "{count} Categories",
        historicalCategories: "Historical Categories",
        historicallyUncategorized: "Historically Uncategorized",
        firstFinancialActivity: "First Financial Activity",
        fromFirstActivity: "From First Activity",
        noFinancialActivity: "No financial activity yet",
        searchCategories: "Search categories...",
        apply: "Apply",
        clearSelections: "Clear Selection",
        newCategory: "New category",
        // 2QA-B — Dedicated selection dialogs
        selectCategoriesTitle: "Select Categories",
        selectHorsesTitle: "Select Horses",
        searchHorsesByIdentifier: "Search by horse name or identifier...",
        allHorses: "All Horses",
        oneHorseSelected: "1 Horse",
        nHorsesSelected: "{count} Horses",
        noMatchingCategories: "No matching categories",
        noMatchingHorses: "No matching horses",
        nSelected: "{count} selected",
        unappliedChanges: "Unapplied changes",
      },
      issuer: "Issued by",
      unallocatedPayments: "Unallocated payments",
      unallocatedCount: "{count} payment(s) not linked to a specific invoice",
    },
  },

  connections: {
    title: "Connections & Sharing",
    description: "Manage connections with other organizations and individuals",
    created: "Connection created successfully",
    accepted: "Connection accepted",
    rejected: "Connection rejected",
    revoked: "Connection revoked",
    tokenCopied: "Connection token copied to clipboard",
    requestSent: "Partnership request sent",
    duplicateRequest: "A partnership request already exists with this partner.",
    requestFailed: "Failed to send partnership request",
    noConnections: "No connections yet",
    createFirst: "Create your first connection",
    searchPlaceholder: "Search connections...",
    create: "New Connection",
    createTitle: "Create Connection",
    createDescription: "Send a connection request to another organization or individual",
    type: "Connection Type",
    types: {
      b2b: "Business to Business",
      b2c: "Business to Customer",
      employment: "Employment",
    },
    typesHelp: {
      b2b: "Connect with another organization (e.g., stable, clinic, lab).",
      b2c: "Invite an individual client (e.g., horse owner) via email or phone.",
      employment: "Link an employee relationship for internal access workflows.",
    },
    recipientMethod: "Contact Method",
    viaEmail: "Via Email",
    viaPhone: "Via Phone",
    copyToken: "Copy Token",
    copyInviteLink: "Copy Invite Link",
    inviteLinkCopied: "Invite link copied to clipboard",
    showQr: "Show QR Code",
    qrTitle: "Scan to Accept Connection",
    revoke: "Revoke",
    accept: "Accept",
    reject: "Reject",
    tenant: "Organization",
    profile: "Profile",
    expiresIn: "Expires in",
    acceptPage: {
      title: "Accept Connection",
      description: "Enter the token to accept or reject a connection request.",
      tokenLabel: "Token",
      tokenFromLink: "Token loaded from invite link",
      secureLabel: "Secure page",
      domainLabel: "Domain:",
      loginRequired: "You must be logged in as the invited recipient to accept or reject this connection.",
      loginCta: "Log in to continue",
    },
    status: {
      pending: "Pending",
      active: "Active",
      accepted: "Accepted",
      revoked: "Revoked",
      expired: "Expired",
      rejected: "Rejected",
    },
    direction: {
      outbound: "Outbound",
      inbound: "Inbound",
    },
    card: {
      noGrants: "No data shared yet",
      grant: "grant",
      grants: "grants",
      createGrant: "Create Data Grant",
      personalProfile: "Personal profile",
      legacyWarning: "Legacy connection — recreate as Partnership",
      recreate: "Recreate",
    },
    addPartner: "Add Partner",
    addPartnerDescription: "Search for an organization to create a partnership connection",
    searchOrganization: "Organization",
    searchOrganizationPlaceholder: "Search organizations...",
    noOrganizationsFound: "No organizations found",
    typeToSearch: "Type to search organizations",
    sendPartnerRequest: "Send Partnership Request",
    presetApplied: "Default sharing permissions applied",
    grants: {
      title: "Data Sharing Grants",
      selectConnection: "Select a connection to manage grants",
      create: "New Grant",
      createTitle: "Create Data Grant",
      createDescription: "Grant access to specific data for this connection",
      noGrants: "No data grants for this connection",
      revoked: "Grant revoked",
      created: "Grant created successfully",
      resourceType: "Resource Type",
      resourceTypes: {
        lab_results: "Lab Results",
        lab_requests: "Lab Requests",
        vet_records: "Vet Records",
        breeding_records: "Breeding Records",
        connection: "Connection",
      },
      accessLevel: "Access Level",
      accessLevels: {
        read: "Read Only",
        write: "Read & Write",
      },
      dateFrom: "From Date",
      dateTo: "To Date",
      forwardOnly: "Forward Only",
      forwardOnlyDesc: "Only share new data created after this grant",
      horsesShared: "horses shared",
      status: {
        active: "Active",
        revoked: "Revoked",
        expired: "Expired",
      },
    },
    portal: {},
    messages: {
      title: "Messages",
      empty: "No messages yet. Start the conversation!",
      placeholder: "Type a message...",
      action: "Messages",
    },
    audit: {
      title: "Activity Log",
      noLogs: "No activity logs yet",
      resources: "resources",
      loadMore: "Load more",
    },
    tabs: {
      connections: "Connections",
      grants: "Data Grants",
      audit: "Activity Log",
      sharedWithMe: "Shared With Me",
    },
    sharedWithMe: {
      title: "Inbound Connections",
      selectConnection: "Select a connection to view shared data",
      previewData: "Preview Data",
      previewTitle: "Shared Data Preview",
      noInboundConnections: "No organizations have shared data with you yet",
      noGrants: "No data has been shared through this connection",
      labResultsCount: "Lab Results",
      horsesCount: "horses",
    },
  },
  
  // Auth Page
  auth: {
    logoutConfirmTitle: "Sign Out",
    logoutConfirmMessage: "Are you sure you want to sign out?",
    welcomeBackTitle: "Welcome back",
    createAccount: "Create your account",
    startManaging: "Start managing your equestrian business today",
    signInToContinue: "Sign in to continue to your dashboard",
    fullName: "Full Name",
    enterFullName: "Enter your full name",
    emailAddress: "Email Address",
    enterEmail: "Enter your email",
    password: "Password",
    enterPassword: "Enter your password",
    forgotPassword: "Forgot password?",
    signIn: "Sign In",
    signUp: "Sign up",
    createAccountBtn: "Create Account",
    signingIn: "Signing in...",
    creatingAccount: "Creating account...",
    hasAccount: "Already have an account?",
    noAccount: "Don't have an account?",
    termsAgreement: "By continuing, you agree to our",
    termsOfService: "Terms of Service",
    privacyPolicy: "Privacy Policy",
    and: "and",
    welcomeBack: "Welcome back!",
    accountCreated: "Account created successfully!",
    testimonial: "Dayli Horse has transformed how we manage our stable. The efficiency gains are incredible.",
    testimonialAuthor: "Mohammed Al-Faisal, Al-Faisal Stables, Riyadh",
    errors: {
      emailExists: "This email is already registered. Please sign in.",
      weakPassword: "Password must be at least 6 characters.",
      createFailed: "Unable to create account",
      invalidCredentials: "Invalid email or password. Please try again.",
      invalidEmail: "Invalid email address. Please check it.",
      rateLimited: "Too many attempts. Please wait a moment.",
      networkError: "Network connection issue. Please check your connection.",
      connectionError: "Unable to connect to server. The supabase.co domain may be blocked on your network.",
      proxyError: "Failed to reach authentication service. Your network appears to be blocking the connection.",
      networkBlocked: "Unable to connect to server. The supabase.co domain appears to be blocked on your network. Try using a VPN or different network (like mobile data).",
    },
    phone: "Phone Number",
    enterPhone: "+966 5XXXXXXXX",
    confirmPassword: "Confirm Password",
    reenterPassword: "Re-enter your password",
    rememberMe: "Remember me",
    clearDataAndRetry: "Clear Data & Retry",
    clearingData: "Clearing data...",
    connectionHelp: "Your network appears to be blocking the server connection. Try using a VPN or a different network (like 4G/5G mobile data):",
    passwordReset: {
      title: "Forgot your password?",
      subtitle: "Enter your email and we'll send you a link to reset your password",
      sendLink: "Send Reset Link",
      sending: "Sending...",
      emailSent: "Reset link sent to your email",
      sendFailed: "Unable to send reset link",
      checkEmail: "Check your email",
      checkEmailDesc: "If this email is registered with us, you'll receive a link to reset your password.",
      backToSignIn: "Back to Sign In",
      newPasswordTitle: "Set a new password",
      newPasswordSubtitle: "Enter your new password below",
      newPassword: "New Password",
      enterNewPassword: "Enter your new password",
      confirmPassword: "Confirm Password",
      confirmNewPassword: "Re-enter your new password",
      updatePassword: "Update Password",
      updating: "Updating...",
      success: "Password updated successfully!",
      successTitle: "Password Updated",
      successDesc: "Redirecting you to sign in...",
      passwordsNoMatch: "Passwords do not match",
      samePassword: "Cannot use the same password",
      updateFailed: "Unable to update password",
      linkExpired: "This link has expired. Please request a new one.",
      invalidLink: "Invalid Link",
      invalidLinkDesc: "This link is invalid or has expired. Please request a new one.",
      requestNewLink: "Request New Link",
    },
  },
  
  // Select Role Page
  selectRole: {
    title: "What describes you best?",
    subtitle: "Select all roles that apply. You can always add more roles later or join additional organizations.",
    horseOwner: "Horse Owner",
    horseOwnerDesc: "I own one or more horses and want to manage their health and care.",
    stableOwner: "Stable Owner",
    stableOwnerDesc: "I own or manage a stable and need to manage horses, staff, and clients.",
    veterinarian: "Veterinarian / Clinic",
    veterinarianDesc: "I run a veterinary clinic and need to manage cases and records.",
    doctor: "Independent Doctor",
    doctorDesc: "I am an independent veterinarian providing services to stables and horse owners.",
    labOwner: "Lab Owner / Staff",
    labOwnerDesc: "I work in a laboratory and need to manage samples and test results.",
    trainer: "Independent Trainer",
    trainerDesc: "I provide independent training services to horses and riders.",
    academy: "Training Academy",
    academyDesc: "I run a training academy with courses, sessions, and students.",
    pharmacy: "Pharmacy",
    pharmacyDesc: "I manage an equine pharmacy and supply medications and supplements.",
    transport: "Transport / Carrier",
    transportDesc: "I provide horse transport and logistics services.",
    auction: "Auction House",
    auctionDesc: "I organize and manage horse auctions and sales events.",
    continue: "Continue",
    roleSelected: "role selected",
    rolesSelected: "roles selected",
    selectAtLeastOne: "Please select at least one role",
  },
  
  // Create Profile Page
  createProfile: {
    createYourProfile: "Create Your",
    steps: {
      basicInfo: "Basic Info",
      location: "Location",
      contact: "Contact",
    },
    basicInformation: "Basic Information",
    locationDetails: "Location Details",
    contactInformation: "Contact Information",
    startWithName: "Start with your",
    helpClientsFind: "Help clients find your",
    howCanPeopleReach: "How can people reach you?",
    uploadLogo: "Upload your",
    nameLabel: "Name",
    enterName: "Enter your",
    description: "Description",
    descriptionPlaceholder: "Tell us about your",
    englishName: "English",
    arabicName: "Arabic",
    enterArabicName: "أدخل الاسم بالعربية",
    descriptionPlaceholderAr: "أخبرنا عن نشاطك بالعربية...",
    horseCapacity: "Horse Capacity",
    maxHorses: "Maximum number of horses",
    streetAddress: "Street Address",
    enterStreetAddress: "Enter your street address",
    city: "City",
    country: "Country",
    saudiArabia: "Saudi Arabia",
    mapComingSoon: "Map integration coming soon",
    phoneNumber: "Phone Number",
    businessEmail: "Business Email",
    websiteOptional: "Website (Optional)",
    continue: "Continue",
    completeSetup: "Complete Setup",
    creating: "Creating...",
    profileCreatedSuffix: "profile created successfully!",
    tenantTypes: {
      stable: "Stable",
      stableDesc: "Manage your stable and horses",
      clinic: "Clinic",
      clinicDesc: "Veterinary clinic management",
      lab: "Laboratory",
      labDesc: "Lab and testing management",
      academy: "Academy",
      academyDesc: "Training and education center",
      horse_owner: "Horse Owner",
      horse_ownerDesc: "Manage your horses and their care",
      pharmacy: "Pharmacy",
      pharmacyDesc: "Equine pharmacy and medication supply",
      transport: "Transport",
      transportDesc: "Horse transport and logistics",
      auction: "Auction House",
      auctionDesc: "Horse auctions and sales management",
      trainer: "Trainer",
      trainerDesc: "Independent horse training services",
      doctor: "Doctor",
      doctorDesc: "Independent veterinary practice",
    },
    errors: {
      validation: "Validation Error",
      tenantInsert: "Failed to create organization",
      memberInsert: "Failed to assign ownership",
    },
  },
  landing: {
    nav: {
      features: "Features",
      directory: "Directory",
      solutions: "Solutions",
      pricing: "Pricing",
      signIn: "Sign In",
      getStarted: "Get Started",
      goToDashboard: "Go to Dashboard",
    },
    hero: {
      badge: "Launching in Saudi Arabia & Gulf Region",
      titlePart1: "The Future of",
      titleHighlight: "Horse Management",
      titlePart2: "is Here",
      subtitle: "A complete ecosystem for stables, clinics, labs, and training academies. Manage your horses, connect with professionals, and grow your equestrian business.",
      startTrial: "Start Free Trial",
      watchDemo: "Watch Demo",
      statHorses: "Horses Managed",
      statStables: "Stables",
      statSatisfaction: "Satisfaction",
    },
    features: {
      badge: "Powerful Features",
      titlePart1: "Everything You Need to",
      titleHighlight: "Excel",
      subtitle: "A comprehensive suite of tools designed for the modern equestrian industry, from individual horse owners to large-scale operations.",
      stable: {
        title: "Stable Management",
        desc: "Complete horse inventory, health records, feeding schedules, and staff management.",
      },
      clinic: {
        title: "Clinic & Veterinary",
        desc: "Digital case management, treatment tracking, and veterinary consultations.",
      },
      lab: {
        title: "Laboratory Services",
        desc: "Sample tracking, test results management, and lab report generation.",
      },
      academy: {
        title: "Training Academy",
        desc: "Student enrollment, lesson scheduling, and progress tracking.",
      },
      transport: {
        title: "Transport Services",
        desc: "Horse transportation booking, route planning, and vehicle management.",
      },
      auctions: {
        title: "Auctions",
        desc: "Online horse auctions, bidding system, and transaction management.",
      },
      multiTenant: {
        title: "Multi-Tenant System",
        desc: "Manage multiple businesses with role-based access and permissions.",
      },
      multiLang: {
        title: "Multi-Language",
        desc: "Support for Arabic, English, Urdu, Hindi, Bengali, and Filipino.",
      },
    },
    solutions: {
      badge: "Built For Everyone",
      titlePart1: "Solutions for",
      titleHighlight: "Every Role",
      subtitle: "Whether you own a single horse or manage a large operation, Dayli Horse adapts to your needs.",
      owners: {
        title: "Horse Owners",
        desc: "Track your horse's health, schedule vet visits, and connect with local services.",
        item1: "Health Records",
        item2: "Vet Scheduling",
        item3: "Expense Tracking",
        item4: "Community Access",
      },
      stables: {
        title: "Stable Owners",
        desc: "Manage your entire operation from boarding to staff to financials.",
        item1: "Boarding Management",
        item2: "Staff Scheduling",
        item3: "Billing & Invoices",
        item4: "Client Portal",
      },
      vets: {
        title: "Veterinarians",
        desc: "Streamline your practice with digital case management and records.",
        item1: "Case Management",
        item2: "Digital Records",
        item3: "Lab Integration",
        item4: "Client Communication",
      },
    },
    pricing: {
      badge: "Simple Pricing",
      titlePart1: "Choose Your",
      titleHighlight: "Plan",
      subtitle: "Flexible pricing that scales with your needs. Start free and upgrade as you grow.",
      popular: "Most Popular",
      perMonth: "/month",
      currency: "SAR",
      free: "Free",
      custom: "Custom",
      starter: {
        name: "Starter",
        desc: "Perfect for individual horse owners",
        feature1: "Up to 3 horses",
        feature2: "Basic health records",
        feature3: "Mobile app access",
        feature4: "Community features",
        cta: "Get Started",
      },
      stable: {
        name: "Stable",
        desc: "For small to medium stables",
        feature1: "Up to 50 horses",
        feature2: "Staff management",
        feature3: "Financial reports",
        feature4: "Clinic integration",
        feature5: "Priority support",
        cta: "Start Free Trial",
      },
      enterprise: {
        name: "Enterprise",
        desc: "For large operations & businesses",
        feature1: "Unlimited horses",
        feature2: "Multi-location support",
        feature3: "Custom integrations",
        feature4: "Dedicated account manager",
        feature5: "SLA guarantee",
        feature6: "White-label options",
        cta: "Contact Sales",
      },
    },
    cta: {
      titlePart1: "Ready to Transform Your",
      titleHighlight: "Equestrian Business",
      subtitle: "Join thousands of horse owners, stables, and veterinarians who trust Dayli Horse to manage their operations. Start your free trial today.",
      startTrial: "Start Free Trial",
      scheduleDemo: "Schedule Demo",
    },
    footer: {
      description: "The complete ecosystem for modern horse management. Built for the Gulf region, ready for the world.",
      platform: "Platform",
      features: "Features",
      pricing: "Pricing",
      solutions: "Solutions",
      mobileApp: "Mobile App",
      company: "Company",
      aboutUs: "About Us",
      careers: "Careers",
      contact: "Contact",
      blog: "Blog",
      support: "Support",
      helpCenter: "Help Center",
      documentation: "Documentation",
      apiReference: "API Reference",
      status: "Status",
      rights: "All rights reserved.",
      privacy: "Privacy Policy",
      terms: "Terms of Service",
    },
  },
  notifications: {
    title: "Notifications",
    subtitle: "Stay updated on partnerships, lab requests and invitations",
    tabs: {
      notifications: "Notifications",
      invitations: "Invitations",
    },
    unread: "Unread",
    read: "Read",
    received: "Received",
    sent: "Sent",
    markAllAsRead: "Mark all as read",
    markAsRead: "Mark as read",
    delete: "Delete",
    close: "Close",
    openFullRecord: "Open full record",
    openRequest: "Open request",
    openConnection: "Open partnership",
    openAdmission: "Open admission",
    openMovement: "Open movements",
    fromOrg: "From organization",
    actor: "Performed by",
    horse: "Horse",
    reference: "Reference",
    families: {
      connection: "Partnerships",
      lab_request: "Laboratory",
      boarding: "Housing",
      movement: "Movements",
      generic: "General",
    },
    summary: {
      status: "Status",
      message: "Message",
      direction: "Direction",
      when: "When",
    },
    direction: {
      in: "Incoming",
      out: "Outgoing",
      transfer: "Transfer",
    },
    notFound: "That notification's record is no longer available here.",
    severity: {
      info: "Info",
      success: "Done",
      warning: "Action",
      critical: "Urgent",
    },
    controlCenter: {
      title: "Notification Control Center",
      description: "Tune how each kind of notification reaches you. Start from a preset, then refine.",
      presetSection: "Starting preset",
      presetSectionDesc: "Pick a profile that matches how you work. You can fine-tune below afterwards.",
      familySection: "Per-area control",
      familySectionDesc: "Choose how loud each notification family should be for you.",
      currentPreset: "Current preset",
      activePreset: "Active",
      switchedToCustom: "Switched to Custom",
      presetApplied: "Preset applied",
    },
    governance: {
      title: "Organization notification governance",
      description: "Set the floor and routing rules for everyone in this organization.",
      readOnlyNotice: "You can view these settings, but only owners and managers can change them.",
      defaultPresetTitle: "Default preset for new members",
      defaultPresetDesc: "Recommended starting point applied when someone joins. Members can refine personally afterwards.",
      floorTitle: "Per-area minimum (floor)",
      floorDesc: "The lowest level a member is allowed to set for each area. Their personal preference can be louder, never quieter. 'No floor' means it's fully personal.",
      floorOptions: {
        none: "No floor",
        important: "At least Important",
        critical: "At least Critical",
        off: "Org-muted",
      },
      routingTitle: "Routing rules",
      routingDesc: "How notifications are routed across the organization.",
      suppressSelfActions: "Suppress self-action notifications",
      suppressSelfActionsDesc: "Don't notify a user about events they triggered themselves.",
      escalateCritical: "Escalate critical events to leadership",
      escalateCriticalDesc: "Owners and managers always receive critical events, even if their personal preset would mute them.",
      precedenceNote: "Order of precedence: organization floor → personal preset → personal per-area level.",
      cardTitle: "Notification governance",
      cardDesc: "Org-wide floor, default preset, escalation, and self-action rules.",
      rolePresetsTitle: "Role-based presets",
      rolePresetsDesc: "When a new member with a matching role joins, they start from this preset instead of the default. Their personal choices later always win.",
      rolePresetsEmpty: "No role-based presets configured. New members start from the default preset above.",
      rolePresetsRoleLabel: "Role key",
      rolePresetsPresetLabel: "Preset",
    },
    levels: {
      all: "All updates",
      important: "Important only",
      critical: "Critical only",
      off: "Off",
      allDesc: "Receive every event in this area.",
      importantDesc: "Only events that need attention or are urgent.",
      criticalDesc: "Only urgent events.",
      offDesc: "Mute this area completely.",
    },
    presets: {
      all: {
        label: "All notifications",
        description: "Get everything across every area. Recommended when you're running things end-to-end.",
      },
      leadership: {
        label: "Leadership",
        description: "Stay across partnerships fully; get only important signals from operations.",
      },
      operations: {
        label: "Operations",
        description: "Day-to-day stable floor focus: movements and admissions front-and-center.",
      },
      lab: {
        label: "Laboratory",
        description: "Focus on lab requests and results; mute most other areas.",
      },
      billing: {
        label: "Billing-adjacent",
        description: "Partnerships and admission/boarding signals (closest billing-relevant streams today). A dedicated finance feed will land in a future update.",
      },
      minimal: {
        label: "Minimal",
        description: "Only urgent events from each area. Quietest setting.",
      },
      custom: {
        label: "Custom",
        description: "Your own per-area mix.",
      },
    },
    noUnread: "No unread notifications",
    noRead: "No read notifications",
    noInvitations: "No pending invitations",
    noSentInvitations: "No invitations sent yet",
    invite: "Invite",
    inviteTitle: "Invite Team Member",
    inviteDescription: "Send an invitation to join",
    emailAddress: "Email Address",
    role: "Role",
    assignHorses: "Assign Horses (optional)",
    sendInvitation: "Send Invitation",
    copyLink: "Copy Link",
    revoke: "Revoke",
    revokeTitle: "Revoke Invitation",
    revokeDescription: "Are you sure? The recipient will no longer be able to accept it.",
    revoking: "Revoking...",
    confirmDecline: "Confirm Decline",
    cancel: "Cancel",
    declineReason: "Reason for declining (optional)",
    from: "From",
    accept: "Accept",
    decline: "Decline",
    invitationAccepted: "Invitation accepted!",
    invitationDeclined: "Invitation declined",
    invitationRevoked: "Invitation revoked",
    inviteLinkCopied: "Invite link copied!",
    failedAccept: "Failed to accept",
    failedDecline: "Failed to decline",
    failedRevoke: "Failed to revoke",
    failedCopyLink: "Failed to copy link",
    enterEmail: "Please enter an email address",
    inviteSent: "Invitation sent successfully",
    inviteFailed: "Failed to send invitation",
    selectRole: "Select role",
    roles: {
      owner: "Owner",
      admin: "Administrator",
      manager: "Manager",
      foreman: "Foreman",
      vet: "Veterinarian",
      trainer: "Trainer",
      employee: "Employee",
    },
    events: {
      connection_request_received: { title: "New Partnership Request", body: "{{actorTenantName}} wants to connect" },
      connection_accepted: { title: "Partnership Accepted", body: "{{actorTenantName}} accepted your request" },
      connection_rejected: { title: "Partnership Declined", body: "{{actorTenantName}} declined your request" },
      lab_request_new: { title: "New Lab Request", body: "{{actorTenantName}} · {{entityLabel}}" },
      lab_request_status_changed: { title: "Lab Request Updated", body: "{{actorTenantName}} · {{statusLabel}}" },
      lab_request_result_published: { title: "Lab Results Available", body: "{{actorTenantName}} · {{entityLabel}}" },
      lab_request_message_added: { title: "New Message", body: "{{actorTenantName}} · {{messagePreview}}" },
      boarding_admission_created: { title: "Horse Admitted", body: "{{horseName}} admitted to boarding" },
      boarding_checkout_initiated: { title: "Checkout Initiated", body: "{{horseName}} · checkout started" },
      boarding_checkout_completed: { title: "Horse Checked Out", body: "{{horseName}} checked out" },
      movement_scheduled: { title: "Departure Scheduled", body: "{{horseName}} · departure scheduled" },
      movement_dispatched: { title: "Horse Dispatched", body: "{{horseName}} dispatched" },
      movement_incoming_pending: { title: "Incoming Horse", body: "From {{actorTenantName}} · {{horseName}}" },
      movement_incoming_confirmed: { title: "Arrival Confirmed", body: "{{actorTenantName}} confirmed arrival · {{horseName}}" },
    },
  },
  teamPartners: {
    title: "Team & Partners",
    subtitle: "Manage your team members and partner organizations",
    partnershipsTitle: "Partnerships & Connected Organizations",
    partnershipsSubtitle: "Manage your partner organizations and connection requests",
    peopleTitle: "People & Invitations",
    peopleSubtitle: "Manage platform members, access, roles and invitations",
    partnersTable: {
      organization: "Organization",
      type: "Organization Type",
      status: "Status",
      direction: "Direction",
      date: "Request Date",
      consents: "Active Consents",
      actions: "Actions",
      incoming: "Incoming",
      outgoing: "Outgoing",
    },
    invitePerson: "Invite Person",
    addPartner: "Add Partner",
    tabs: { people: "People", partners: "Partners" },
    pendingInvitations: "Pending Invitations",
    activeMembers: "Active Members",
    teamMembers: "Team Members",
    receivedInvitations: "Invitations for You",
    noMembers: "No team members yet. Invite someone to get started.",
    noPartners: "No partner organizations yet.",
    unnamed: "Unnamed",
    unknownInvitee: "Unknown",
    unknownPartner: "Unknown Organization",
    configure: "Configure",
    partnerRequestSent: "Partner request sent!",
    status: { pending: "Pending", accepted: "Active", rejected: "Declined", revoked: "Revoked", expired: "Expired" },
    connectionStatus: { pending: "Pending", accepted: "Connected", rejected: "Declined", revoked: "Revoked", expired: "Expired" },
    personStatus: {
      pending: "Pending",
      active: "Platform Access",
      hrOnly: "HR Only",
    },
    classification: {
      internal: "Employee",
      external: "Collaborator",
    },
    counts: {
      total: "Total",
      withAccess: "With Access",
      hrOnly: "HR Only",
    },
    partnerTypes: {
      service: "Service Partner",
      operational: "Operational Partner",
      organization: "Organization",
    },
    invite: {
      title: "Invite Team Member",
      description: "Send an invitation to join",
      byEmail: "By Email",
      byPhone: "By Phone",
      phoneNumber: "Phone Number",
      enterEmail: "Please enter an email address",
      enterPhone: "Please enter a phone number",
      roleHint: "Suggested Role",
      roleHintDesc: "This can be adjusted after the person accepts.",
      missing: {
        email: "Enter an email address",
        phone: "Enter a phone number",
      },
    },
    setup: {
      title: "Configure Member",
      description: "Set role, horse access, and review HR details for this team member.",
      confirmRole: "Role",
      assignHorses: "Horse Access",
      noHorses: "No horses in this organization yet.",
      hrRecord: "HR Record",
      viewInHR: "View in HR / Team Directory",
      hrHint: "Salary, employment details, and classification are managed in the HR module.",
      save: "Save Configuration",
      saved: "Member configuration saved!",
      saveFailed: "Failed to save configuration",
    },
    personDetail: {
      title: "Person Details",
      description: "View and configure this team member's access, role, and HR information.",
      platformAccess: "Platform Access",
      hrRecord: "HR Record",
      inviteToPlatform: "Invite to Platform",
      employmentClassification: "Employment Classification",
      classificationHint: "Internal staff are on payroll. External collaborators work without internal payroll tracking.",
    },
    partnerConfig: {
      title: "Partner Configuration",
      description: "Configure access scope and relationship settings for this partner.",
      partnerType: "Partner Type",
      horseScope: "Horse Access Scope",
      horseScopeDesc: "Select which horses this operational partner can access.",
      accessLevel: "Access Level",
      readOnly: "Read Only",
      readWrite: "Read & Write",
      save: "Save Access",
      accessSaved: "Horse access updated successfully",
      accessSaveFailed: "Failed to update horse access",
      serviceInfo: "This service partner can send and receive requests through the platform. No additional horse-level configuration is needed.",
    },
    partnerDetail: {
      title: "Partner Details",
      description: "View relationship details, permissions, and activity for this partner.",
      dataSharing: "Data Sharing Permissions",
      dataSharingDesc: "Control what data types this partner can access.",
      noPermissions: "No data sharing permissions yet",
      addPermission: "Add Permission",
      addPermissionDesc: "Grant this partner access to a specific type of data.",
      dataType: "Data Type",
      dataTypeHints: {
        lab_results: "Laboratory test results and reports",
        lab_requests: "Laboratory test requests and submissions",
        vet_records: "Veterinary visits, treatments, and vaccinations",
        breeding_records: "Breeding attempts, matings, and reproductive records",
      },
      sharedWithYou: "Shared With You",
      noInboundData: "This partner has not shared any data with you yet",
      activity: "Recent Activity",
      noActivity: "No activity recorded yet",
      events: {
        connection_created: "Partnership created",
        connection_accepted: "Partnership accepted",
        connection_revoked: "Partnership revoked",
        grant_created: "Permission granted",
        grant_revoked: "Permission revoked",
        data_accessed: "Data accessed",
      },
      acceptPartner: "Accept",
      rejectPartner: "Decline",
      pendingInbound: "Wants to connect with you",
    },
  },
  inviteLanding: {
    invalidLink: "Invalid invitation link",
    loadFailed: "Failed to load invitation details. Please try again.",
    acceptFailed: "Failed to accept invitation. Please try again.",
    acceptedSuccess: "Invitation accepted successfully!",
    loadingTitle: "Loading Invitation | Dayli Horse",
    loadingDetails: "Loading invitation details...",
    acceptingTitle: "Accepting Invitation | Dayli Horse",
    acceptingDetails: "Accepting invitation...",
    successTitle: "Invitation Accepted | Dayli Horse",
    welcome: "Welcome!",
    redirecting: "Redirecting to your dashboard...",
    errorTitle: "Invitation Error | Dayli Horse",
    errorHeading: "Invitation Error",
    unableToProcess: "Unable to process invitation",
    errorExplanation: "This invitation may have expired, been revoked, or already been accepted.",
    signIn: "Sign In",
    goHome: "Go to Homepage",
    joinTitle: "Join",
    youreInvited: "You're Invited!",
    invitedToJoin: "You've been invited to join an organization",
    organization: "Organization",
    type: "Type",
    yourRole: "Your Role",
    continueToSignIn: "Continue to Sign In / Sign Up",
    signInHint: "You'll need to sign in or create an account to accept this invitation.",
    errors: {
      expired: "This invitation has expired",
      revoked: "This invitation has been revoked",
      alreadyProcessed: "This invitation has already been processed",
      alreadyDeclined: "This invitation was already declined",
      emailMismatch: "This invitation was sent to a different email address",
      emailUnavailable: "Unable to verify your email. Please update your profile.",
      notFound: "Invitation not found or has already been processed",
      unexpected: "An unexpected error occurred",
    },
  },
  doctor: {
    dashboard: "Doctor Dashboard",
    overviewSubtitle: "Overview of your practice",
    patients: "Patients",
    consultations: "Consultations",
    services: "Services",
    // Stats
    statPatients: "Patients",
    statActiveConsultations: "Active Consultations",
    statTodayFollowups: "Today's Follow-ups",
    statRevenue: "Revenue (Paid)",
    // Overview
    recentConsultations: "Recent Consultations",
    upcomingFollowups: "Upcoming Follow-ups",
    noConsultationsYet: "No consultations yet",
    noUpcomingFollowups: "No upcoming follow-ups",
    noComplaint: "No complaint",
    unknownPatient: "Unknown Patient",
    // Patients page
    searchPatients: "Search patients...",
    showArchived: "Show Archived",
    hideArchived: "Hide Archived",
    addPatient: "Add Patient",
    noPatients: "No patients found. Add your first patient to get started.",
    // Patient form
    editPatient: "Edit Patient",
    nameRequired: "Name *",
    nameArabic: "Name (Arabic)",
    gender: "Gender",
    approxAge: "Approx. Age",
    approxAgePlaceholder: "e.g. 5 years",
    breed: "Breed",
    color: "Color",
    microchip: "Microchip #",
    passport: "Passport #",
    ownerName: "Owner Name",
    ownerPhone: "Owner Phone",
    stableName: "Stable Name",
    // Genders
    stallion: "Stallion",
    mare: "Mare",
    gelding: "Gelding",
    colt: "Colt",
    filly: "Filly",
    // Consultations page
    searchConsultations: "Search consultations...",
    allStatuses: "All statuses",
    newConsultation: "New Consultation",
    noConsultations: "No consultations found.",
    noComplaintRecorded: "No complaint recorded",
    // Consultation detail
    consultationDetail: "Consultation Detail",
    
    updateStatus: "Update Status",
    linkedInvoices: "Linked Invoices",
    consultationNotFound: "Consultation not found.",
    // Consultation form
    editConsultation: "Edit Consultation",
    patient: "Patient *",
    selectPatient: "Select patient",
    noPatientsRegistered: "No patients registered. Add one first.",
    type: "Type",
    priority: "Priority",
    scheduledFor: "Scheduled For",
    chiefComplaint: "Chief Complaint",
    findings: "Findings",
    diagnosis: "Diagnosis",
    recommendations: "Recommendations",
    cost: "Cost",
    createConsultation: "Create Consultation",
    // Consultation types
    checkup: "Checkup",
    emergency: "Emergency",
    followUp: "Follow-up",
    procedure: "Procedure",
    dental: "Dental",
    reproductive: "Reproductive",
    // Priority
    low: "Low",
    normal: "Normal",
    high: "High",
    urgent: "Urgent",
    // Status
    draft: "Draft",
    scheduled: "Scheduled",
    inProgress: "In Progress",
    completed: "Completed",
    cancelled: "Cancelled",
    // Detail labels
    created: "Created",
    // Services page
    showInactive: "Show Inactive",
    addService: "Add Service",
    noServices: "No services configured. Add your first service.",
    editService: "Edit Service",
    basePrice: "Base Price (SAR)",
    category: "Category",
    // Service categories
    catConsultation: "Consultation",
    catProcedure: "Procedure",
    catFollowUp: "Follow-up",
    catEmergency: "Emergency",
    catDental: "Dental",
    catReproductive: "Reproductive",
    selectCategory: "Select category",
    // Prescriptions
    prescriptions: "Prescriptions",
    noPrescriptions: "No prescriptions added.",
    medication: "Medication *",
    dose: "Dose",
    dosePlaceholder: "e.g. 500mg",
    frequency: "Frequency",
    frequencyPlaceholder: "e.g. twice daily",
    durationDays: "Duration (days)",
    addPrescription: "Add Prescription",
    // Follow-ups
    followups: "Follow-ups",
    noFollowups: "No follow-ups scheduled.",
    dateRequired: "Date *",
    scheduleFollowup: "Schedule Follow-up",
    markCompleted: "Mark completed",
    cancelFollowup: "Cancel",
    pending: "pending",
    // Status labels for follow-ups
    followupPending: "pending",
    followupCompleted: "completed",
    followupCancelled: "cancelled",
    // Invoice dialog
    createInvoiceTitle: "Create Invoice from Consultation",
    service: "Service",
    selectService: "Select a service…",
    client: "Client",
    selectClient: "Select a client…",
    searchClients: "Search clients…",
    noClientsFound: "No clients found.",
    clientFallbackHint: "Or type a name below if no client record exists.",
    clientNameManual: "Client Name (manual)",
    clientNamePlaceholder: "Client or stable name",
    totalAmount: "Total Amount",
    creating: "Creating...",
    createInvoice: "Create Invoice",
    invoiceCreated: "Invoice created and linked to consultation",
    invoiceFailed: "Failed to create invoice",
  },
  services: {
    subtitle: "Manage your services catalog and service packages",
    noTenant: "No Business Selected",
    noTenantDesc: "Please select or create a business to manage services.",
    accessRestrictedDesc: "Only owners and managers can manage services.",
    public: "Public",
    private: "Private",
    parentServiceHint: "Link this package to a parent service for grouping",
    tabs: {
      catalog: "Services",
      plans: "Packages",
    },
    kinds: {
      general: "General",
      boarding: "Boarding",
      breeding: "Breeding",
      vet: "Veterinary",
      training: "Training",
      transport: "Transport",
    },
    types: {
      emergency: "Emergency",
      checkup: "Checkup",
      treatment: "Treatment",
      surgery: "Surgery",
      vaccination: "Vaccination",
      consultation: "Consultation",
      procedure: "Procedure",
      follow_up: "Follow-up",
      dental: "Dental",
      reproductive: "Reproductive",
    },
    billingCycles: {
      daily: "Daily",
      weekly: "Weekly",
      monthly: "Monthly",
      yearly: "Yearly",
      oneTime: "One-Time",
    },
    table: {
      type: "Type",
      kind: "Domain",
      unitPrice: "Unit Price",
      plans: "Packages",
      public: "Public",
      active: "Active",
    },
    emptyState: {
      title: "No Services Yet",
      desc: "Create your first service to showcase what your business offers",
    },
    form: {
      addService: "Add Service",
      addTitle: "Add New Service",
      editTitle: "Edit Service",
      addDesc: "Fill in the details to create a new service",
      editDesc: "Update the service details below",
      name: "Service Name (English)",
      namePlaceholder: "e.g., Horse Boarding",
      nameAr: "Service Name (Arabic)",
      nameArPlaceholder: "مثال: إيواء الخيول",
      descPlaceholder: "Describe what this service includes...",
      serviceKind: "Service Domain",
      category: "Category",
      categoryPlaceholder: "e.g., Boarding, Training, Medical",
      categoryDesc: "Optional category for organization",
      unitPrice: "Unit Price",
      unitPriceDesc: "Actual numeric price used for billing",
      priceDisplay: "Price Display",
      priceDisplayPlaceholder: "e.g., Starting from 500 SAR/month",
      priceDisplayDesc: "How the price appears publicly",
      activeDesc: "Service is available",
      publicDesc: "Visible on profile",
      taxable: "Taxable",
      taxableDesc: "When off, no tax is calculated for this service on invoices",
      validation: {
        nameMin: "Name must be at least 2 characters",
        nameArRequired: "Arabic service name is required",
        nameArMin: "Arabic name must be at least 2 characters",
        priceNonNegative: "Price cannot be negative",
      },
    },
    packages: {
      title: "Service Packages",
      addPackage: "Add Package",
      editPackage: "Edit Package",
      empty: "No packages yet",
      createFirst: "Create First Package",
      name: "Package Name (English)",
      nameAr: "Package Name (Arabic)",
      description: "Description",
      basePrice: "Base Price",
      currency: "Currency",
      billingCycle: "Billing Cycle",
      packageType: "Package Type",
      public: "Public",
      types: {
        boarding: "Boarding",
        training: "Training",
        medical: "Medical",
        premium: "Premium",
        wellness: "Wellness",
        commercial: "Commercial",
      },
      selectPackage: "Select a Package (optional)",
      noPackage: "No package (manual rates)",
      parentService: "Parent Service",
      noParentService: "No parent service",
      linkedPackages: "packages",
      subtitle: "Bundle multiple services into packages for your clients",
      dialogDesc: "Define the package identity, pricing structure, and included services",
      sectionIdentity: "Package Identity",
      sectionPricing: "Pricing & Structure",
      sectionComposition: "Included Services",
      namePlaceholder: "e.g., Premium Boarding Package",
      nameArPlaceholder: "مثال: باقة الإيواء المميزة",
      descPlaceholder: "Describe what this package includes...",
      selectType: "Select type",
      noTypeSelected: "No type selected",
      selectCycle: "Select cycle",
      noCycleSelected: "No cycle selected",
      includedServices: "Included Services",
      addIncludedService: "Add Included Service",
      searchServices: "Search services...",
      noServicesFound: "No services found",
      noServicesAvailable: "No services defined yet. Create services first.",
      includedService: "Included",
      validation: {
        nameMin: "Name must be at least 2 characters",
        nameArRequired: "Arabic package name is required",
        nameArMin: "Arabic name must be at least 2 characters",
        typeRequired: "Package type is required",
        cycleRequired: "Billing cycle is required",
        priceNonNegative: "Price cannot be negative",
      },
      toasts: {
        created: "Package created",
        createFailed: "Failed to create package",
        updated: "Package updated",
        updateFailed: "Failed to update package",
        deactivated: "Package deactivated",
        deactivateFailed: "Failed to deactivate package",
      },
      views: {
        switchTitle: "View",
      },
      table: {
        name: "Name",
        parentService: "Parent Service",
        type: "Type",
        billingCycle: "Billing Cycle",
        price: "Base Price",
        included: "Included",
        public: "Public",
        active: "Active",
      },
    },
  },
  pwa: {
    installTitle: "Install Dayli Horse",
    installSubtitle: "Install Dayli Horse for faster access and a better experience",
    installNow: "Install Now",
    notNow: "Not now",
    iosTitle: "Install Dayli Horse",
    iosStep1: "Tap the Share button",
    iosStep2: "Then tap \"Add to Home Screen\"",
    fallbackTitle: "Install Dayli Horse",
    fallbackSubtitle: "Get faster access by installing Dayli Horse on your device",
    fallbackHint: "Use your browser's install option or menu to add Dayli Horse to your device.",
  },
  myProfile: {
    title: "My Profile",
    subtitle: "Manage your personal information and identity",
    personalInfo: "Personal Information",
    fullNameAr: "Name (Arabic)",
    fullNameArPlaceholder: "Enter your name in Arabic",
    fullNameEn: "Name (English)",
    fullNameEnPlaceholder: "Enter your name in English",
    phone: "Phone Number",
    bio: "Bio",
    bioPlaceholder: "Tell us about yourself...",
    locationAndWeb: "Location & Web",
    location: "Location",
    locationPlaceholder: "e.g., Riyadh, Saudi Arabia",
    website: "Website",
    unnamed: "Unnamed User",
    unsavedChanges: "You have unsaved changes",
  },
  publicProfile: {
    title: "Public Profile & Directory",
    subtitle: "Control how your business appears in the public directory",
    visibility: "Visibility Settings",
    makePublic: "Make Profile Public",
    makePublicDesc: "Allow anyone to view your business profile page",
    listInDirectory: "List in Directory",
    listedDesc: "Your business appears in the public directory",
    unlistedDesc: "Hidden from directory, but accessible via direct link",
    publicUrl: "Public URL",
    urlSlug: "Unique URL Slug",
    slugPlaceholder: "your-business-name",
    preview: "Preview",
    available: "Available",
    taken: "Taken",
    publicInfo: "Public Information",
    displayName: "Display Name",
    displayNamePlaceholder: "Business name",
    displayNameDesc: "Leave empty to use your organization name",
    description: "Description",
    descriptionPlaceholder: "Tell visitors about your business...",
    region: "Region",
    selectRegion: "Select region",
    locationDetails: "Location Details",
    locationPlaceholder: "e.g., North Riyadh",
    contactInfo: "Contact Information",
    phoneNumber: "Phone Number",
    emailAddress: "Email Address",
    website: "Website",
    tagsCategories: "Tags & Categories",
    addTag: "Add a tag...",
    suggestedTags: "Suggested tags:",
    saveChanges: "Save Changes",
    saving: "Saving...",
    yourPublicPage: "Your Public Page",
    profileCompleteness: "Profile Completeness",
    checklistSlug: "Unique URL (slug)",
    checklistName: "Business name",
    checklistDescription: "Description",
    checklistLocation: "Location",
    checklistPhone: "Phone number",
    checklistEmail: "Email address",
    checklistLogo: "Logo",
    checklistCover: "Cover image",
    checklistTags: "Tags/Categories",
  },
  inventory: {
    title: "Inventory",
    subtitle: "Track stock items, transactions, and low-stock alerts",
    items: "Items",
    transactions: "Transactions",
    addItem: "Add Item",
    editItem: "Edit Item",
    newTransaction: "New Transaction",
    stockIn: "Stock In",
    stockOut: "Stock Out",
    adjustment: "Adjustment",
    currentQuantity: "Current Quantity",
    lowStockThreshold: "Low Stock Threshold",
    costPerUnit: "Cost per Unit",
    unit: "Unit",
    category: "Category",
    sku: "SKU",
    supplier: "Supplier",
    lowStock: "Low Stock",
    lowStockAlert: "Low Stock Alert",
    lowStockCount: "{{count}} item(s) below threshold",
    noItems: "No inventory items yet",
    noTransactions: "No transactions recorded",
    archive: "Archive",
    restore: "Restore",
    archived: "Archived",
    active: "Active",
    showArchived: "Show archived",
    quantity: "Quantity",
    unitCost: "Unit Cost",
    totalCost: "Total Cost",
    occurredAt: "Date",
    notes: "Notes",
    nameEn: "Name (English)",
    nameAr: "Name (Arabic)",
    selectCategory: "Select category",
    selectUnit: "Select unit",
    selectSupplier: "Select supplier",
    categories: {
      feed: "Feed",
      bedding: "Bedding",
      medication: "Medication",
      equipment: "Equipment",
      supplies: "Supplies",
      other: "Other",
    },
    units: {
      kg: "Kilogram",
      g: "Gram",
      bag: "Bag",
      bale: "Bale",
      liter: "Liter",
      ml: "Milliliter",
      piece: "Piece",
      box: "Box",
      unit: "Unit",
    },
    linkToExpense: {
      title: "Add to inventory stock",
      description: "Optionally record this expense as a stock-in transaction for an inventory item.",
      enable: "Link to an inventory item",
      selectItem: "Select inventory item",
      noItems: "No inventory items — create one from the Inventory page first.",
      quantity: "Quantity received",
      unitCost: "Unit cost (auto from amount)",
    },
  },
  horseOwner: {
    myHorsesTitle: "My Horses",
    comingSoon: "Coming soon",
    soonBadge: "Soon",
    requiresBoardingContract: "These sections will unlock with the upcoming Boarding Contract phase.",
    myHorsesTab: "My Horses",
    notCurrentlyBoarded: "Not currently boarded",
    nav: {
      boardingRequests: "Boarding Requests",
      invoices: "Invoices",
      labResults: "Lab Results",
      providers: "Providers",
    },
    banner: {
      title: "Welcome to your Horse Owner workspace",
      body: "Add horses you own here. Boarding, invoices and lab results will be linked once the Boarding Contract phase is enabled.",
      unhostedCount: "{count} horse(s) are not currently boarded at a stable.",
    },
    hosted: {
      title: "Hosted Horses",
      hostedAt: "Hosted at",
      admittedSince: "Admitted since",
      expectedArrival: "Expected arrival",
      openServiceRequests: "Open service requests",
      placementNotShared: "Placement details not shared yet.",
      phase: {
        awaitingArrival: "Awaiting Arrival",
        arrivalScheduled: "Arrival Scheduled",
        arrivedPendingPlacement: "Arrived, Pending Placement",
        currentlyHosted: "Currently Hosted",
        departed: "Departed",
      },
      empty: {
        title: "No hosted horses yet",
        body: "When a stable admits one of your horses under an active boarding contract, it will appear here.",
      },
      columns: {
        horse: "Horse",
        stable: "Stable",
        phase: "Phase",
        date: "Date",
        openRequests: "Open requests",
        placement: "Placement",
      },
    },
    emptyHorse: {
      notLinked: "This horse is not yet linked to a stable.",
      boardingLater: "Boarding requests will be added in the Boarding Contract phase.",
    },
    dashboard: {
      ownedHorses: "Owned horses",
      unhostedHorses: "Unhosted horses",
      addHorse: "Add horse",
      viewMyHorses: "View my horses",
      comingSoonTitle: "Coming soon",
      comingSoonBoarding: "Boarding requests",
      comingSoonBoardingDesc: "Will unlock with the Boarding Contract phase.",
      comingSoonInvoices: "Invoices",
      comingSoonInvoicesDesc: "Owner-visible invoices arrive in a later phase.",
      comingSoonLabs: "Lab results",
      comingSoonLabsDesc: "Lab results arrive in a later phase.",
      comingSoonProviders: "Providers",
      comingSoonProvidersDesc: "Provider preferences arrive in a later phase.",
    },
  },
  boardingContracts: {
    pageTitle: "Boarding Contracts",
    pageSubtitle: "Manage boarding agreements between horse owners and stables.",
    contractsListTitle: "Contracts",
    contractIdShort: "Contract",
    noContracts: "No boarding contracts yet.",
    unhostedHorsesTitle: "Horses without a stable",
    unhostedHorsesDesc: "Request boarding for any horse that is not currently hosted at a stable.",
    requestBoarding: "Request Boarding",
    requestBoardingDesc: "Send a boarding request to a stable.",
    requestBoardingForHorse: "Send a boarding request for {name}.",
    searchStables: "Search stables",
    searchStablesPlaceholder: "Type a stable name…",
    noStablesFound: "No stables found.",
    sendRequest: "Send request",
    createBoardingContract: "Create Boarding Contract",
    createBoardingContractDesc: "Create a boarding contract for a linked horse owner.",
    ownerCounterparty: "Horse owner",
    ownerSourceHint: "Only linked clients and accepted owner connections are shown. Private horse owner workspaces are not globally searchable.",
    noLinkedOwners: "No linked horse owners.",
    selectOwner: "Select a horse owner",
    selectOwnerFirst: "Select a horse owner first",
    horse: "Horse",
    selectHorse: "Select a horse",
    noHorsesForOwner: "This owner has no horses.",
    boardingPackage: "Boarding package",
    selectPlan: "Select a boarding package",
    noActivePlans: "No active boarding packages.",
    sendToOwner: "Send to owner",
    reviewAndSetPlan: "Review & set plan",
    reviewAndSetPlanDescription: "Review the owner's request, select a boarding package, then send it to the owner for final approval.",
    planRequired: "Please select a boarding package.",
    approveContract: "Approve",
    cancelContract: "Cancel",
    endContract: "End contract",
    status: {
      pending_stable: "Pending stable",
      pending_owner: "Pending owner",
      active: "Active",
      cancelled: "Cancelled",
      ended: "Ended",
    },
    operationalPhase: {
      label: "Operational phase",
      not_started: "Not started",
      awaiting_arrival: "Awaiting arrival",
      arrival_scheduled: "Arrival scheduled",
      arrived_pending_placement: "Arrived — pending placement",
      admitted: "Admitted",
      ended: "Ended",
    },
    scheduleArrival: {
      cta: "Schedule arrival",
      title: "Schedule arrival",
      description: "Set the expected arrival for this boarded horse. This does not start billing.",
      expectedArrival: "Expected arrival",
      branchPreference: "Branch preference (optional)",
      branchPreferencePlaceholder: "e.g. main branch, near track…",
      preferredBranch: "Assign to branch (optional)",
      preferredBranchHint: "Internal only. The owner does not see specific branches.",
      noBranchOption: "No preference",
      notes: "Arrival notes (optional)",
      submit: "Schedule arrival",
      scheduled: "Arrival scheduled.",
      reschedule: "Reschedule arrival",
    },
    arrivalSource: {
      fromBoardingContract: "From boarding contract",
    },
    review: "Review",
    reviewAndApprove: "Review & approve",
    reviewAndApproveTitle: "Review Boarding Contract Summary",
    approvalMeaning: "By approving, this Boarding Contract becomes active.",
    contractSummary: "Contract summary",
    stable: "Stable",
    owner: "Horse owner",
    planSummary: "Plan summary",
    price: "Price",
    billingCycle: "Billing cycle",
    technicalId: "Technical ID",
    displayContextUnavailable: "Details unavailable",
    toasts: {
      created: "Boarding contract created.",
      reused: "Reusing the existing open contract.",
      sentToOwner: "Sent to owner for approval.",
      activated: "Boarding contract activated.",
      cancelled: "Boarding contract cancelled.",
      ended: "Boarding contract ended.",
    },
  },
  serviceRequests: {
    sectionTitle: "Service Requests",
    newRequest: "New request",
    newRequestOwnerDesc: "Send a service request to the stable for this boarded horse.",
    newRequestStableDesc: "Send a request to the horse owner that needs their approval.",
    empty: "No service requests yet.",
    requestType: "Request type",
    notes: "Notes",
    notesPlaceholder: "Describe what is being requested…",
    externalProvider: "External provider (optional)",
    externalProviderPlaceholder: "Name of external provider, if any",
    ownerSuppliedItem: "I will supply the item myself",
    costEstimate: "Cost estimate",
    approvedCost: "Approved cost (optional)",
    billingResponsibility: "Billing responsibility",
    notSpecified: "Not specified",
    submit: "Send request",
    review: "Review",
    reviewTitle: "Review Service Request",
    approve: "Approve",
    reject: "Reject",
    confirmReject: "Confirm rejection",
    rejectionReason: "Rejection reason",
    cancel: "Cancel",
    markFulfilled: "Mark fulfilled",
    createLabRequestCta: "Create Lab Request from Approved Request",
    fulfillmentLabel: "Fulfillment",
    fulfillment: {
      not_required: "Not required",
      pending_fulfillment: "Pending fulfillment",
      fulfilled: "Fulfilled",
    },
    status: {
      pending: "Pending",
      approved: "Approved",
      rejected: "Rejected",
      cancelled: "Cancelled",
      completed: "Completed",
    },
    direction: {
      owner_to_stable: "From horse owner",
      stable_to_owner: "From stable",
    },
    type: {
      extra_lab: "Extra lab",
      extra_vet_visit: "Extra vet visit",
      extra_supplement: "Extra supplement",
      feeding_change: "Feeding change",
      package_change: "Package change",
      movement: "Movement",
      provider_preference: "Provider preference",
      other: "Other",
    },
    billing: {
      owner_pays: "Owner pays",
      included_in_package: "Included in package",
      stable_absorbs: "Stable absorbs",
      deduct_from_prepaid: "Deduct from prepaid",

    },
    toasts: {
      created: "Service request sent.",
      approved: "Service request approved.",
      rejected: "Service request rejected.",
      cancelled: "Service request cancelled.",
      fulfillmentUpdated: "Fulfillment updated.",
    },
    availableAfterArrival: "Available after the horse arrives.",
    typeRequiresArrival: "This request type is only available after the horse arrives.",
  },
  contracts: {
    pageTitle: "Contracts",
    pageSubtitle: "Manage agreements, documents, and contract forms across your workspace.",
    noTypes: "No contract types are available for this workspace yet.",
    tabs: { more: "More" },
    hub: {
      operational: "Operational Contracts",
      documents: "Contract Documents",
      forms: "Contract Forms",
    },
    cta: {
      newContract: "New Contract",
      operationalContract: "Operational Contract",
      operationalContractDesc: "Create a contract tied to an operational flow such as Boarding.",
      contractDocument: "Contract Document",
      contractDocumentDesc: "Create a formatted contract document for review and approval.",
      fromContractForm: "Document from Saved Form",
      fromContractFormDesc: "Create a contract document based on a saved form.",
      noPublishedFormsTitle: "No published contract forms",
      noPublishedFormsDesc: "Create and publish a contract form first, then you can create a document from it.",
    },
    documents: {
      title: "Contract Documents",
      subtitle: "Drafts and sent contracts. Sent documents are frozen snapshots.",
      newDocument: "New Document",
      empty: "No documents.",
      filters: {
        all: "All",
        draft: "Drafts",
        sent_for_review: "Sent for Review",
        approved: "Approved",
        rejected: "Rejected",
        archived: "Archived",
      },
    },
    forms: {
      title: "Contract Forms",
      subtitle: "Reusable rich-text contract forms with bilingual variables.",
      newForm: "New Contract Form",
      empty: "No contract forms yet.",
      createFirst: "Create your first contract form",
      dialogTitle: "New contract form",
      status: { draft: "Draft", published: "Published", archived: "Archived" },
      nameEn: "Name (English)",
      nameAr: "Name (Arabic) — optional",
      contractType: "Contract type",
      placeholderEn: "e.g. Standard Boarding Contract",
      placeholderAr: "مثال: عقد إيواء قياسي",
    },
    documentDialog: {
      titleEn: "Title (English)",
      titleAr: "Title (Arabic) — optional",
      basedOn: "Create from",
      blank: "Blank document",
    },
    types: {
      boarding: { label: "Boarding", badge: "Boarding Contract" },
      training: { label: "Training" },
      reproduction: { label: "Reproduction" },
      custom: { label: "Custom" },
    },
    columns: {
      contract: "Contract",
      type: "Type",
      horse: "Horse",
      counterparty: "Owner / Stable",
      status: "Status",
      operationalPhase: "Operational phase",
      plan: "Plan",
      date: "Date",
      actions: "Actions",
      name: "Name",
      updated: "Updated",
      open: "Open",
    },
    rowActions: { menuLabel: "Actions" },
    detailsSheet: {
      title: "Contract details",
      summaryNote: "Contract summary — operational and reference details only.",
      summary: "Contract Summary",
      parties: "Parties & horse",
      availableActions: "Available actions",
      destructiveActions: "Destructive actions",
    },

    confirmCancel: {
      title: "Cancel this contract?",
      message: "This will mark the contract as Cancelled. Any pending arrival schedule and service requests on this contract will no longer be actionable. This does not automatically adjust invoices or remove the horse from its current housing. This action cannot be undone.",
      confirm: "Cancel contract",
      dismiss: "Keep contract",
    },
    confirmEnd: {
      title: "End this contract?",
      message: "This will mark the contract as Ended. New service requests under this contract will no longer be possible. This does not automatically check the horse out of housing, close open invoices, or settle outstanding balances. This action cannot be undone.",
      confirm: "End contract",
      dismiss: "Keep active",
    },
    editor: {
      loading: "Loading…",
      notFoundDocument: "Document not found.",
      notFoundForm: "Contract form not found.",
      backToDocuments: "Documents",
      backToForms: "Forms",
      bodySection: "Body",
      variablesSection: "Variables",
      variableValuesSection: "Variable values",
      historySection: "History",
      sentAt: "Sent",
      publishedPrefix: "Published",
      frozen: "Frozen",
      missingRequired: "Missing required",
      rejectionPrefix: "Rejection",
      addVariable: "Add variable",
      identitySection: "Identity",
      reviewSection: "Review & status",
      publishingSection: "Publishing",
      noVariables: "No variables yet. Add one to start collecting values.",
      noHistory: "No history yet.",
      filledOfTotal: "{filled} / {total} filled",
      actions: {
        saveDraft: "Save draft",
        sendForReview: "Send for review",
        approve: "Approve",
        reject: "Reject",
        archive: "Archive",
        print: "Print",
        publish: "Publish",
      },
      fields: {
        key: "Key",
        labelEn: "Label (EN)",
        labelAr: "Label (AR)",
        type: "Type",
        required: "Required",
        requiredShort: "Req",
        deleteVariable: "Delete variable",
      },
      variableTypes: {
        text: "Text",
        number: "Number",
        currency: "Currency",
        date: "Date",
        identity_bilingual: "Identity (bilingual)",
      },
      events: {
        created: "Created",
        draft_updated: "Draft updated",
        sent_for_review: "Sent for review",
        approved: "Approved",
        rejected: "Rejected",
        archived: "Archived",
        cancelled: "Cancelled",
        published: "Published",
        unpublished: "Unpublished",
      },
      reject: {
        title: "Reject document",
        description: "Provide a reason. The document goes back to the sender as rejected.",
        placeholder: "Reason for rejection",
        action: "Reject",
      },
      archive: {
        title: "Archive document?",
        description: "The document will be marked as archived. Data is preserved; the document is no longer shown in active lists and cannot be edited.",
        action: "Archive",
      },
      toolbar: {
        bold: "Bold",
        italic: "Italic",
        underline: "Underline",
        heading1: "Heading 1",
        heading2: "Heading 2",
        heading3: "Heading 3",
        paragraph: "Paragraph",
        bulletList: "Bullet list",
        orderedList: "Numbered list",
        alignStart: "Align start",
        alignCenter: "Align center",
        alignEnd: "Align end",
        color: "Text color",
        colorDefault: "Default",
        colorPrimary: "Primary",
        colorMuted: "Muted",
        colorDestructive: "Destructive",
        fontSize: "Font size",
        insertVariable: "Insert variable…",
        undo: "Undo",
        redo: "Redo",
      },
    },
  },
};


