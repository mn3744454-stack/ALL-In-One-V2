-- Migration B rollback — restore `_invoice_items_validate_source` to the
-- pre-Migration-B live baseline captured in File 03.
--
-- Trigger `trg_invoice_items_validate_source` remains bound to `invoice_items`;
-- only the function body is replaced. No table schema, no data, no ACL change.
--
-- To use: run the CREATE OR REPLACE below in a transaction; the trigger will
-- immediately re-use the restored function on next INSERT/UPDATE.
